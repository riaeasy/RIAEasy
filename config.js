...
var riasServerConfig = {
	...
	jettyConfig: {
		...
		minThreads: 4,
		maxThreads: 50,
		maxQueued: 5000,///请求队列长度
		maxFormContentSize: 4 * 1024 * 1024, /// form 提交的最大字节数
		maxUploadSize: 10 * 1024 * 1024,///该参数不是 jetty 参数，是 common-fileupload 所用，如果不设置，RSServer 默认为 0，即不允许上传。
		port: 8080,
		...
		//"sessionCookie": "JSESSIONID",///cookie key-name
		//"sessionHttpOnly": false,
		//"sessionSecureCookies": false,
		"sessionSaveInterval": 15,///秒，持久化间隔(缓存时间)，分布式时，建议为 0.

		requestHeaderSize: 8192,///请求头大小
	...
	appPackage: {
		...
		"app-riaeasy": {
			...
			servletExt: [{
				name: "com.riastudio.server.HelloServlet", ///servlet 类名
				path: "/hello/*"///url path
			}],
			dbs: {
	...
	dbConfigs: {
		"app-riaeasy": {
			...
			url: "jdbc:mysql://localhost:3306/riastudio?autoReconnect=true&allowMultiQueries=true&useUnicode=true&characterEncoding=utf-8&mysqlEncoding=utf8&useSSL=false",
			username: "riaeasy",
			password: "riaeasy",
