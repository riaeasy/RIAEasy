define([
	...
	return rias.declare("rias.riasw.form._ButtonBox", null, {
		...
		templateString:
			'<span class="dijit dijitReset dijitInline dijitLeft" id="widget_${id}" role="combobox" aria-haspopup="true" data-dojo-attach-point="_popupStateNode">'+
				'<span class="riaswTextBoxLabel" data-dojo-attach-point="labelNode" tabIndex="-1" readonly="readonly" role="presentation"></span>'+
				'<span class="dijitInputField riaswTextBoxContainer" data-dojo-attach-point="containerNode,_aroundNode">'+
		...
		forceWidth: false,
		maxHeight: -1,
		//dropDownPosition: ["below", "above"],///不能在这里定义，如果需要默认值，应该在运行期设置。
		...
		toggleDropDown: function(){
			...
			if(!this._opened){
				if(this.onButtonClick != onButtonClick && rias.isFunction(this.onButtonClick)){
					this.onButtonClick();
				}else{
					this.openDropDown();
				}
		...
		openDropDown: function(){
			...
			args.afterLoadedAndShown = function(){
			...
			var _setValue = args.setValueAfterSubmit;
			delete args.setValueAfterSubmit;
			if(!rias.isFunction(_setValue)){
				_setValue = function(moduleResult){
					self.set("value", moduleResult);
				}
			}
			...
			dd.own(rias.before(dd, "afterSubmit", function(){
				_setValue.apply(self, [dd.get("moduleResult")]);
			}));
