	...
	var Widget = rias.declare(riaswType, [ContentPanel], {
		...
		getShowingView: function(){
			...
			for(var i = 0; i < nodes.length; i++){
				var n = nodes[i];
				n = rias.by(n);
				if(n && rias.isInstanceOf(n, Widget) && n.isShown()){
					return n;
				}
				//if(n.nodeType === 1 && n.style.display !== "none" && rias.dom.hasClass(n, "mblView")){
		...
		getSiblingViews: function(){
			...
			return rias.map(rias.filter(this.domNode.parentNode.childNodes, function(n){
				return rias.isInstanceOf(rias.by(n), Widget);// n.nodeType === 1 && rias.isInstanceOf(n, Widget);// rias.dom.hasClass(n, "mblView");
			}), function(n){
				return rias.by(n);//rias.registry.byNode(n);
			});
		},
