...
define([
	...
	var Widget = rias.declare(riaswType, [CaptionPanel], {
		...
		templateString:
			"<div class='dijit dijitReset dijitPopup' role='dialog' data-dojo-attach-event='onmouseenter: _onEnter, onmouseleave: _onBlur'>"+
		...
		buildRendering: function(){
			...
			var p = rias.by(this.popupParent || this.getOwnerRiasw());
			if(p){
		...
		_onShow: function(newState){
			...
			return rias.when(self.inherited(arguments), function(){
				...
				if(self.isShown() && self.focusOnShow && !(self.closeDelay > 0) && !self.isTip()){
		...
		_restore: function(){
			...
			return rias.when(self.inherited(arguments), function(result){
				if(self.isShown() && self.focusOnShow && !(self.closeDelay > 0) && !self.isTip()){///防止无限递归
					///有些时候，show 之前已经 focus，导致 onFocus 时不能 selected
					if(self._playingDeferred){
						rias.when(self._playingDeferred, function(){
							self.focus();
