...
define([
	...
	var Widget = rias.declare(riaswType, [_Widget, _Container, _Contained, _CssStateMixin], {
			...
			if(value === displayClosed){
				...
				return rias.when(this._onClose()).always(function(result){
					if(result === true || result > 0){
						...
						return rias.when(self._playingDeferred || true).always(_always);
					}
					return result;
				});
			}else{
				this._set("displayState", value);
				return rias.when(this._playingDeferred || true).always(_always);
			}
		},
		...
		_doPlay: function(newState, forcePlay){
			...
			function setExpand(value){
				...
				if(cpn){
					...
					rias.forEach(self.baseClass.split(" "), function(cls){
						rias.dom.toggleClass(cpn, cls + "CaptionNodeVertical", isV && !value);
					}, self);
					if(self.captionTextNode){
						rias.dom.toggleClass(self.captionTextNode, "riaswTextVertical", isV && !value);
					}
		...
		restore: function(forceVisible, child){
			...
			return rias.when(d).always(function(){
		...
		isShown: function(excludeShrink){
			return !!this._wasShown &&
				(this.displayState == rias.riasw.layout.displayShowNormal || this.displayState == rias.riasw.layout.displayShowMax
					|| (!excludeShrink && this.displayState == rias.riasw.layout.displayCollapsed));
		},
		isHidden: function(){
