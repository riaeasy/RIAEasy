...
///studio******************************************************************************///
	...
	rias.require([
			...
			try{
				if(args._riaswIdOfModule){
					if(d = rias.ownerModuleBy(args.ownerRiasw)[args._riaswIdOfModule]){
						if(!args.reCreate && rias.isInstanceOf(d, "rias.riasw.layout.DialogPanel")){
							d.focus();
						}else{
							rias.error("已经存在[" + args.ownerRiasw.id + "." + args._riaswIdOfModule + "]");
						}
						return d;
					}
				}
		...
		function _toShowArgs(args, dialogType, contentType, caption, closeDelay, actionBar) {
			var _args;
		...
		rias.show = function(args) {
			...
			if(!rias.isObjectSimple(args)){
				s = "The rias.select\'s parameter type must be Object.";
		...
		rias.message = rias.info = function(args) {
			if(arguments.length > 1){
				args = rias.toArray(arguments).join("\n");
			}
		...
		rias.select = function(args){
			var _args;
			if(!rias.isObjectSimple(args)){
				_args = "The rias.select\'s parameter type must be Object.";
				console.error(_args, args);
				throw _args;
			}
			_args = rias.mixin({}, args);///因为有可能 content 中有实例，最好不用 mixinDeep，防止递归循环
			//_args.actionBar = _args.actionBar || [
