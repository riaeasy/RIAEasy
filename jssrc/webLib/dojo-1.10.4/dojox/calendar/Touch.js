...
define("dojox/calendar/Touch", ["dojo/_base/array", "dojo/_base/lang", "dojo/_base/declare", "dojo/dom", "dojo/dom-geometry", "dojo/_base/window", "dojo/on", "dojo/_base/event", "dojo/keys"], function (arr, lang, declare, dom, domGeometry, win, on, event, keys) {
    return declare("dojox.calendar.Touch", null, {touchStartEditingTimer:500, touchEndEditingTimer:10000, postMixInProperties:function () {
        ...
        if (!p) {
            ...
            this._setEditingProperties({touchMoved:false, item:theItem, renderer:renderer, rendererKind:renderer.rendererKind, event:e, handles:handles, liveLayout:this.liveLayout});
            p = this._edProps;
        }
        if (this._isEditing) {
        ...
        if (p.startEditingTimer && (Math.abs(touch.x - p.start.x) > 25 || Math.abs(touch.y - p.start.y) > 25)) {
            clearTimeout(p.startEditingTimer);
        ...
        if (this._isEditing) {
            ...
            if (!p.touchMoved) {
                ...
                if (this._pendingDoubleTap && this._pendingDoubleTap.item == p.item) {
                    ...
                    this._pendingDoubleTap = {item:p.item, timer:setTimeout(lang.hitch(this, function () {
                        delete this._pendingDoubleTap;
                    }), this.doubleTapDelay)};
        ...
        if (fromResizeStart && fromResizeEnd || this._editingGesture && p.touchesLen == 2 && (fromResizeEnd && p.editKind == "resizeStart" || fromResizeStart && p.editKind == "resizeEnd")) {
            ...
            if (fromResizeStart && p.touchesLen == 1 && !this._editingGesture) {
                this._startItemEditingGesture([this.getTime(e, -1, -1, p.resizeStartTouchIndex)], "resizeStart", "touch", e);
            } else {
                if (fromResizeEnd && p.touchesLen == 1 && !this._editingGesture) {
        ...
        return {touchesLen:len, resizeStartTouchIndex:resizeStartTouchIndex, resizeEndTouchIndex:resizeEndTouchIndex, moveTouchIndex:moveTouchIndex};
    }});
});
