...
define("dojox/calendar/MonthColumnView", ["./ViewBase", "dijit/_TemplatedMixin", "./_ScrollBarBase", "dojo/text!./templates/MonthColumnView.html", "dojo/_base/declare", "dojo/_base/event", "dojo/_base/lang", "dojo/_base/array", "dojo/_base/sniff", "dojo/_base/fx", "dojo/_base/html", "dojo/on", "dojo/dom", "dojo/dom-class", "dojo/dom-style", "dojo/dom-geometry", "dojo/dom-construct", "dojo/mouse", "dojo/query", "dojo/i18n", "dojox/html/metrics"], function (ViewBase, _TemplatedMixin, _ScrollBarBase, template, declare, event, lang, arr, has, fx, html, on, dom, domClass, domStyle, domGeometry, domConstruct, mouse, query, i18n, metrics) {
    return declare("dojox.calendar.MonthColumnView", [ViewBase, _TemplatedMixin], {baseClass:"dojoxCalendarMonthColumnView", templateString:template, viewKind:"monthColumns", _setTabIndexAttr:"domNode", renderData:null, startDate:null, columnCount:6, daySize:30, showCellLabel:true, showHiddenItems:true, verticalRenderer:null, verticalDecorationRenderer:null, percentOverlap:0, horizontalGap:4, columnHeaderFormatLength:null, gridCellDatePattern:null, roundToDay:true, _layoutUnit:"month", _columnHeaderHandlers:null, constructor:function () {
        ...
        if (this.gridCellPattern) {
            ...
            return days[d.getDay()].substring(0, 1) + " " + this.renderData.dateLocaleModule.format(d, {selector:"date", datePattern:format});
        }
    }, scrollPosition:null, scrollBarRTLPosition:"left", _setScrollPositionAttr:function (value) {
        ...
        if (maxDuration) {
            ...
            this._scrollAnimation = new fx.Animation({curve:[this.scrollContainer.scrollTop, position], duration:duration, easing:easing, onAnimate:lang.hitch(this, function (position) {
        ...
        this._onColumnHeaderClick({index:index, date:this.renderData.dates[index][0], triggerEvent:e});
    ...
    }, styleGridCellFunc:null, defaultStyleGridCell:function (node, date, col, row, renderData) {
        if (date == null) {
            return;
        }
        ...
        return pos;
    }, _layoutVerticalItems:function (renderData, index, startTime, endTime, items, itemsType) {
        ...
        for (i = 0; i < layoutItems.length; i++) {
            ...
            if (itemsType === "dataItems") {
                ...
                domStyle.set(ir.container, {"top":item.start + "px", "left":"0", "width":"100%", "height":(item.end - item.start + 1) + "px"});
            }
        ...
        this.inherited(arguments);
    }, _layoutBgItems:function (renderData, col, startTime, endTime, items) {
        var bgItems = {};
        for (var i = 0; i < items.length; i++) {
            var item = items[i];
            var overlap = this.computeRangeOverlap(renderData, item.startTime, item.endTime, startTime, endTime);
