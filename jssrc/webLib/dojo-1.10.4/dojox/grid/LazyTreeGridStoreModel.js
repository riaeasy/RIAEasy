...
define("dojox/grid/LazyTreeGridStoreModel", ["dojo/_base/declare", "dojo/_base/array", "dojo/_base/lang", "dijit/tree/ForestStoreModel"], function (declare, array, lang, ForestStoreModel) {
    return declare("dojox.grid.LazyTreeGridStoreModel", ForestStoreModel, {serverStore:false, constructor:function (args) {
        ...
        if (queryObj) {
            var start = queryObj.start || 0, count = queryObj.count, parentId = queryObj.parentId, sort = queryObj.sort;
            if (parentItem === this.root) {
                this.root.size = 0;
                this.store.fetch({start:start, count:count, sort:sort, query:this.query, onBegin:lang.hitch(this, function (size) {
                    this.root.size = size;
                ...
                if (this.serverStore && !this._isChildrenLoaded(parentItem)) {
                    this.childrenSize = 0;
                    this.store.fetch({start:start, count:count, sort:sort, query:lang.mixin({parentId:parentId}, this.query || {}), onBegin:lang.hitch(this, function (size) {
