...
define("dojox/grid/LazyTreeGrid", ["dojo/_base/kernel", "dojo/_base/declare", "dojo/_base/lang", "dojo/_base/event", "dojo/_base/array", "dojo/query", "dojo/parser", "dojo/dom-construct", "dojo/dom-class", "dojo/dom-style", "dojo/dom-geometry", "dojo/dom", "dojo/keys", "dojo/text!./resources/Expando.html", "dijit/_Widget", "dijit/_TemplatedMixin", "./TreeGrid", "./_Builder", "./_View", "./_Layout", "./cells/tree", "./_RowManager", "./_FocusManager", "./_EditManager", "./DataSelection", "./util"], function (dojo, declare, lang, event, array, query, parser, domConstruct, domClass, domStyle, domGeometry, dom, keys, template, _widget, _templatedMixin, TreeGrid, _Builder, _View, _Layout, TreeCell, _RowManager, _FocusManager, _EditManager, DataSelection, util) {
    var _LazyExpando = declare("dojox.grid._LazyExpando", [_widget, _templatedMixin], {grid:null, view:null, rowIdx:-1, cellIdx:-1, level:0, itemId:"", templateString:template, onToggle:function (evt) {
        ...
        var g = this.grid, item = g._by_idx[this.rowIdx].item;
    ...
    var _TreeGridContentBuilder = declare("dojox.grid._TreeGridContentBuilder", _Builder._ContentBuilder, {generateHtml:function (inDataIndex, rowIndex) {
        var html = this.getTableArray(), grid = this.grid, v = this.view, cells = v.structure.cells, item = grid.getItem(rowIndex), level = 0, toggleClass = "", treePath = grid._treeCache.items[rowIndex] ? grid._treeCache.items[rowIndex].treePath : null;
        ...
        var i = 0, j = 0, row, cell, mergedCells, totalWidth = 0, totalWidthes = [];
        for (; row = cells[j]; j++) {
    ...
    declare("dojox.grid._TreeGridView", _View, {_contentBuilderClass:_TreeGridContentBuilder, postCreate:function () {
    ...
    var LazyTreeCell = lang.mixin(lang.clone(TreeCell), {formatAtLevel:function (item, level, rowIndex) {
        if (!item) {
            return this.formatIndexes(rowIndex, item, level);
        }
        var result = "", ret = "", content;
        if (this.isCollapsable && this.grid.store.isItem(item)) {
            ret = "<span " + dojo._scopeName + "Type=\"dojox.grid._LazyExpando\" level=\"" + level + "\" class=\"dojoxGridExpando\"" + " itemId=\"" + this.grid.store.getIdentity(item) + "\" cellIdx=\"" + this.index + "\"></span>";
        }
        ...
        var info = this.grid.edit.info, d = this.get ? this.get(rowIndex, item) : (this.value || this.defaultValue);
    ...
    var _LazyTreeLayout = declare("dojox.grid._LazyTreeLayout", _Layout, {setStructure:function (structure) {
    ...
    var _LazyTreeGridCache = declare("dojox.grid._LazyTreeGridCache", null, {constructor:function () {
        this.items = [];
        ...
        var i = rowIndex + 1, count, tp, treePath = this.items[rowIndex] ? this.items[rowIndex].treePath : [];
    ...
    var LazyTreeGrid = declare("dojox.grid.LazyTreeGrid", TreeGrid, {_layoutClass:_LazyTreeLayout, _size:0, treeModel:null, defaultState:null, colSpans:null, postCreate:function () {
        this._setState();
        this.inherited(arguments);
        if (!this._treeCache) {
        ...
        var _this = this, selection = this.keepSelection ? this.selection.preserver._selectedById : this.selection.selected;
        return {cache:lang.clone(_this._treeCache), query:lang.clone(_this.query), sortInfo:lang.clone(_this.sortInfo), scrollTop:lang.clone(_this.scrollTop), selection:lang.clone(selection)};
        ...
        if (!addingChild) {
            ...
            var parentItem = parentInfo.item, parentIdty = this.store.getIdentity(parentItem), rowIndex = -1, i = 0;
        ...
        if (fetchedItems.length === count) {
            ...
            var level, nextLevel, len = 1, items = this._treeCache.items, treePath = items[start] ? items[start].treePath : [];
            for (i = 1; i < count; i++) {
        ...
        var onError = lang.hitch(this, "_onFetchError"), start = this._treeCache.getSiblingIndex(req.startRowIdx, req.treePath);
        if (req.treePath.length === 0) {
            this.store.fetch({start:start, startRowIdx:req.startRowIdx, treePath:req.treePath, count:req.count, query:this.query, sort:this.getSortProps(), queryOptions:this.queryOptions, onBegin:lang.hitch(this, "_onFetchBegin"), onComplete:lang.hitch(this, "_onFetchComplete"), onError:lang.hitch(this, "_onFetchError")});
        } else {
            var parentId = req.treePath[req.treePath.length - 1], parentItem;
            var queryObj = {start:start, startRowIdx:req.startRowIdx, treePath:req.treePath, count:req.count, parentId:parentId, sort:this.getSortProps()};
            var _this = this;
            ...
            if (this._by_idty[parentId]) {
                ...
                this.store.fetchItemByIdentity({identity:parentId, onItem:function (item) {
        ...
        var startRowIdx = request.startRowIdx, count = request.count, start = items.length <= count ? 0 : request.start, treePath = request.treePath || [];
        ...
        if (this._loading || !this._by_idx[rowIndex]) {
            return;
        }
        this._loading = true;
        ...
        if (open) {
            var queryObj = {start:0, count:this.rowsPerPage, parentId:this.store.getIdentity(this._by_idx[rowIndex].item), sort:this.getSortProps()};
        ...
        var views = this.views.views, node, rowNode = views[views.length - 1].getRowNode(rowIndex);
        if (rowNode) {
