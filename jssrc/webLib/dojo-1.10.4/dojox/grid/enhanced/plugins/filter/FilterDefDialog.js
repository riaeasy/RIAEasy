...
define("dojox/grid/enhanced/plugins/filter/FilterDefDialog", ["dojo/_base/declare", "dojo/_base/array", "dojo/_base/connect", "dojo/_base/lang", "dojo/_base/event", "dojo/_base/html", "dojo/_base/sniff", "dojo/keys", "dojo/on", "dojo/string", "dojo/window", "dojo/date/locale", "./FilterBuilder", "../Dialog", "dijit/form/ComboBox", "dijit/form/TextBox", "dijit/form/NumberTextBox", "dijit/form/DateTextBox", "dijit/form/TimeTextBox", "dijit/form/Button", "dijit/layout/AccordionContainer", "dijit/layout/ContentPane", "dijit/_Widget", "dijit/_TemplatedMixin", "dijit/_WidgetsInTemplateMixin", "dijit/focus", "dojox/html/metrics", "dijit/a11y", "dojo/text!../../templates/FilterDefPane.html", "dojo/text!../../templates/CriteriaBox.html", "dojo/text!../../templates/FilterBoolValueBox.html", "dijit/Tooltip", "dijit/form/Select", "dijit/form/RadioButton", "dojox/html/ellipsis", "../../../cells/dijit"], function (declare, array, connect, lang, event, html, has, keys, on, string, win, dateLocale, FilterBuilder, Dialog, ComboBox, TextBox, NumberTextBox, DateTextBox, TimeTextBox, Button, AccordionContainer, ContentPane, _Widget, _TemplatedMixin, _WidgetsInTemplateMixin, dijitFocus, metrics, dijitA11y, defPaneTemplate, criteriaTemplate, boolValueTemplate) {
    var _tabIdxes = {relSelect:60, accordionTitle:70, removeCBoxBtn:-1, colSelect:90, condSelect:95, valueBox:10, addCBoxBtn:20, filterBtn:30, clearBtn:40, cancelBtn:50};
    var FilterAccordionContainer = declare("dojox.grid.enhanced.plugins.filter.AccordionContainer", AccordionContainer, {nls:null, addChild:function (child, insertIndex) {
        var pane = arguments[0] = child._pane = new ContentPane({content:child});
        ...
        if ((fromTitle && c == k.UP_ARROW) || (e.ctrlKey && c == k.PAGE_UP)) {
            ...
            if ((fromTitle && c == k.DOWN_ARROW) || (e.ctrlKey && (c == k.PAGE_DOWN || c == k.TAB))) {
                toNext = true;
            } else {
        ...
        (child._removeCBoxBtn = new Button({label:this.nls.removeRuleButton, showLabel:false, iconClass:"dojoxGridFCBoxRemoveCBoxBtnIcon", tabIndex:_tabIdxes.removeCBoxBtn, onClick:lang.hitch(child.content, "onRemove"), onKeyPress:function (e) {
    ...
    }, _hackHeight:function (toGrow, heightDif) {
        var children = this.getChildren(), dn = this.domNode, h = html.style(dn, "height");
        if (!toGrow) {
    ...
    var FilterDefPane = declare("dojox.grid.enhanced.plugins.filter.FilterDefPane", [_Widget, _TemplatedMixin, _WidgetsInTemplateMixin], {templateString:defPaneTemplate, widgetsInTemplate:true, dlg:null, postMixInProperties:function () {
        ...
        (this.cboxContainer = new FilterAccordionContainer({nls:this.plugin.nls})).placeAt(this.criteriaPane);
    ...
    var CriteriaBox = declare("dojox.grid.enhanced.plugins.filter.CriteriaBox", [_Widget, _TemplatedMixin, _WidgetsInTemplateMixin], {templateString:criteriaTemplate, widgetsInTemplate:true, dlg:null, postMixInProperties:function () {
        this.plugin = this.dlg.plugin;
        ...
        return array.map(array.filter(this.plugin.grid.layout.cells, function (cell) {
            ...
            return {label:cell.name || cell.field, value:String(cell.index), selected:colIdx == String(cell.index)};
        });
        ...
        return new cls(lang.mixin(arg, {tabIndex:_tabIdxes.valueBox, onKeyPress:func, onChange:func, "class":"dojoxGridFCBoxValueBox"}));
    }, _createRangeBox:function (cls, arg) {
        var func = lang.hitch(arg.cbox, "_checkValidCriteria");
        lang.mixin(arg, {tabIndex:_tabIdxes.valueBox, onKeyPress:func, onChange:func});
        var div = html.create("div", {"class":"dojoxGridFCBoxValueBox"}), start = new cls(arg), txt = html.create("span", {"class":"dojoxGridFCBoxRangeValueTxt", "innerHTML":this.plugin.nls.rangeTo}), end = new cls(arg);
        ...
        div.get = function () {
            var s = start.get("value"), e = end.get("value");
            return s && e ? {start:s, end:e} : "";
        };
        return div;
    }, changeCurrentColumn:function (selectCurCol) {
        ...
        var colIdx = this._colSelect.get("value"), type = this.dlg.getColumnType(colIdx), value = this.curValue(), cond = this._condSelect.get("value");
        return {"column":colIdx, "condition":cond, "value":value, "formattedVal":this.formatValue(type, cond, value), "type":type, "colTxt":this.curColumn(), "condTxt":this.curCondition()};
    }, load:function (obj) {
        ...
        return this.dlg.getExprForCriteria({"type":this.dlg.getColumnType(colval), "column":colval, "condition":this._condSelect.get("value"), "value":this.curValue()});
    }, isEmpty:function () {
        var cond = this._condSelect.get("value");
        if (cond == "isempty") {
            return false;
        ...
        var title = ["<div class='dojoxEllipsis'>"];
        if (isEmpty || this.isEmpty()) {
            ...
            title.push(column, "&nbsp;<span class='dojoxGridRuleTitleCondition'>", condition, "</span>&nbsp;", value);
            node.title = [column, " ", condition, " ", value].join("");
        }
        node.innerHTML = title.join("");
        if (has("mozilla")) {
            var tt = html.create("div", {"style":"width: 100%; height: 100%; position: absolute; top: 0; left: 0; z-index: 9999;"}, node);
            tt.title = node.title;
        }
        ...
        var colDisabledConds = isNaN(colIdx) ? (this.plugin.args.disabledConditions || {})["anycolumn"] : this.plugin.grid.layout.cells[colIdx].disabledConditions;
        ...
        var vbcls = this.dlg._dataTypeMap[type].valueBoxCls[this._getValueBoxClsInfo(this._colSelect.get("value"), type)], vboxArg = this._getValueBoxArgByType(type);
        ...
        if (type == "string") {
            if (cell && (cell.suggestion || cell.autoComplete)) {
                lang.mixin(res, {store:g.store, searchAttr:cell.field || cell.name, query:g.query || {}, fetchProperties:{sort:[{"attribute":cell.field || cell.name}], queryOptions:lang.mixin({ignoreCase:true, deep:true}, g.queryOptions || {})}});
            }
        } else {
            if (type == "boolean") {
        ...
        if (type == "date" || type == "time") {
            var opt = {selector:type}, fmt = dateLocale.format;
            if (cond == "range") {
        ...
        return v;
    }, _getValueBoxClsInfo:function (colIndex, type) {
    ...
    var UniqueComboBox = declare("dojox.grid.enhanced.plugins.filter.UniqueComboBox", ComboBox, {_openResultList:function (results) {
    ...
    var BooleanValueBox = declare("dojox.grid.enhanced.plugins.filter.BooleanValueBox", [_Widget, _TemplatedMixin, _WidgetsInTemplateMixin], {templateString:boolValueTemplate, widgetsInTemplate:true, constructor:function (args) {
    ...
    var FilterDefDialog = declare("dojox.grid.enhanced.plugins.filter.FilterDefDialog", null, {curColIdx:-1, _relOpCls:"logicall", _savedCriterias:null, plugin:null, constructor:function (args) {
        var plugin = this.plugin = args.plugin;
        this.builder = new FilterBuilder();
        this._setupData();
        this._cboxes = [];
        this.defaultType = plugin.args.defaultType || "string";
        (this.filterDefPane = new FilterDefPane({"dlg":this})).startup();
        (this._defPane = new Dialog({"refNode":this.plugin.grid.domNode, "title":plugin.nls.filterDefDialogTitle, "class":"dojoxGridFDTitlePane", "iconClass":"dojoxGridFDPaneIcon", "content":this.filterDefPane})).startup();
        ...
        var func = function () {
            if (rules.length) {
                ...
                exprs = this.builder.buildExpression(exprs.length == 1 ? exprs[0] : {"op":this._relOpCls, "data":exprs});
        ...
        var cbs = this._cboxes, cc = this.filterDefPane.cboxContainer, total = this.plugin.args.ruleCount, len = cbs.length, cbox;
        if (total > 0 && len + cnt > total) {
            cnt = total - len;
        }
        for (; cnt > 0; --cnt) {
            cbox = new CriteriaBox({dlg:this});
            cbs.push(cbox);
            cc.addChild(cbox);
        }
        cc.startup();
    ...
    }, removeCriteriaBoxes:function (cnt, isIdx) {
        var cbs = this._cboxes, cc = this.filterDefPane.cboxContainer, len = cbs.length, start = len - cnt, end = len - 1, cbox, curIdx = array.indexOf(cbs, cc.selectedChildWidget.content);
        if (lang.isArray(cnt)) {
            var i, idxes = cnt;
            idxes.sort();
            cnt = idxes.length;
        ...
        if (this._savedCriterias && this._savedCriterias[idx]) {
            return lang.mixin({relation:this._relOpCls == "logicall" ? this.plugin.nls.and : this.plugin.nls.or}, this._savedCriterias[idx]);
        }
        return null;
        ...
        if (rule.column == "anycolumn") {
            ...
            return {"op":"logicany", "data":array.map(cells, function (cell) {
        ...
        var cell = this.plugin.grid.layout.cells[colIdx], colName = cell.field || cell.name, obj = {"datatype":type || this.getColumnType(colIdx), "args":cell.dataTypeArgs, "isColumn":true}, operands = [lang.mixin({"data":this.plugin.args.isServerSide ? colName : cell}, obj)];
        obj.isColumn = false;
        if (condition == "range") {
            operands.push(lang.mixin({"data":value.start}, obj), lang.mixin({"data":value.end}, obj));
        } else {
            if (condition != "isempty") {
                operands.push(lang.mixin({"data":value}, obj));
            }
        }
        return {"op":condition, "data":operands};
    ...
    }, _prepareDialog:function (colIndex) {
        ...
        var cbs = this._cboxes, filterCboxes = function (method) {
        ...
        exprs = exprs.length == 1 ? exprs[0] : {"op":this._relOpCls, "data":exprs};
        ...
        var cbs = this._cboxes, defPane = this.filterDefPane;
