...
define("dojox/grid/enhanced/plugins/Search", ["dojo/_base/kernel", "dojo/_base/lang", "dojo/_base/declare", "dojo/_base/array", "dojo/data/util/filter", "../../EnhancedGrid", "../_Plugin"], function (dojo, lang, declare, array, dFilter, EnhancedGrid, _Plugin) {
    var Search = declare("dojox.grid.enhanced.plugins.Search", _Plugin, {name:"search", constructor:function (grid, args) {
        this.grid = grid;
        args = (args && lang.isObject(args)) ? args : {};
        this._cacheSize = args.cacheSize || -1;
        grid.searchRow = lang.hitch(this, "searchRow");
    }, searchRow:function (searchArgs, onSearched) {
        if (!lang.isFunction(onSearched)) {
            return;
        }
    ...
    }, _search:function (searchArgs, start, onSearched, isGlobal) {
        var _this = this, cnt = this._cacheSize, args = {start:start, query:this.grid.query, sort:this.grid.getSortProps(), queryOptions:this.grid.queryOptions, onBegin:function (size) {
    ...
    }, _checkRow:function (item, searchArgs, isGlobal) {
        var g = this.grid, s = g.store, i, field, cells = array.filter(g.layout.cells, function (cell) {
            return !cell.hidden;
        });
        if (isGlobal) {
    ...
    EnhancedGrid.registerPlugin(Search);
    return Search;
});
