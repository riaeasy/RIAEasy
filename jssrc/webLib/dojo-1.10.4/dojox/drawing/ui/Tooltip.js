...
define("dojox/drawing/ui/Tooltip", ["dojo", "../util/oo", "../plugins/_Plugin", "../manager/_registry"], function (dojo, oo, Plugin, registry) {
    var master = null;
    var MasterC = oo.declare(Plugin, function (options) {
        this.createDom();
        ...
        add({x:x, y:y + 5}, {t:"Q", x:x, y:y}, {x:x + r, y:y});
        add({t:"L", x:x2 - r, y:y});
        add({t:"Q", x:x2, y:y}, {x:x2, y:y + r});
        add({t:"L", x:x2, y:y2 - r});
        add({t:"Q", x:x2, y:y2}, {x:x2 - r, y:y2});
        add({t:"L", x:x + r, y:y2});
        add({t:"Q", x:x, y:y2}, {x:x, y:y2 - r});
        add({t:"L", x:x, y:y + r});
    ...
    var Tooltip = oo.declare(Plugin, function (options) {
        if (!master) {
    ...
    registry.register({name:"dojox.drawing.ui.Tooltip"}, "stencil");
    return Tooltip;
});
