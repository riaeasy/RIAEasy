...
define("dojox/fx/scroll", ["dojo/_base/kernel", "dojo/_base/lang", "dojo/_base/fx", "dojox/fx/_base", "dojox/fx/_core", "dojo/dom-geometry", "dojo/_base/sniff"], function (kernel, lang, baseFx, fxExt, Line, domGeom, has) {
    kernel.experimental("dojox.fx.scroll");
    var fx = lang.getObject("dojox.fx", true);
    fxExt.smoothScroll = function (args) {
        if (!args.target) {
            args.target = domGeom.position(args.node);
        }
        var isWindow = lang[(has("ie") ? "isObject" : "isFunction")](args["win"].scrollTo), delta = {x:args.target.x, y:args.target.y};
        if (!isWindow) {
        ...
        var anim = new baseFx.Animation(lang.mixin({beforeBegin:function () {
            if (this.curve) {
                delete this.curve;
            }
