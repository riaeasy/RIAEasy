...
define("dojox/fx/split", ["dojo/_base/lang", "dojo/dom", "dojo/_base/window", "dojo/_base/html", "dojo/dom-geometry", "dojo/dom-construct", "dojo/dom-attr", "dojo/_base/fx", "dojo/fx", "./_base", "dojo/fx/easing", "dojo/_base/connect"], function (lang, dom, winUtil, htmlUtil, domGeom, domConstruct, domAttr, baseFx, coreFx, fxExt, easingUtil, connectUtil) {
    ...
    lang.mixin(dojoxFx, {_split:function (args) {
        ...
        args.pieceAnimation = function (piece, x, y, coords) {
            ...
            var pieceSlide = baseFx.animateProperty({node:piece, duration:duration, delay:delay, easing:(args.easing || (args.unhide ? easingUtil.sinOut : easingUtil.circOut)), beforeBegin:(args.unhide ? function () {
                if (args.fade) {
            ...
            if (args.fade) {
                var pieceFade = baseFx.animateProperty({node:piece, duration:duration, delay:delay, easing:(args.fadeEasing || easingUtil.quadOut), properties:{opacity:(args.unhide ? {start:"0", end:"1"} : {start:"1", end:"0"})}});
        ...
        args.pieceAnimation = function (piece, x, y, coords) {
            ...
            var pieceAnimation = baseFx.animateProperty({node:piece, duration:duration, delay:delay, easing:(args.easing || (args.unhide ? easingUtil.sinIn : easingUtil.circIn)), properties:properties, beforeBegin:(args.unhide ? function () {
                if (args.fade) {
                    htmlUtil.style(piece, {opacity:"0"});
                }
        ...
        args.pieceAnimation = function (piece, x, y, coords) {
            ...
            var pieceAnimation = baseFx.animateProperty({node:piece, duration:duration, delay:delay, easing:(args.easing || easingUtil.sinInOut), properties:properties, beforeBegin:(args.unhide ? function () {
                if (args.fade) {
                    ps.opacity = "0";
                }
                if (colIsOdd == rowIsOdd) {
        ...
        var random = Math.abs(args.random), duration = args.duration - (args.rows + args.columns) * args.interval;
        args.pieceAnimation = function (piece, x, y, coords) {
            var randomDelay = Math.random() * args.duration, uniformDelay = (args.reverseOrder) ? (((args.rows + args.columns) - (x + y)) * Math.abs(args.interval)) : ((x + y) * args.interval), delay = randomDelay * random + Math.max(1 - random, 0) * uniformDelay, pieceAnimation = baseFx.animateProperty({node:piece, duration:duration, delay:delay, easing:(args.easing || easingUtil.sinInOut), properties:{opacity:(args.unhide ? {start:"0", end:"1"} : {start:"1", end:"0"})}, beforeBegin:(args.unhide ? function () {
                htmlUtil.style(piece, {opacity:"0"});
            } : function () {
                piece.style.filter = "";
            })});
        ...
        args.unhide = true;
        return dojoxFx.blockFadeOut(args);
    }});
    return fxExt;
});
