...
define("dojox/help/_base", ["dijit", "dojo", "dojox", "dojo/require!dojox/rpc/Service,dojo/io/script"], function (dijit, dojo, dojox) {
    ...
    dojox.help = {locate:function (searchFor, searchIn, maxResults) {
        ...
        if (!namespaces.length) {
            ...
            dojo.forEach(dojox.help._namespaces, function (item) {
                roots[item] = true;
            });
        }
    ...
    out:
            ...
            if (name && !shorter) {
                ...
                for (var j = 0, variable; variable = names[j]; j++) {
                    if ((name == "window" || variable.indexOf(name + ".") == 0) && variable.toLowerCase().indexOf(searchForLower) != -1) {
                        if (variable.slice(-10) == ".prototype") {
                            continue;
                        }
    ...
    }, refresh:function (namespace, recursive) {
        if (arguments.length < 2) {
            recursive = true;
        }
        dojox.help._recurse(namespace, recursive);
    }, noConflict:function (item) {
        if (arguments.length) {
    ...
    }, init:function (namespaces, noConflict) {
        if (namespaces) {
        ...
        return item;
    }, _namespaces:["dojo", "dojox", "dijit", "djConfig"], _rpc:new dojox.rpc.Service(dojo.moduleUrl("dojox.rpc.SMDLibrary", "dojo-api.smd")), _attributes:["summary", "type", "returns", "parameters"], _clean:function (self) {
        var obj = {};
        ...
        if (!attributes.length || this.__searched__) {
            ...
            dojox.help._rpc.get(dojox.help._addVersion({name:search, exact:true, attributes:attributes})).addCallback(this, function (data) {
    ...
    }, _toStrings:{}, _toString:function (data) {
        if (!this.__source__) {
        ...
        while (names.length) {
            dojox.help._rpc.batch(dojox.help._addVersion({names:names.splice(-50, 50), exact:true, attributes:["parameters"]})).addCallback(this, function (datas) {
    ...
    }, _overrides:[], _recursions:[], _names:{}, _recurse:function (namespace, recursive) {
        if (arguments.length < 2) {
            recursive = true;
        }
        var items = [];
        ...
        for (var key in namespace) {
            ...
            if (typeof item == "undefined" || item === document || item === window || item === dojox.help._toString || item === dojox.help._help || item === null || (+dojo.isIE && item.tagName) || item.__seen__) {
                continue;
            }
