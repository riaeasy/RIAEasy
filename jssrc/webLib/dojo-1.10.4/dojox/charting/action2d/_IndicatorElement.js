...
define("dojox/charting/action2d/_IndicatorElement", ["dojo/_base/lang", "dojo/_base/array", "dojo/_base/declare", "../plot2d/Indicator", "dojo/has", "../plot2d/common", "../axis2d/common", "dojox/gfx"], function (lang, array, declare, Indicator, has) {
    var getXYCoordinates = function (v, values, data) {
        var c2, c1 = v ? {x:values[0], y:data[0][0]} : {x:data[0][0], y:values[0]};
        if (values.length > 1) {
            c2 = v ? {x:values[1], y:data[1][0]} : {x:data[1][0], y:values[1]};
        }
        return [c1, c2];
    };
    var _IndicatorElement = declare("dojox.charting.action2d._IndicatorElement", Indicator, {constructor:function (chart, kwArgs) {
        if (!kwArgs) {
            kwArgs = {};
        }
        ...
        this.inter.onChange({start:coords[0], end:coords[1], label:text});
    ...
    if (0) {
        _IndicatorElement.extend({_checkXCoords:function (cp1, cp2) {
