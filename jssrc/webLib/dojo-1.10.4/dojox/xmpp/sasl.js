...
define("dojox/xmpp/sasl", ["dijit", "dojo", "dojox", "dojo/require!dojox/xmpp/util,dojo/AdapterRegistry,dojox/encoding/digests/MD5"], function (dijit, dojo, dojox) {
    ...
    dojo.declare("dojox.xmpp.sasl._Base", null, {mechanism:null, closeAuthTag:true, constructor:function (session) {
        this.session = session;
        this.startAuth();
    }, startAuth:function () {
        var auth = new dojox.string.Builder(dojox.xmpp.util.createElement("auth", {xmlns:dojox.xmpp.sasl.saslNS, mechanism:this.mechanism}, this.closeAuthTag));
    ...
    dojo.declare("dojox.xmpp.sasl.Plain", dojox.xmpp.sasl._Base, {mechanism:"PLAIN", closeAuthTag:false, appendToAuth:function (auth) {
        var id = this.session.jid;
        var index = this.session.jid.indexOf("@");
        if (index != -1) {
            id = this.session.jid.substring(0, index);
        }
        var token = this.session.jid + "\x00" + id + "\x00" + this.session.password;
        token = dojox.xmpp.util.Base64.encode(token);
        auth.append(token);
        auth.append("</auth>");
        delete this.session.password;
    }});
    dojo.declare("dojox.xmpp.sasl.DigestMD5", dojox.xmpp.sasl._Base, {mechanism:"DIGEST-MD5", onFirstChallenge:function (msg) {
        ...
        A1.append(H(username + ":" + ch.realm + ":" + this.session.password), ":", ch.nonce + ":" + cnonce);
        delete this.session.password;
        var A2_rspauth = ":" + digest_uri + A2_append;
        var A2 = "AUTHENTICATE" + A2_rspauth;
        var response_value = new dojox.string.Builder();
        response_value.append(HEX(A1.toString()), ":", ch.nonce, ":00000001:", cnonce, ":", ch.qop, ":");
        var ret = new dojox.string.Builder();
        ret.append("username=\"", username, "\",", "realm=\"", ch.realm, "\",", "nonce=", ch.nonce, ",", "cnonce=\"", cnonce, "\",", "nc=\"00000001\",qop=\"", ch.qop, "\",digest-uri=\"", digest_uri, "\",", "response=\"", HEX(response_value.toString() + HEX(A2)), "\",charset=\"utf-8\"");
        var response = new dojox.string.Builder(dojox.xmpp.util.createElement("response", {xmlns:dojox.xmpp.xmpp.SASL_NS}, false));
        ...
        if (this.rspauth == ch_str.substring(8)) {
            var response = new dojox.string.Builder(dojox.xmpp.util.createElement("response", {xmlns:dojox.xmpp.xmpp.SASL_NS}, true));
            this.session.dispatchPacket(response.toString());
        } else {
        }
    }});
    dojox.xmpp.sasl.registry = new dojo.AdapterRegistry();
    dojox.xmpp.sasl.registry.register("SUN-COMMS-CLIENT-PROXY-AUTH", function (mechanism) {
    ...
    dojox.xmpp.sasl.registry.register("DIGEST-MD5", function (mechanism) {
        return mechanism == "DIGEST-MD5";
    }, function (mechanism, session) {
        return new dojox.xmpp.sasl.DigestMD5(session);
    });
    dojox.xmpp.sasl.registry.register("PLAIN", function (mechanism) {
