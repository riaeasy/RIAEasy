...
define("dojox/widget/_CalendarMonthYearView", ["dojo/_base/declare", "./_CalendarView", "dijit/_TemplatedMixin", "dojo/query", "dojo/dom-class", "dojo/_base/connect", "dojo/_base/event", "dojo/_base/lang", "dojo/date/locale", "dojo/text!./Calendar/CalendarMonthYear.html"], function (declare, _CalendarView, _TemplatedMixin, query, domClass, connect, event, lang, dojoDateLocale, template) {
    return declare("dojox.widget._CalendarMonthYearView", [_CalendarView, _TemplatedMixin], {templateString:template, datePart:"year", displayedYears:10, useHeader:false, postCreate:function () {
        ...
        var match, monthNames = this._getMonthNames("abbr"), currYear = this.get("value").getFullYear(), currMonth = monthNames[this.get("value").getMonth()], displayedYear = this.get("displayedYear");
        ...
        if (constraints) {
            ...
            query(".dojoxCalendarMonthLabel", this.monthContainer).forEach(lang.hitch(this, function (node, cnt) {
                domClass[(cnt < min || cnt > max) ? "add" : "remove"](node, "dijitCalendarDisabledDate");
            }));
        }
    }, _populateYears:function (year) {
        var match, constraints = this.get("constraints"), thisYear = this.get("value").getFullYear(), dispYear = year || thisYear, firstYear = dispYear - Math.floor(this.displayedYears / 2), min = constraints && constraints.min ? constraints.min.getFullYear() : firstYear - 10000;
        ...
        query(queryNode, this.domNode).forEach(function (node, idx, array) {
            domClass.toggle(node.parentNode, sel, filter(node, idx, array));
        });
        var selMonth = query(".dojoxCal-MY-M-Template div", this.myContainer).filter(function (node) {
