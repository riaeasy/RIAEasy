...
define("dojox/data/StoreExplorer", ["dijit", "dojo", "dojox", "dojo/require!dojox/grid/DataGrid,dojox/data/ItemExplorer,dijit/layout/BorderContainer,dijit/layout/ContentPane"], function (dijit, dojo, dojox) {
    ...
    dojo.declare("dojox.data.StoreExplorer", dijit.layout.BorderContainer, {constructor:function (options) {
        dojo.mixin(this, options);
    }, store:null, columnWidth:"", stringQueries:false, showAllColumns:false, postCreate:function () {
        var self = this;
        this.inherited(arguments);
        var contentPane = new dijit.layout.ContentPane({region:"top"}).placeAt(this);
        ...
        addButton("Add Column", function () {
            ...
            if (columnName) {
                self.gridLayout.push({field:columnName, name:columnName, formatter:dojo.hitch(self, "_formatCell"), editable:true});
                self.grid.attr("structure", self.gridLayout);
            }
        });
        var centerCP = new dijit.layout.ContentPane({region:"center"}).placeAt(this);
        var grid = this.grid = new dojox.grid.DataGrid({store:this.store});
        ...
        var trailingCP = new dijit.layout.ContentPane({region:"trailing", splitter:true, style:"width: 300px"}).placeAt(this);
        var tree = this.tree = new dojox.data.ItemExplorer({store:this.store});
        ...
        grid._onFetchComplete = function (items, req) {
            ...
            for (i = 0; i < idAttributes.length; i++) {
                key = idAttributes[i];
                layout.push({field:key, name:key, _score:100, formatter:formatCell, editable:false});
            }
            for (i = 0; item = items[i++]; ) {
                ...
                for (k = 0; key = keys[k++]; ) {
                    ...
                    if (!found) {
                        layout.push({field:key, name:key, _score:1, formatter:formatCell, styles:"white-space:nowrap; ", editable:true});
                    }
                }
            }
