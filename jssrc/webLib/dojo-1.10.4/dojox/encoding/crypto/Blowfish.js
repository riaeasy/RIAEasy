...
define("dojox/encoding/crypto/Blowfish", ["dojo/_base/lang", "dojo/_base/array", "../base64", "./_base"], function (lang, arrayUtil, base64, crypto) {
    crypto.Blowfish = new function () {
        ...
        function init(key) {
            ...
            var box = {p:arrayUtil.map(boxes.p.slice(0), function (item) {
                var l = k.length, j;
                for (j = 0; j < 4; j++) {
                    data = (data * POW8) | k[pos++ % l];
                }
                return (((item >> 16) ^ (data >> 16)) << 16) | (((item & 65535) ^ (data & 65535)) & 65535);
            }), s0:boxes.s0.slice(0), s1:boxes.s1.slice(0), s2:boxes.s2.slice(0), s3:boxes.s3.slice(0)};
            ...
            switch (out) {
              case crypto.outputTypes.Hex:
                return arrayUtil.map(iv, function (item) {
                    return (item <= 15 ? "0" : "") + item.toString(16);
                }).join("");
        ...
        this.encrypt = function (plaintext, key, ao) {
            ...
            for (var i = 0; i < count; i++) {
                ...
                if (isCBC) {
                    o.left = (((o.left >> 16) ^ (vector.left >> 16)) << 16) | (((o.left & 65535) ^ (vector.left & 65535)) & 65535);
                    o.right = (((o.right >> 16) ^ (vector.right >> 16)) << 16) | (((o.right & 65535) ^ (vector.right & 65535)) & 65535);
                }
                eb(o, bx);
                if (isCBC) {
                    vector.left = o.left;
