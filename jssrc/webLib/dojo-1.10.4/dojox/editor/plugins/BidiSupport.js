...
define("dojox/editor/plugins/BidiSupport", ["dojo/_base/declare", "dojo/_base/array", "dojo/aspect", "dojo/_base/lang", "dojo/dom-attr", "dojo/dom-class", "dojo/dom-construct", "dojo/i18n", "dojo/NodeList-dom", "dojo/NodeList-traverse", "dojo/dom-style", "dojo/sniff", "dojo/query", "dijit", "dojox", "dijit/_editor/_Plugin", "dijit/_editor/range", "dijit/_editor/plugins/EnterKeyHandling", "dijit/_editor/plugins/FontChoice", "./NormalizeIndentOutdent", "dijit/form/ToggleButton", "dojo/i18n!./nls/BidiSupport"], function (declare, array, aspect, lang, domAttr, domClass, domConstruct, i18n, listDom, listTraverse, domStyle, has, query, dijit, dojox, _Plugin, rangeapi, EnterKeyHandling, FontChoice, NormIndentOutdent, ToggleButton) {
    var BidiSupport = declare("dojox.editor.plugins.BidiSupport", _Plugin, {useDefaultCommand:false, buttonClass:null, iconClassPrefix:"dijitAdditionalEditorIcon", command:"bidiSupport", blockMode:"DIV", shortcutonly:false, bogusHtmlContent:"&nbsp;", buttonLtr:null, buttonRtl:null, _indentBy:40, _lineTextArray:["DIV", "P", "LI", "H1", "H2", "H3", "H4", "H5", "H6", "ADDRESS", "PRE", "DT", "DE", "TD"], _lineStyledTextArray:["H1", "H2", "H3", "H4", "H5", "H6", "ADDRESS", "PRE", "P"], _tableContainers:["TABLE", "THEAD", "TBODY", "TR"], _blockContainers:["TABLE", "OL", "UL", "BLOCKQUOTE"], _initButton:function () {
        ...
        this.editor.onLoadDeferred.addCallback(lang.hitch(this, function () {
            var edPlugins = this.editor._plugins, i, p, ind = edPlugins.length, f = false, h = lang.hitch(this, "_changeState", "mirror"), hl = lang.hitch(this, "_changeState", "ltr"), hr = lang.hitch(this, "_changeState", "rtl");
            ...
            for (i = 0; i < edPlugins.length; i++) {
                ...
                if (p.constructor === EnterKeyHandling) {
                    ...
                    if (p.constructor === NormIndentOutdent) {
                        this.editor._normalizeIndentOutdent = true;
                        this.editor._indentImpl = lang.hitch(this, "_indentAndOutdent", "indent");
                        this.editor._outdentImpl = lang.hitch(this, "_indentAndOutdent", "outdent");
                    } else {
                        if (p.constructor === FontChoice && p.command === "formatBlock") {
        ...
        if (curBlock.innerHTML === this.bogusHtmlContent && curBlock.previousSibling) {
            ...
            if (curBlock.innerHTML !== this.bogusHtmlContent && curBlock.previousSibling && curBlock.previousSibling.innerHTML === this.bogusHtmlContent) {
        ...
        var range = sel.getRangeAt(0), tempRange = range.cloneRange(), startNode, endNode, startOffset, endOffset;
        ...
        if (this._isBlockElement(endNode) || this._hasTagFrom(endNode, this._tableContainers)) {
            while (endNode.hasChildNodes()) {
                ...
                if (endNode.hasChildNodes()) {
                    ...
                    if (endNode.nodeType == 3 && endNode.nodeValue) {
        ...
        var parent = node.parentNode, firstSibling, lastSibling, createOwnBlock = false, multiText = false;
        removeOffset = false;
        while (true) {
            var sibling = node;
            ...
            while (true) {
                ...
                if (!sibling) {
                    ...
                    if (this._isBlockElement(sibling) || this._hasTagFrom(sibling, this._blockContainers) || this._hasTag(sibling, "BR")) {
                        createOwnBlock = true;
                        break;
                    } else {
                        if (sibling.nodeType == 3 && sibling.nextSibling.nodeType == 3) {
                            ...
                            if (point == "start" && sibling === tempRange.startContainer) {
                                ...
                                if (point == "end" && (sibling === tempRange.endContainer || sibling.nextSibling === tempRange.endContainer)) {
            ...
            while (true) {
                ...
                if (!sibling) {
                    ...
                    if (this._isBlockElement(sibling) || this._hasTagFrom(sibling, this._blockContainers)) {
                        createOwnBlock = true;
                        break;
                    } else {
                        if (this._hasTag(sibling, "BR") && sibling.nextSibling && !(this._isBlockElement(sibling.nextSibling) || this._hasTagFrom(sibling.nextSibling, this._blockContainers))) {
                            lastSibling = sibling;
                            createOwnBlock = true;
                            break;
                        } else {
                            if (sibling.nodeType == 3 && sibling.previousSibling.nodeType == 3) {
                                sibling.previousSibling.nodeValue += sibling.nodeValue;
                                multiText = true;
                                if (point == "start" && sibling === tempRange.startContainer) {
                                    tempRange.setStart(sibling.previousSibling, 0);
                                } else {
                                    if (point == "end" && (sibling === tempRange.endContainer || sibling.previousSibling === tempRange.endContainer)) {
            ...
            if (createOwnBlock || (this._isBlockElement(parent) && !this._isBlockWithText(parent) && firstSibling)) {
                var origStartOffset = tempRange ? tempRange.startOffset : 0, origEndOffset = tempRange ? tempRange.endOffset : 0, origStartContainer = tempRange ? tempRange.startContainer : null, origEndContainer = tempRange ? tempRange.endContainer : null, divs = this._repackInlineElements(firstSibling, lastSibling, parent), div = divs[point == "start" ? 0 : divs.length - 1];
        ...
        var origStartOffset = tempRange ? tempRange.startOffset : 0, origEndOffset = tempRange ? tempRange.endOffset : 0, origStartContainer = tempRange ? tempRange.startContainer : null, origEndContainer = tempRange ? tempRange.endContainer : null;
        var info = this._collectNodes(startNode, endNode, commonAncestor, tempRange, nodes, origStartContainer, origStartOffset, origEndContainer, origEndOffset, cmd);
        ...
        var node = startNode, sibling, child, parent = node.parentNode, divs = [], firstSibling, lastSibling, groups = [], group = [], cells = [], curTD = this.editor.editNode;
        ...
        while (true) {
            if (this._hasTagFrom(node, this._tableContainers)) {
                ...
                if (this._isBlockElement(node)) {
                    ...
                    if (this._isInlineOrTextElement(node) && !this._hasTagFrom(node.parentNode, this._tableContainers)) {
                        firstSibling = node;
                        while (node) {
        ...
        switch (cmd) {
          ...
          case "rtl":
            array.forEach(nodesInfo.nodes, function (x) {
                var style = domStyle.getComputedStyle(x), curDir = style.direction, oppositeDir = curDir == "ltr" ? "rtl" : "ltr", realDir = (cmd != "mirror" ? cmd : oppositeDir), curAlign = style.textAlign, marginLeft = isNaN(parseInt(style.marginLeft)) ? 0 : parseInt(style.marginLeft), marginRight = isNaN(parseInt(style.marginRight)) ? 0 : parseInt(style.marginRight);
                    ...
                    if (has("webkit")) {
                        ...
                        if (x.firstChild && x.firstChild.tagName) {
                            if (this._hasTagFrom(x.firstChild, this._lineStyledTextArray)) {
                                var style = domStyle.getComputedStyle(x), align = this._refineAlignment(style.direction, style.textAlign);
                                if (has("mozilla")) {
            ...
            query("table", this.editor.editNode).forEach(function (table, idx, array) {
                ...
                for (var i = 0; i < nodesInfo.cells.length; i++) {
                    if (!first && listTD[0] === nodesInfo.cells[i]) {
                        ...
                        if (listTD[listTD.length - 1] === nodesInfo.cells[i]) {
                            last = true;
                            break;
                        }
                    }
          ...
          case "explicitdir":
            array.forEach(nodesInfo.nodes, function (x) {
                var style = domStyle.getComputedStyle(x), curDir = style.direction;
                domAttr.remove(x, "dir");
        ...
        array.forEach(nodesInfo.nodes, function (x, index, arr) {
            if (has("mozilla") || has("webkit")) {
                ...
                var styledSpan;
                if (has("webkit") && this._hasTagFrom(x, this._lineStyledTextArray) || (this._hasTag(x, "LI") && this._hasStyledTextLineTag(x.firstChild))) {
        ...
        array.forEach(nodesInfo.nodes, function (x, index) {
            if (this._hasTag(x, "LI")) {
                if (x.firstChild && x.firstChild.tagName) {
                    if (this._hasTagFrom(x.firstChild, this._lineStyledTextArray)) {
                        var style = domStyle.getComputedStyle(x.firstChild), align = this._refineAlignment(style.direction, style.textAlign);
            ...
            if (has("ie")) {
                ...
                var style = domStyle.getComputedStyle(div), dir = style.direction, align = style.textAlign, curAlign = domStyle.getComputedStyle(x).textAlign;
            ...
            if (this._hasTagFrom(x, this._lineTextArray)) {
                if (this._hasTag(x.parentNode, "DIV") && x.parentNode !== this.editor.editNode) {
                    while (x.parentNode.lastChild) {
                        if (!(x.parentNode.lastChild.nodeType == 3 && lang.trim(x.parentNode.lastChild.nodeValue) == "" || this._hasTag(x.parentNode.lastChild, "BR"))) {
                            break;
                        }
                ...
                if (this._hasTag(x.parentNode, "DIV") && x.parentNode !== this.editor.editNode && x.parentNode.childNodes.length == 1) {
                    var div = x.parentNode, style = domStyle.getComputedStyle(div), align = this._refineAlignment(style.direction, style.textAlign);
                    domStyle.set(x, {direction:style.direction, textAlign:align});
        ...
        while (node) {
            if (this._isInlineOrTextElement(node) && !this._hasTagFrom(node, this._tableContainers)) {
                ...
                if (this._isBlockElement(node) || this._hasTagFrom(node, this._tableContainers)) {
                    if (firstSibling) {
        ...
        if (align.indexOf("left") >= 0 && dir == "rtl") {
            ...
            if (align.indexOf("right") >= 0 && dir == "ltr") {
                align = "right";
            } else {
        ...
        var liDir = domStyle.get(node, "direction"), pDir = domStyle.get(node.parentNode, "direction"), level = 0, tNode = node.parentNode, name, style, offs, val;
        if (has("webkit")) {
        ...
        var parent = node.parentNode, liDir = domStyle.get(node, "direction"), pDir = domStyle.get(parent, "direction"), margin = liDir === "rtl" ? "marginRight" : "marginLeft";
