...
define("dojox/dtl/contrib/data", ["dojo/_base/kernel", "dojo/_base/lang", "../_base", "dojo/_base/array"], function (kernel, lang, dd, array) {
    ...
    ddcd.BindDataNode = lang.extend(function (items, query, store, alias) {
        ...
        if (query) {
            var sync = false;
            store.fetch({query:query, sync:true, scope:this, onComplete:function (it) {
                sync = true;
                items = it;
            }});
            if (!sync) {
    ...
    lang.mixin(ddcd, {_get:function (key) {
        if (this.length) {
