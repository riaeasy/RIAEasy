...
define("dojox/gantt/GanttChart", ["./GanttProjectItem", "./GanttResourceItem", "./GanttProjectControl", "./GanttTaskControl", "./GanttTaskItem", "./TabMenu", "dijit/Tooltip", "dojo/_base/declare", "dojo/_base/array", "dojo/_base/lang", "dojo/date/locale", "dojo/request", "dojo/request/util", "dojo/on", "dojo/dom", "dojo/dom-class", "dojo/dom-construct", "dojo/dom-style", "dojo/dom-attr", "dojo/dom-geometry", "dojo/keys", "dojo/has", "dojo/_base/window", "dojo/json", "dojo/domReady!"], function (GanttProjectItem, GanttResourceItem, GanttProjectControl, GanttTaskControl, GanttTaskItem, TabMenu, Tooltip, declare, arrayUtil, lang, locale, request, util, on, dom, domClass, domConstruct, domStyle, domAttr, domGeometry, keys, has, win, JSON) {
    return declare("dojox.gantt.GanttChart", [], {constructor:function (configuration, node) {
        ...
        if (project) {
            ...
            project.nextProject && project.shiftNextProject(project, -rowHeight);
            ...
            if (this.project.length == 0) {
                ...
                var pi = new GanttProjectItem({id:1, name:"New Project", startDate:t});
        ...
        var project = new GanttProjectItem({id:id, name:name, startDate:startDate});
        this.project.push(project);
        var _project = new GanttProjectControl(this, project);
        for (var i = 0; i < this.arrProjects.length; i++) {
            var curProject = this.arrProjects[i], preProject = this.arrProjects[i - 1], nextProject = this.arrProjects[i + 1];
        ...
        if (task.parentTask) {
            if ((!task.parentTask.isExpanded) || (task.parentTask.cTaskNameItem[2].style.display == "none")) {
        ...
        request.get(_this.dataFilePath, {sync:true}).then(function (response) {
        ...
        arrayUtil.forEach(items, function (pItem) {
            ...
            var project = new GanttProjectItem({id:pItem.id, name:pItem.name, startDate:new Date(startDate[0], (parseInt(startDate[1]) - 1), startDate[2])});
            var tItems = pItem.tasks;
            arrayUtil.forEach(tItems, function (tItem) {
                var id = tItem.id, name = tItem.name, starttime = tItem.starttime.split("-"), duration = tItem.duration, percentage = tItem.percentage, previousTaskId = tItem.previousTaskId, taskOwner = tItem.taskOwner;
                var task = new GanttTaskItem({id:id, name:name, startTime:new Date(starttime[0], (parseInt(starttime[1]) - 1), starttime[2]), duration:duration, percentage:percentage, previousTaskId:previousTaskId, taskOwner:taskOwner});
        ...
        arrayUtil.forEach(this.project, function (proj) {
            var project = {id:proj.id, name:proj.name, startdate:proj.startDate.getFullYear() + "-" + (proj.startDate.getMonth() + 1) + "-" + proj.startDate.getDate(), tasks:[]};
            jsonObj.items.push(project);
            arrayUtil.forEach(proj.parentTasks, function (pTask) {
                var task = {id:pTask.id, name:pTask.name, starttime:pTask.startTime.getFullYear() + "-" + (pTask.startTime.getMonth() + 1) + "-" + pTask.startTime.getDate(), duration:pTask.duration, percentage:pTask.percentage, previousTaskId:(pTask.previousTaskId || ""), taskOwner:(pTask.taskOwner || ""), children:this.getChildTasksData(pTask.cldTasks)};
        ...
        childTasks && childTasks.length > 0 && arrayUtil.forEach(childTasks, function (childTask) {
            var ctask = {id:childTask.id, name:childTask.name, starttime:childTask.startTime.getFullYear() + "-" + (childTask.startTime.getMonth() + 1) + "-" + childTask.startTime.getDate(), duration:childTask.duration, percentage:childTask.percentage, previousTaskId:(childTask.previousTaskId || ""), taskOwner:(childTask.taskOwner || ""), children:this.getChildTasksData(childTask.cldTasks)};
            cTaskObj.push(ctask);
        }, this);
        return cTaskObj;
        ...
        try {
            request.post(_this.saveProgramPath, {query:{filename:_this.dataFilePath, data:JSON.stringify(_this.getJSONData())}}).response.then(function (response) {
                if ((util.checkStatus(response.options.status)) || (response.options.status == 405)) {
        ...
        var panelTask = domConstruct.create("div", {className:"ganttTaskPanel"});
        domStyle.set(panelTask, {height:(this.contentHeight - this.panelTimeHeight - this.scrollBarWidth) + "px"});
        return panelTask;
        ...
        var tblHeader = domConstruct.create("table", {cellPadding:"0px", border:"0px", cellSpacing:"0px", bgColor:"#FFFFFF", className:"ganttToolbar"}, panelHeader);
        ...
        var zoomIn = domConstruct.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarZoomIn"}, firstRow);
        ...
        var zoomOut = domConstruct.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarZoomOut"}, firstRow);
        ...
        var micro = domConstruct.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarMicro"}, secondRow);
        ...
        var tele = domConstruct.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarTele"}, secondRow);
        ...
        var save = domConstruct.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarSave"}, thirdRow);
        ...
        var load = domConstruct.create("td", {align:"center", vAlign:"middle", className:"ganttToolbarLoad"}, thirdRow);
        ...
        var actions = [zoomIn, zoomOut, micro, tele, save, load], titles = ["Enlarge timeline", "Shrink timeline", "Zoom in time zone(microscope view)", "Zoom out time zone(telescope view)", "Save gantt data to json file", "Load gantt data from json file"];
        ...
        var panelNameTask = domConstruct.create("div", {innerHTML:"&nbsp;", className:"ganttPanelNames"});
        domStyle.set(panelNameTask, {height:(this.contentHeight - this.panelTimeHeight - this.scrollBarWidth) + "px", width:this.maxWidthPanelNames + "px"});
        return panelNameTask;
    }, createPanelTime:function () {
        var panelTime = domConstruct.create("div", {className:"ganttPanelTime"});
        var tblTime = domConstruct.create("table", {cellPadding:"0px", border:"0px", cellSpacing:"0px", bgColor:"#FFFFFF", className:"ganttTblTime"}, panelTime);
        ...
        var maxEndPos = arrayUtil.map(this.arrProjects, function (project) {
            return (parseInt(project.projectItem[0].style.left) + parseInt(project.projectItem[0].firstChild.style.width) + project.descrProject.offsetWidth + this.panelTimeExpandDelta);
        ...
        var data = "Year   " + year;
        var newCell = domConstruct.create("td", {colSpan:count, align:"center", vAlign:"middle", className:"ganttYearNumber", innerHTML:this.pixelsPerDay * count > 20 ? data : "", innerHTMLData:data}, row);
        ...
        var newCell = domConstruct.create("td", {colSpan:count, align:"center", vAlign:"middle", className:"ganttMonthNumber", innerHTML:this.pixelsPerDay * count > 30 ? data : "", innerHTMLData:data}, row);
        domStyle.set(newCell, "width", (this.pixelsPerDay * count) + "px");
    }, addWeekInPanelTime:function (row, count, week) {
        var data = "Week   " + week;
        var newCell = domConstruct.create("td", {colSpan:count, align:"center", vAlign:"middle", className:"ganttWeekNumber", innerHTML:this.pixelsPerDay * count > 20 ? data : "", innerHTMLData:data}, row);
        domStyle.set(newCell, "width", (this.pixelsPerDay * count) + "px");
    }, addDayInPanelTime:function (row) {
        var date = new Date(this.startDate);
        date.setDate(date.getDate() + parseInt(row.cells.length));
        var newCell = domConstruct.create("td", {align:"center", vAlign:"middle", className:"ganttDayNumber", innerHTML:this.pixelsPerDay > 20 ? date.getDate() : "", innerHTMLData:String(date.getDate()), data:row.cells.length}, row);
        domStyle.set(newCell, "width", this.pixelsPerDay + "px");
        (date.getDay() >= 5) && domClass.add(newCell, "ganttDayNumberWeekend");
        this._events.push(on(newCell, "mouseover", lang.hitch(this, function (event) {
        ...
        this._events.push(on(newCell, "mouseout", lang.hitch(this, function (event) {
            var dayTime = event.target || event.srcElement;
            dayTime && dijit.hideTooltip(dayTime);
        })));
    }, addHourInPanelTime:function (row) {
        var newCell = domConstruct.create("td", {align:"center", vAlign:"middle", className:"ganttHourNumber", data:row.cells.length}, row);
        domStyle.set(newCell, "width", this.pixelsPerDay + "px");
        var hourTable = domConstruct.create("table", {cellPadding:"0", cellSpacing:"0"}, newCell);
        var newRow = hourTable.insertRow(hourTable.rows.length);
        for (var i = 0; i < this.hsPerDay; i++) {
            var hourTD = domConstruct.create("td", {className:"ganttHourClass"}, newRow);
        ...
        for (var i = 0; i < parenttask.cldTasks.length; i++) {
            if (parenttask.cldTasks[i].cldTasks.length > 0) {
                this.sortChildTasks(parenttask.cldTasks[i]);
            }
        }
        return parenttask;
        ...
        this.startDate = this.getStartDate();
        domStyle.set(this.content, {width:this.contentWidth + "px", height:this.contentHeight + "px"});
        this.tableControl = domConstruct.create("table", {cellPadding:"0", cellSpacing:"0", className:"ganttTabelControl"});
        ...
        this.contentData.appendChild(this.createPanelTasks());
        var newCellTblControl = domConstruct.create("td", {vAlign:"top"});
        this.panelNameHeaders = domConstruct.create("div", {className:"ganttPanelNameHeaders"}, newCellTblControl);
        domStyle.set(this.panelNameHeaders, {height:this.panelTimeHeight + "px", width:this.maxWidthPanelNames + "px"});
        ...
        newCellTblControl = domConstruct.create("td", {vAlign:"top"});
        ...
        if (!this.panelNameHeadersCover) {
            ...
            domStyle.set(this.panelNameHeadersCover, {left:cpos.l + "px", top:cpos.t + "px", height:cpos.h + "px", width:cpos.w + "px", display:"none"});
        }
    }, postBindEvents:function () {
        var pos = domGeometry.position(this.tableControl, true);
        has("dom-addeventlistener") && this._events.push(on(this.tableControl, "mousemove", lang.hitch(this, function (event) {
            ...
            if (elem == this.panelNames.firstChild || elem == this.contentData.firstChild) {
                ...
                if (hlTop != this.oldHLTop && hlTop < (pos.h - 50)) {
                    if (this.highLightDiv) {
                        ...
                        this.highLightDiv = domConstruct.create("div", {className:"ganttRowHighlight"}, win.body());
                        domStyle.set(this.highLightDiv, {top:(pos.y + hlTop) + "px", left:pos.x + "px", width:(pos.w - 20) + "px", height:rowHeight + "px"});
                    }
                }
                this.oldHLTop = hlTop;
            }
