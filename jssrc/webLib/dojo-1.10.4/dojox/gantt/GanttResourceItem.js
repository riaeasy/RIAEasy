...
define("dojox/gantt/GanttResourceItem", ["dojo/_base/declare", "dojo/_base/array", "dojo/_base/lang", "dojo/date/locale", "dojo/request", "dojo/on", "dojo/dom", "dojo/dom-class", "dojo/dom-construct", "dojo/dom-style", "dojo/dom-attr", "dojo/dom-geometry", "dojo/keys", "dojo/domReady!"], function (declare, arrayUtil, lang, locale, request, on, dom, domClass, domConstruct, domStyle, domAttr, domGeometry, keys) {
    return declare("dojox.gantt.GanttResourceItem", [], {constructor:function (ganttchart) {
        ...
        this.ownerTimeConsume = this.buildOwnerTimeConsume();
        this.tableControl = domConstruct.create("table", {cellPadding:"0", cellSpacing:"0", className:"ganttResourceTableControl"});
        ...
        newRowTblControl.appendChild(newCellTblControl);
        domStyle.set(this.panelNames, {height:(this.contentHeight - this.ganttChart.panelTimeHeight - this.ganttChart.scrollBarWidth) + "px", width:this.ganttChart.maxWidthPanelNames + "px"});
        ...
        var resourceHeader = domConstruct.create("div", {innerHTML:"Resource Chart:", className:"ganttResourceHeader"}, this.ganttChart.content, "after");
        ...
        domStyle.set(content, {width:this.ganttChart.contentWidth + "px", height:(this.ganttChart.resourceChartHeight || (this.ganttChart.contentHeight * 0.8)) + "px"});
        ...
        var ownerName = domConstruct.create("div", {id:owner, title:owner, innerHTML:owner, className:"ganttOwnerNameItem"});
        domStyle.set(ownerName, "top", posY + "px");
        return ownerName;
    }, refreshOwnerItem:function (owner) {
        var item = this.ownerTaskNodeMapping[owner][owner][0], start = this.ownerTimeConsume[owner].min, dur = this.ownerTimeConsume[owner].dur, posX = this.ganttChart.getPosOnDate(start);
        item.style.left = posX + "px";
        item.style.width = dur * this.ganttChart.pixelsPerWorkHour + "px";
        arrayUtil.forEach(this.resourceInfo[owner], function (task, i) {
            var tposX = this.ganttChart.getPosOnDate(task.taskItem.startTime);
            domStyle.set(item.childNodes[i], {left:(tposX - posX) + "px", width:task.taskItem.duration * this.ganttChart.pixelsPerWorkHour + "px"});
        }, this);
    }, createOwnerItem:function (owner, posY) {
        var start = this.ownerTimeConsume[owner].min, dur = this.ownerTimeConsume[owner].dur;
        var posX = this.ganttChart.getPosOnDate(start);
        var ownerControl = domConstruct.create("div", {id:owner, owner:true, className:"ganttOwnerBar"});
        domStyle.set(ownerControl, {left:posX + "px", top:posY + "px", width:dur * this.ganttChart.pixelsPerWorkHour + "px", height:this.ganttChart.heightTaskItem + "px"});
        arrayUtil.forEach(this.resourceInfo[owner], function (task) {
            var ownerTaskItem = domConstruct.create("div", {id:owner, className:"ganttOwnerTaskBar"}, ownerControl);
            var tposX = this.ganttChart.getPosOnDate(task.taskItem.startTime);
            domStyle.set(ownerTaskItem, {left:(tposX - posX) + "px", width:task.taskItem.duration * this.ganttChart.pixelsPerWorkHour + "px", height:this.ganttChart.heightTaskItem + "px"});
        }, this);
        return ownerControl;
        ...
        if (this.panelNames) {
            ...
            taskItems.push({"v":arrConnectingLinesNames[0], "h":arrConnectingLinesNames[1]});
            this.checkWidthTaskNameItem(this.ownerNameItem[this.ownerNameItem.length - 1]);
        }
        return taskItems;
    }, createTaskNameItem:function (owner, posY) {
        var taskNameItem = domConstruct.create("div", {id:owner, className:"ganttTaskNameItem", title:owner, innerHTML:owner});
        domStyle.set(taskNameItem, "top", posY + "px");
        return taskNameItem;
    }, refreshTaskItem:function (item, task) {
        var posX = this.ganttChart.getPosOnDate(task.taskItem.startTime);
        domStyle.set(item, {left:posX + "px", width:task.taskItem.duration * this.ganttChart.pixelsPerWorkHour + "px"});
    }, createTaskItem:function (task, posY) {
        var posX = this.ganttChart.getPosOnDate(task.taskItem.startTime);
        var itemControl = domConstruct.create("div", {id:task.taskItem.name, className:"ganttTaskBar"});
        domStyle.set(itemControl, {left:posX + "px", top:posY + "px", width:task.taskItem.duration * this.ganttChart.pixelsPerWorkHour + "px", height:this.ganttChart.heightTaskItem + "px"});
        return itemControl;
    }, createConnectingLinesPN:function (parentNode, currentNode) {
        var arrConnectingLinesNames = [];
        var lineVerticalLeft = domConstruct.create("div", {innerHTML:"&nbsp;", className:"ganttResourceLineVerticalLeft"}, this.panelNames.firstChild);
        lineVerticalLeft.cNode = currentNode;
        lineVerticalLeft.pNode = parentNode;
        var LineHorizontalLeft = domConstruct.create("div", {noShade:true, color:"#000", className:"ganttResourceLineHorizontalLeft"}, this.panelNames.firstChild);
        ...
        var treeImg = domConstruct.create("div", {id:ownerNameItem.id, className:"ganttImageTreeExpand"});
        ...
        domStyle.set(treeImg, {left:(domStyle.get(ownerNameItem, "left") - 12) + "px", top:(domStyle.get(ownerNameItem, "top") + 3) + "px"});
        return treeImg;
    }, styleOwnerItem:function (tItem, owner, displayType, topOffset) {
        if (tItem.v || tItem.h) {
            domStyle.set(tItem.v, {height:Math.max(1, (tItem.v.cNode.offsetTop - tItem.v.pNode.offsetTop)) + "px", top:(tItem.v.pNode.offsetTop + 5) + "px", left:(tItem.v.pNode.offsetLeft - 9) + "px", display:displayType});
            domStyle.set(tItem.h, {width:Math.max(1, (tItem.h.cNode.offsetLeft - tItem.h.pNode.offsetLeft + 4)) + "px", top:(tItem.h.cNode.offsetTop + 5) + "px", left:(tItem.h.pNode.offsetLeft - 9) + "px", display:displayType});
        } else {
            domStyle.set(tItem, {display:displayType, top:parseInt(owner.style.top) + topOffset + "px"});
        }
        ...
        if (taskNameItem && taskNameItem.offsetWidth + taskNameItem.offsetLeft > this.ganttChart.maxWidthPanelNames) {
            var width = taskNameItem.offsetWidth + taskNameItem.offsetLeft - this.ganttChart.maxWidthPanelNames, countChar = Math.round(width / (taskNameItem.offsetWidth / taskNameItem.firstChild.length)), tName = taskNameItem.id.substring(0, taskNameItem.firstChild.length - countChar - 3);
            taskNameItem.innerHTML = tName + "...";
        }
    }, createPanelOwners:function () {
        var panelOwner = domConstruct.create("div", {className:"ganttOwnerPanel"});
        domStyle.set(panelOwner, {height:(this.contentHeight - this.ganttChart.panelTimeHeight - this.ganttChart.scrollBarWidth) + "px"});
        return panelOwner;
    }, createPanelNamesOwners:function () {
        var panelNameOwner = domConstruct.create("div", {innerHTML:"&nbsp;", className:"ganttResourcePanelNamesOwners"});
        domStyle.set(panelNameOwner, {height:(this.contentHeight - this.ganttChart.panelTimeHeight - this.ganttChart.scrollBarWidth) + "px", width:this.ganttChart.maxWidthPanelNames + "px"});
