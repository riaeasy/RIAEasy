...
define("dojox/gantt/GanttProjectControl", ["./GanttTaskItem", "./GanttTaskControl", "dijit/focus", "dojo/_base/declare", "dojo/_base/array", "dojo/_base/lang", "dojo/date/locale", "dojo/request", "dojo/on", "dojo/dom", "dojo/dom-class", "dojo/dom-construct", "dojo/dom-style", "dojo/dom-attr", "dojo/dom-geometry", "dojo/keys", "dojo/domReady!"], function (GanttTaskItem, GanttTaskControl, focus, declare, arrayUtil, lang, locale, request, on, dom, domClass, domConstruct, domStyle, domAttr, domGeometry, keys) {
    return declare("dojox.gantt.GanttProjectControl", [], {constructor:function (ganttChart, projectItem) {
        ...
        domStyle.set(projectItem, {"left":this.posX + "px", "width":this.duration * this.ganttChart.pixelsPerWorkHour + "px"});
        ...
        if (this.percentage != -1) {
            if (this.percentage != 0) {
                ...
                domStyle.set(imageProgress, {width:(!this.duration ? 1 : (this.percentage * this.duration * this.ganttChart.pixelsPerWorkHour / 100)) + "px", height:this.ganttChart.heightTaskItem + "px"});
            }
            if (this.percentage != 100) {
                var cellprojectItem = rowprojectItem.lastChild;
                cellprojectItem.width = (100 - this.percentage) + "%";
                var imageProgress = cellprojectItem.firstChild;
                domStyle.set(imageProgress, {width:(!this.duration ? 1 : ((100 - this.percentage) * this.duration * this.ganttChart.pixelsPerWorkHour / 100)) + "px", height:this.ganttChart.heightTaskItem + "px"});
            }
        } else {
            var cellprojectItem = rowprojectItem.firstChild;
            cellprojectItem.width = "1px";
            var imageProgress = cellprojectItem.firstChild;
            domStyle.set(imageProgress, {width:"1px", height:this.ganttChart.heightTaskItem + "px"});
        }
        var divTaskInfo = projectItem.lastChild;
        var tblTaskInfo = divTaskInfo.firstChild;
        domStyle.set(tblTaskInfo, {height:this.ganttChart.heightTaskItem + "px", width:(!this.duration ? 1 : (this.duration * this.ganttChart.pixelsPerWorkHour)) + "px"});
        ...
        var percentage = this.percentage, pItem = this.projectItem[0];
        ...
        var posX = (this.posX + this.duration * this.ganttChart.pixelsPerWorkHour + 10);
        var divDesc = domConstruct.create("div", {innerHTML:this.getDescStr(), className:"ganttDescProject"});
        ...
        var projectItem = domConstruct.create("div", {id:this.project.id, className:"ganttProjectItem"});
        domStyle.set(projectItem, {left:this.posX + "px", top:this.posY + "px", width:this.duration * this.ganttChart.pixelsPerWorkHour + "px"});
        var tblProjectItem = domConstruct.create("table", {cellPadding:"0", cellSpacing:"0", className:"ganttTblProjectItem"}, projectItem);
        var width = this.duration * this.ganttChart.pixelsPerWorkHour;
        tblProjectItem.width = ((width == 0) ? 1 : width) + "px";
        tblProjectItem.style.width = ((width == 0) ? 1 : width) + "px";
        ...
        if (this.percentage != -1) {
            if (this.percentage != 0) {
                var cellprojectItem = domConstruct.create("td", {width:this.percentage + "%"}, rowprojectItem);
                cellprojectItem.style.lineHeight = "1px";
                var imageProgress = domConstruct.create("div", {className:"ganttImageProgressFilled"}, cellprojectItem);
                domStyle.set(imageProgress, {width:(this.percentage * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
            }
            if (this.percentage != 100) {
                var cellprojectItem = domConstruct.create("td", {width:(100 - this.percentage) + "%"}, rowprojectItem);
                cellprojectItem.style.lineHeight = "1px";
                var imageProgress = domConstruct.create("div", {className:"ganttImageProgressBg"}, cellprojectItem);
                domStyle.set(imageProgress, {width:((100 - this.percentage) * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
            }
        } else {
            var cellprojectItem = domConstruct.create("td", {width:"1px"}, rowprojectItem);
            cellprojectItem.style.lineHeight = "1px";
            var imageProgress = domConstruct.create("div", {className:"ganttImageProgressBg"}, cellprojectItem);
            domStyle.set(imageProgress, {width:"1px", height:this.ganttChart.heightTaskItem + "px"});
        }
        var divTaskInfo = domConstruct.create("div", {className:"ganttDivTaskInfo"});
        var tblTaskInfo = domConstruct.create("table", {cellPadding:"0", cellSpacing:"0", height:this.ganttChart.heightTaskItem + "px", width:((this.duration * this.ganttChart.pixelsPerWorkHour == 0) ? 1 : this.duration * this.ganttChart.pixelsPerWorkHour) + "px"}, divTaskInfo);
        var rowTaskInfo = tblTaskInfo.insertRow(0);
        domConstruct.create("td", {align:"center", vAlign:"top", height:this.ganttChart.heightTaskItem + "px", className:"ganttMoveInfo"}, rowTaskInfo);
        ...
        var divName = domConstruct.create("div", {className:"ganttProjectNameItem", innerHTML:this.project.name, title:this.project.name});
        domStyle.set(divName, {left:"5px", top:this.posY + "px"});
        ...
        if (this.ganttChart.isShowConMenu) {
            ...
            this.ganttChart._events.push(on(this.ganttChart.tabMenu.menuPanel, "keydown", lang.hitch(this, function () {
                if (this.ganttChart.tabMenu.isShow && event.keyCode == keys.ESCAPE) {
                    this.ganttChart.tabMenu.hide();
                }
            })));
        ...
        var prow = this.projectItem[0].firstChild.rows[0], rc0 = prow.cells[0], rc1 = prow.cells[1];
            ...
            if (((percentage == 0) || (percentage == 100)) && (this.percentage > 0) && (this.percentage < 100)) {
                ...
                if (((percentage == 0) || (percentage == 100)) && ((this.percentage == 0) || (this.percentage == 100))) {
                    if ((percentage == 0) && (this.percentage == 100)) {
                        ...
                        if ((percentage == 100) && (this.percentage == 0)) {
                    ...
                    if (((percentage > 0) || (percentage < 100)) && ((this.percentage == 0) || (this.percentage == 100))) {
                        rc0.parentNode.removeChild(rc0);
                        var cellprojectItem = domConstruct.create("td", {width:percentage + "%"}, prow);
                        cellprojectItem.style.lineHeight = "1px";
                        var imageProgress = domConstruct.create("div", {className:"ganttImageProgressFilled"}, cellprojectItem);
                        domStyle.set(imageProgress, {width:(percentage * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
                        cellprojectItem = domConstruct.create("td", {width:(100 - percentage) + "%"}, prow);
                        cellprojectItem.style.lineHeight = "1px";
                        imageProgress = domConstruct.create("div", {className:"ganttImageProgressBg"}, cellprojectItem);
                        domStyle.set(imageProgress, {width:((100 - percentage) * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
                    } else {
                        if (this.percentage == -1) {
                            if (percentage == 100) {
                                domClass.remove(rc0.firstChild, "ganttImageProgressBg");
                                ...
                                if (percentage < 100 && percentage > 0) {
                                    ...
                                    var cellprojectItem = domConstruct.create("td", {width:percentage + "%"}, prow);
                                    cellprojectItem.style.lineHeight = "1px";
                                    imageProgress = domConstruct.create("div", {className:"ganttImageProgressFilled"}, cellprojectItem);
                                    domStyle.set(imageProgress, {width:(percentage * this.duration * this.ganttChart.pixelsPerWorkHour) / 100 + "px", height:this.ganttChart.heightTaskItem + "px"});
                                    cellprojectItem = domConstruct.create("td", {width:(100 - percentage) + "%"}, prow);
                                    cellprojectItem.style.lineHeight = "1px";
                                    imageProgress = domConstruct.create("div", {className:"ganttImageProgressBg"}, cellprojectItem);
        ...
        if (task) {
            var tItem0 = task.cTaskItem[0], tNameItem0 = task.cTaskNameItem[0], tItem1 = task.cTaskItem[1], tNameItem1 = task.cTaskNameItem[1], tNameItem2 = task.cTaskNameItem[2];
        ...
        if ((parentTaskId) && (parentTaskId != "")) {
            ...
            task = new GanttTaskItem({id:id, name:name, startTime:startTime, duration:duration, percentage:percentage, previousTaskId:previousTaskId, taskOwner:taskOwner});
            ...
            if (_parentTask.cTaskItem[0].style.display == "none") {
                ...
                if (_parentTask.cTaskNameItem[2]) {
            ...
            _task.create();
            if (task.nextChildTask) {
                _task.nextChildTask = _task.project.getTaskById(task.nextChildTask.id);
            }
            ...
            _task.create();
            if (task.nextParentTask) {
                _task.nextParentTask = _task.project.getTaskById(task.nextParentTask.id);
            }
            _task.adjustPanelTime();
        ...
        var tNameItem0 = task.cTaskNameItem[0], tNameItem1 = task.cTaskNameItem[1], tNameItem2 = task.cTaskNameItem[2], tItem1 = task.cTaskItem[1];
