...
define("dojox/gfx/vml", ["dojo/_base/lang", "dojo/_base/declare", "dojo/_base/array", "dojo/_base/Color", "dojo/_base/sniff", "dojo/_base/config", "dojo/dom", "dojo/dom-geometry", "dojo/_base/kernel", "./_base", "./shape", "./path", "./arc", "./gradient", "./matrix"], function (lang, declare, arr, Color, has, config, dom, domGeom, kernel, g, gs, pathLib, arcLib, gradient, m) {
    var vml = g.vml = {};
    ...
    vml.Shape = declare("dojox.gfx.vml.Shape", gs.Shape, {setFill:function (fill) {
        if (!fill) {
            this.fillStyle = null;
        ...
        if (!clip) {
            ...
            if ("width" in clip) {
                var matrix = this._getRealMatrix(), l = parseFloat(nodeStyle.left), t = parseFloat(nodeStyle.top);
                if (isNaN(l)) {
                    l = 0;
                }
                if (isNaN(t)) {
                ...
                nodeStyle.clip = "rect(" + Math.round(clipt.y - pt.y) + "px " + Math.round(clipt.x - pt.x + clipt.width) + "px " + Math.round(clipt.y - pt.y + clipt.height) + "px " + Math.round(clipt.x - pt.x) + "px)";
            }
        }
        return this;
    }});
    vml.Group = declare("dojox.gfx.vml.Group", vml.Shape, {constructor:function () {
        ...
        if (!clip) {
            ...
            if ("width" in clip) {
                ...
                var offx = bbox && bbox.x < 0 ? bbox.x : 0, offy = bbox && bbox.y < 0 ? bbox.y : 0;
                nodeStyle.position = "absolute";
                nodeStyle.clip = "rect(" + Math.round(clipt.y - offy) + "px " + Math.round(clipt.x + clipt.width - offx) + "px " + Math.round(clipt.y + clipt.height - offy) + "px " + Math.round(clipt.x - offx) + "px)";
            }
        }
        return this;
    }, destroy:function () {
    ...
    vml.Rect = declare("dojox.gfx.vml.Rect", [vml.Shape, gs.Rect], {setShape:function (newShape) {
    ...
    vml.Ellipse = declare("dojox.gfx.vml.Ellipse", [vml.Shape, gs.Ellipse], {setShape:function (newShape) {
        var shape = this.shape = g.makeParameters(this.shape, newShape);
        this.bbox = null;
        var style = this.rawNode.style;
    ...
    vml.Circle = declare("dojox.gfx.vml.Circle", [vml.Shape, gs.Circle], {setShape:function (newShape) {
        var shape = this.shape = g.makeParameters(this.shape, newShape);
        this.bbox = null;
        var style = this.rawNode.style;
    ...
    vml.Line = declare("dojox.gfx.vml.Line", [vml.Shape, gs.Line], {constructor:function (rawNode) {
        if (rawNode) {
    ...
    vml.Polyline = declare("dojox.gfx.vml.Polyline", [vml.Shape, gs.Polyline], {constructor:function (rawNode) {
        if (rawNode) {
    ...
    vml.Image = declare("dojox.gfx.vml.Image", [vml.Shape, gs.Image], {setShape:function (newShape) {
        var shape = this.shape = g.makeParameters(this.shape, newShape);
        this.bbox = null;
    ...
    vml.Text = declare("dojox.gfx.vml.Text", [vml.Shape, gs.Text], {constructor:function (rawNode) {
        if (rawNode) {
    ...
    vml.Path = declare("dojox.gfx.vml.Path", [vml.Shape, pathLib.Path], {constructor:function (rawNode) {
        ...
        for (var i = 0; i < l; i += 7) {
            ...
            var result = arcLib.arcAsBezier(last, n[i], n[i + 1], n[i + 2], n[i + 3] ? 1 : 0, n[i + 4] ? 1 : 0, x1, y1);
    ...
    vml.TextPath = declare("dojox.gfx.vml.TextPath", [vml.Path, pathLib.TextPath], {constructor:function (rawNode) {
        if (rawNode) {
    ...
    vml.Surface = declare("dojox.gfx.vml.Surface", gs.Surface, {constructor:function () {
        gs.Container._init.call(this);
    }, destroy:function () {
        this.clear(true);
        this.inherited(arguments);
    ...
    vml.createSurface = function (parentNode, width, height) {
        ...
        var s = new vml.Surface(), p = dom.byId(parentNode), c = s.clipNode = p.ownerDocument.createElement("div"), r = s.rawNode = p.ownerDocument.createElement("v:group"), cs = c.style, rs = r.style;
        if (has("ie") > 7) {
    ...
    var C = gs.Container, Container = {add:config.fixVmlAdd === true ? addPatch9624 : add15, remove:function (shape, silently) {
        if (this == shape.getParent()) {
