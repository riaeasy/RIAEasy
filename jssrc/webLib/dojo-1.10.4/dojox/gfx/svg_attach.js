...
define("dojox/gfx/svg_attach", ["dojo/_base/kernel", "dojo/_base/lang", "dojo/_base/array", "dojo/_base/Color", "./_base", "./svg", "./matrix"], function (kernel, lang, arr, Color, g, svg, Matrix) {
    ...
    function _getGradient(defaultGradient, gradient) {
        ...
        for (var i = 0; i < gradient.childNodes.length; ++i) {
            fillStyle.colors.push({offset:gradient.childNodes[i].getAttribute("offset"), color:new Color(gradient.childNodes[i].getAttribute("stop-color"))});
        }
        return fillStyle;
    }
    ...
    function attachTransform(object) {
        ...
        if (matrix.match(/^matrix\(.+\)$/)) {
            ...
            object.matrix = Matrix.normalize({xx:parseFloat(t[0]), xy:parseFloat(t[2]), yx:parseFloat(t[1]), yy:parseFloat(t[3]), dx:parseFloat(t[4]), dy:parseFloat(t[5])});
        } else {
            object.matrix = null;
        }
    }
    function attachFont(object) {
        var fontStyle = object.fontStyle = lang.clone(g.defaultFont), r = object.rawNode;
    ...
    function attachText(object) {
        var shape = object.shape = lang.clone(g.defaultText), r = object.rawNode;
    ...
    function attachTextPath(object) {
        var shape = object.shape = lang.clone(g.defaultTextPath), r = object.rawNode;
        shape.align = r.getAttribute("text-anchor");
        shape.decoration = r.getAttribute("text-decoration");
        shape.rotated = parseFloat(r.getAttribute("rotate")) != 0;
        shape.kerning = r.getAttribute("kerning") == "auto";
