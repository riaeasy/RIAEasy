...
define("dojox/gfx/vml_attach", ["dojo/_base/kernel", "dojo/_base/lang", "./_base", "./matrix", "./path", "dojo/_base/Color", "./vml"], function (kernel, lang, g, m, pathLib, Color, vml) {
    ...
    var attachFill = function (object) {
        ...
        if (fo.on && fo.type == "gradient") {
            fillStyle = lang.clone(g.defaultLinearGradient), rad = m._degToRad(fo.angle);
            ...
            if (fo.on && fo.type == "gradientradial") {
                fillStyle = lang.clone(g.defaultRadialGradient), w = parseFloat(r.style.width), h = parseFloat(r.style.height);
                ...
                if (fo.on && fo.type == "tile") {
                    ...
                    fillStyle.width = g.pt2px(fo.size.x);
                    fillStyle.height = g.pt2px(fo.size.y);
    ...
    var attachStroke = function (object) {
        ...
        var strokeStyle = object.strokeStyle = lang.clone(g.defaultStroke), rs = r.stroke;
    ...
    var attachTransform = function (object) {
        ...
        object.matrix = m.normalize({xx:sm.xtox, xy:sm.ytox, yx:sm.xtoy, yy:sm.ytoy, dx:g.pt2px(so.x), dy:g.pt2px(so.y)});
    };
    var attachGroup = function (object) {
        object.bgNode = object.rawNode.firstChild;
    };
    var attachRect = function (object) {
        var r = object.rawNode, arcsize = r.outerHTML.match(/arcsize = \"(\d*\.?\d+[%f]?)\"/)[1], style = r.style, width = parseFloat(style.width), height = parseFloat(style.height);
        arcsize = (arcsize.indexOf("%") >= 0) ? parseFloat(arcsize) / 100 : vml._parseFloat(arcsize);
        object.shape = g.makeParameters(g.defaultRect, {x:parseInt(style.left), y:parseInt(style.top), width:width, height:height, r:Math.min(width, height) * arcsize});
    };
    var attachEllipse = function (object) {
        var style = object.rawNode.style, rx = parseInt(style.width) / 2, ry = parseInt(style.height) / 2;
        object.shape = g.makeParameters(g.defaultEllipse, {cx:parseInt(style.left) + rx, cy:parseInt(style.top) + ry, rx:rx, ry:ry});
    };
    var attachCircle = function (object) {
        var style = object.rawNode.style, r = parseInt(style.width) / 2;
        object.shape = g.makeParameters(g.defaultCircle, {cx:parseInt(style.left) + r, cy:parseInt(style.top) + r, r:r});
    };
    var attachLine = function (object) {
        var shape = object.shape = lang.clone(g.defaultLine), p = object.rawNode.path.v.match(g.pathVmlRegExp);
        do {
    ...
    var attachPolyline = function (object) {
        var shape = object.shape = lang.clone(g.defaultPolyline), p = object.rawNode.path.v.match(g.pathVmlRegExp);
        do {
            if (p.length < 3 || p[0] != "m") {
                break;
            }
    ...
    var attachImageTransform = function (object) {
        ...
        object.matrix = m.normalize({xx:mm.M11, xy:mm.M12, yx:mm.M21, yy:mm.M22, dx:mm.Dx, dy:mm.Dy});
    };
    var attachText = function (object) {
        var shape = object.shape = lang.clone(g.defaultText), r = object.rawNode, p = r.path.v.match(g.pathVmlRegExp);
        do {
            if (!p || p.length != 7) {
                break;
            }
    ...
    var attachFont = function (object) {
        var fontStyle = object.fontStyle = lang.clone(g.defaultFont), c = object.rawNode.childNodes, i = 0;
        for (; i < c.length && c[i].tagName == "textpath"; ++i) {
        }
        if (i >= c.length) {
    ...
    var attachPath = function (object) {
        var shape = object.shape = lang.clone(g.defaultPath), p = object.rawNode.path.v.match(g.pathVmlRegExp), t = [], skip = false, map = pathLib._pathVmlToSvgMap;
