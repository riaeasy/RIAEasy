...
define("dojox/gfx/decompose", ["./_base", "dojo/_base/lang", "./matrix"], function (g, lang, m) {
    function eq(a, b) {
        return Math.abs(a - b) <= 0.000001 * (Math.abs(a) + Math.abs(b));
    }
    function calcFromValues(r1, m1, r2, m2) {
        if (!isFinite(r1)) {
            return r2;
        } else {
            if (!isFinite(r2)) {
    ...
    return g.decompose = function (matrix) {
        ...
        if (eq(M.xy, 0) && eq(M.yx, 0)) {
            return lang.mixin(result, {sx:M.xx, sy:M.yy});
        }
        ...
        S.xx *= result.sx;
        S.yy *= result.sy;
        return lang.mixin(result, {sx:S.xx, sy:S.yy});
    };
});
