...
define("dojox/highlight/_base", ["dojo/_base/lang", "dojo/_base/array", "dojo/dom", "dojo/dom-class"], function (lang, array, dom, domClass) {
    var dh = lang.getObject("dojox.highlight", true), C_NUMBER_RE = "\\b(0x[A-Za-z0-9]+|\\d+(\\.\\d+)?)";
    dh.languages = dh.languages || {};
    dh.constants = {IDENT_RE:"[a-zA-Z][a-zA-Z0-9_]*", UNDERSCORE_IDENT_RE:"[a-zA-Z_][a-zA-Z0-9_]*", NUMBER_RE:"\\b\\d+(\\.\\d+)?", C_NUMBER_RE:C_NUMBER_RE, APOS_STRING_MODE:{className:"string", begin:"'", end:"'", illegal:"\\n", contains:["escape"], relevance:0}, QUOTE_STRING_MODE:{className:"string", begin:"\"", end:"\"", illegal:"\\n", contains:["escape"], relevance:0}, BACKSLASH_ESCAPE:{className:"escape", begin:"\\\\.", end:"^", relevance:0}, C_LINE_COMMENT_MODE:{className:"comment", begin:"//", end:"$", relevance:0}, C_BLOCK_COMMENT_MODE:{className:"comment", begin:"/\\*", end:"\\*/"}, HASH_COMMENT_MODE:{className:"comment", begin:"#", end:"$"}, C_NUMBER_MODE:{className:"number", begin:C_NUMBER_RE, end:"^", relevance:0}};
    function esc(value) {
    ...
    lang.extend(Highlighter, {buildRes:function () {
        ...
        var mode = this.modes[this.modes.length - 1], terminators = {};
        if (mode.contains) {
        ...
        return {buffer:match.index ? value.substr(0, match.index) : "", lexeme:match[0], end:false};
        ...
        for (var className in mode.keywordGroups) {
            if (matchStr in mode.keywordGroups[className]) {
                return className;
            }
        }
        return "";
        ...
        var result = [], lastIndex = 0, match = mode.lexemsRe.exec(buffer);
        while (match) {
    ...
    function replaceText(node, className, text) {
        if (String(node.tagName).toLowerCase() == "code" && String(node.parentNode.tagName).toLowerCase() == "pre") {
            var container = document.createElement("div"), environment = node.parentNode.parentNode;
    ...
    function highlightStringAuto(str) {
        var result = "", langName = "", bestRelevance = 2, textBlock = str;
        for (var key in dh.languages) {
            if (!dh.languages[key].defaultMode) {
                continue;
            }
            var highlight = new Highlighter(key, textBlock), relevance = highlight.keywordCount + highlight.relevance, relevanceMax = 0;
    ...
    dojox.highlight.init = function (node) {
        node = dom.byId(node);
        if (domClass.contains(node, "no-highlight")) {
            return;
        }
        if (!verifyText(node)) {
            return;
        }
        var classes = node.className.split(/\s+/), flag = array.some(classes, function (className) {
