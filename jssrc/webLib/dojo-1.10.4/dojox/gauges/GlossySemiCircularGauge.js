...
define("dojox/gauges/GlossySemiCircularGauge", ["dojo/_base/declare", "dojo/_base/Color", "./GlossyCircularGaugeBase"], function (declare, Color, GlossyCircularGaugeBase) {
    return declare("dojox.gauges.GlossySemiCircularGauge", [GlossyCircularGaugeBase], {_designWidth:381.25, _designHeight:221.25, _designCx:190.6675, _designCy:185.87665, _designTextIndicatorX:190.6675, _designTextIndicatorY:145.87665, constructor:function () {
        this.min = 0;
        this.max = 100;
        ...
        var transform = {xx:scale, xy:0, yx:0, yy:scale, dx:-23.33928 * scale + (this.width - scale * this._designWidth) / 2, dy:-483.30859 * scale + (this.height - scale * this._designHeight) / 2};
        ...
        var scale = Math.min((this.width / this._designWidth), (this.height / this._designHeight));
        var transform = {xx:scale, xy:0, yx:0, yy:scale, dx:(-160) * scale + (this.width - scale * this._designWidth) / 2, dy:(-264.5) * scale + (this.height - scale * this._designHeight) / 2};
        var lighterColor1 = Color.blendColors(new Color(this.color), new Color("white"), 0.4);
        var lighterColor2 = Color.blendColors(new Color(this.color), new Color("white"), 0.8);
        if (this._foreground) {
        ...
        this._foreground.setTransform(transform);
        var group1 = this._foreground.createGroup().setTransform({xx:1.25, xy:0, yx:0, yy:-1.25, dx:-43.30358, dy:1015.57642});
        group1.createPath({path:"M0 0 C0.004 -12.579 -10.189 -22.779 -22.768 -22.784 C-35.349 -22.788 -45.549 -12.594 -45.553 -0.016 L-45.553 0 C-45.558 12.579 -35.363 22.779 -22.783 22.784 C-10.205 22.788 -0.004 12.594 0 0.015 L0 0 Z"}).setTransform({xx:1, xy:0, yx:0, yy:1, dx:336.31049, dy:451.43359}).setFill(this.color);
        group1.createPath({path:"M333.443 451.434 C333.446 440.438 324.537 431.523 313.541 431.519 C302.546 431.515 293.63 440.425 293.626 451.42 L293.626 451.434 C293.622 462.429 302.532 471.345 313.527 471.349 C324.523 471.353 333.439 462.442 333.443 451.447 L333.443 451.434 Z"}).setFill({type:"linear", x1:293.62201, y1:431.51501, x2:293.62201, y2:451.43401, colors:[{offset:0, color:lighterColor1}, {offset:1, color:this.color}]});
        group1.createPath({path:"M333.438 451.858 C333.215 462.663 324.386 471.353 313.528 471.349 C302.675 471.345 293.854 462.658 293.632 451.858 C299.709 450.222 306.702 453.486 313.799 453.486 C320.895 453.486 327.377 450.221 333.438 451.858"}).setFill({type:"linear", x1:293.63199, y1:450.22101, x2:293.63199, y2:471.353, colors:[{offset:0, color:this.color}, {offset:1, color:lighterColor2}]});
        group1.createPath({path:"M0 0 C-0.223 10.805 -9.052 19.494 -19.909 19.49 C-30.763 19.486 -39.583 10.799 -39.806 0 C-33.729 -1.636 -26.735 1.628 -19.639 1.628 C-12.543 1.628 -6.061 -1.638 0 0"}).setTransform({xx:1, xy:0, yx:0, yy:1, dx:333.4375, dy:451.8584}).setFill([255, 255, 255, 0.12157]);
    }});
});
