...
define("dojox/treemap/_utils", ["dojo/_base/array"], function (arr) {
    var utils = {group:function (items, groupingFunctions, measureFunction) {
        ...
        arr.forEach(items, function (entry) {
            ...
            arr.forEach(groupingFunctions, function (groupingFunction, j) {
                ...
                if (!child) {
                    r.children.push(child = {__treeName:data, __treeID:data + Math.random(), children:[]});
                }
        ...
        if (dataTotal == 0) {
            if (elements.length == 0) {
                return {items:items, rects:[], total:0};
            }
        ...
        rects = arr.map(elements, function (element) {
            return {x:rtl ? width - element.x - element.width : element.x, y:element.y, w:element.width, h:element.height};
        });
        ...
        var elements = arr.map(items, function (item, index) {
            ...
            total += size;
            return {index:index, size:size, sizeTmp:size};
        });
        ...
        aspect = Math.max(elements[end].heightTmp / elements[end].widthTmp, elements[end].widthTmp / elements[end].heightTmp);
        if (aspect == undefined) {
            return 1;
        }
        return aspect;
