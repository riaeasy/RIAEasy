		...
		_downloadFile: function(url, fn, cb, dir){
			...
			function _download1(file) {
				...
				fileTransfer.download(
					...
					function(error){
						console.error(error);
						rias.error("下载'" + url + "'失败...", null, self);
					}
				);
			}
			function _download(file){
				...
				fileTransfer.download(
					...
					fileURL,
					function (entry) {
						console.info("下载完成: " + url, null, self);
						if(cb){
							cb(entry);
						}
					},
		...
		_installFile: function(entry){
			window.plugins.webintent.startActivity({
				...
				function () {
					...
					rias.error("安装成功.", null, self);
				},
				function (error) {
					console.error("Failed to open URL via Android Intent.\n" + error);
					rias.error("安装'" + entry.fullPath + "'失败...", null, self);
				}
			);
		},
			...
			if(!url || !rias.isString(url)){
				...
				shell._downloadFile(url, "cbxjmy.apk", function(entry){
					if(entry){
						...
						rias.error("安装失败，缺少安装文件...", null, self);
					}
				});
			}
		},
