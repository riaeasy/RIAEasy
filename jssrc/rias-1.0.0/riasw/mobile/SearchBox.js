...
define([
...
], function(rias, TextBox, _SearchMixin){
	...
	var Widget = rias.declare(riaswType, [TextBox, _SearchMixin], {
		// summary:
		...
		_onInput: function(e){
			// tags:
			//		private
			if(e.charOrCode == rias.keys.ENTER){
				e.charOrCode = 229;
		...
		postCreate: function(){
			...
			if(rias.has("ios") < 5){
				rias.dom.addClass(this.domNode, 'iphone4'); // cannot click cancel button after focus so just remove it
					...
					function(){
						if(this.textbox.value !== ''){
							this.defer(
								function(){
									if(this.textbox.value === ''){
										this._onInput({ charOrCode: rias.keys.ENTER }); // emulate onsearch
									}
								}
							);
						}
			...
			this.connect(this.textbox, "onsearch",
				function(){
					this._onInput({ charOrCode: rias.keys.ENTER });
				}
			);
			...
			var handleRelease;
			if(rias.has("ios")){
				this.on(rias.touch.press, function(evt){
					var rect;
					...
					handleRelease = rias.on(rias.dom.doc, rias.touch.release,
						function(evt){
							var rect, dx, dy;
							if(_this.get("value") != ""){
								...
								if(Math.abs(dx) <= 4 && Math.abs(dy) <= 4){
									evt.preventDefault();
									_this.set("value", "");
									_this._onInput({ charOrCode: rias.keys.ENTER });
								}
							}
