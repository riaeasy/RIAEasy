...
define([
	...
	var riaswType = "rias.riasw.mobile.SwapView";
	var Widget = rias.declare(riaswType, [_Widget, _ScrollableMixin], {
		...
		isSwapView: function(/*DomNode*/node){
			...
			return (node && node.nodeType === 1 && rias.dom.containsClass(node, "mblSwapView"));
		},
		...
		findDisp: function(/*DomNode*/node){
			...
			if(!rias.dom.containsClass(node, "mblSwapView")){
			...
			for(var i = 0; i < nodes.length; i++){
				var n = nodes[i];
				if(n.nodeType === 1 && rias.dom.containsClass(n, "mblSwapView")
					&& !rias.dom.containsClass(n, "mblIn") && n.style.display !== "none"){
					return n;
				}
			}
			return node;
		...
		onAnimationEnd: function(/*Event*/e){
			...
			if(e && e.target && rias.dom.containsClass(e.target, "mblScrollableScrollTo2")){ return; }
			this.inherited(arguments);
		},
