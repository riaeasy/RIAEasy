...
define([
	...
	rias, ContentPanel, _WidgetBase, _Contained, _Container,
	ViewController, common, transitDeferred, viewRegistry, css3
	){
	...
	var riaswType = "rias.riasw.mobile.View";
	//var Widget = rias.declare(riaswType, [_WidgetBase, _Container, _Contained], {
	var Widget = rias.declare(riaswType, [ContentPanel], {

		//loadMetaOnStartup: false,

		isLayoutContainer: true,
		animate: false,
		initDisplayState: rias.riasw.layout.displayHidden,
		...
		startup: function(){
			...
			//if(this._visible === undefined){
				...
				(fragView || selectedView || firstView).initDisplayState = rias.riasw.layout.displayShowNormal;
			//}
			/*if(this._visible){
			...
			if(this.initDisplayState === rias.riasw.layout.displayShowNormal){
				this.onStartView();
				rias.publish("/dojox/mobile/startView", [this]);
			}
			...
			///扩展===================================///
			this.subscribe("/dojox/mobile/afterTransitionIn", "_afterTransitionIn");
		},
		...
		_afterTransitionIn: function(args){
			///args = [view, moveTo, dir, transition, context, method]
			if(args[0] == this){
				//this.resize();
				//this._parentResize();///在 _onAnimationEnd 中处理
			}
		},
		...
		buildRendering: function(){
			...
			//this._animEndHandle = this.connect(this.domNode, css3.name("animationEnd"), "_onAnimationEnd");
			//this._animStartHandle = this.connect(this.domNode, css3.name("animationStart"), "_onAnimationStart");
			//if(!rias.config.mblCSS3Transition){
			//	this._transEndHandle = this.connect(this.domNode, css3.name("transitionEnd"), "_onAnimationEnd");
			//}
		...
		performTransition: function(/*String*/moveTo, /*Number*/transitionDir, /*String*/transition,
			...
			var toWidget = rias.by(toNode);
			if(toWidget){
				if(rias.isInstanceOf(toWidget, Widget) && toWidget.displayState !== rias.riasw.layout.displayShowNormal){
					toWidget.show();
				}
		...
		getShowingView: function(){
			...
			for(var i = 0; i < nodes.length; i++){
				var n = nodes[i];
				n = rias.by(n);
				if(n && rias.isInstanceOf(n, Widget) && n.isShown(false)){
					return n;
				}
				//if(n.nodeType === 1 && n.style.display !== "none" && rias.dom.containsClass(n, "mblView")){
				//	returnrias.by(n);
				//}
			}
			return null;
		...
		getSiblingViews: function(){
			...
			return rias.map(rias.filter(this.domNode.parentNode.childNodes, function(n){
				return rias.isInstanceOf(rias.by(n), Widget);// n.nodeType === 1 && rias.isInstanceOf(n, Widget);// rias.dom.containsClass(n, "mblView");
			}), function(n){
				return rias.by(n);
			});
		},
