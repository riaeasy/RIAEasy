...
define([
	...
	var riasType = "rias.riasw.mobile.TreeView";
	var Widget = rias.declare(riasType, [ScrollableView], {
		...
		handleClick: function(li){
			...
			if(rias.registry.byId(newViewId)){  // view already exists, just transition to it
				rias.registry.byNode(li.domNode).transitionTo(newViewId);
				return;
			}
			...
			this.model.getChildren(li.item,
				rias.hitch(this, function(items){
					...
					newView.startup();
					prog.stop();
					rias.registry.byNode(li.domNode).transitionTo(newView.id);
				})
			)
		}
