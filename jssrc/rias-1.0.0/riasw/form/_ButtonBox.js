define([
	...
	return rias.declare("rias.riasw.form._ButtonBox", null, {
		...
		openDropDown: function(){
			...
			args._riasrInitialModuleResult = self.get("value");
			if(!args._riasrHookAfterSubmit){
				args._riasrHookAfterSubmit = function(){
					self.set('value', (rias.isFunction(dd.getRiasrModuleResult) ? dd.getRiasrModuleResult() :
						("_riasrModuleResult" in dd) ? dd._riasrModuleResult : dd._riasrInitialModuleResult), false);
				};
			}
			...
			if(dd.isShown()){
				...
				dd.own(rias.after(dd, "onShow", function(){
					_size();
				}));
			}
