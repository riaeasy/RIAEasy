...
define([
	...
	"dijit/form/_TextBoxMixin"
			...
			parser, query, NodeListDom, NodeListTraverse, cookie, hash, ready, unload, _Url,
			Mover,
			typematic, //manager, _basefocus, WidgetSet,
			a11y, a11yclick, registry,
			selection, focus, place, Viewport, popup, BackgroundIframe,
			_WidgetBase, _Widget, _CssStateMixin, _TemplatedMixin, _WidgetsInTemplateMixin, _FocusMixin, _Container,
			_TextBoxMixin) {
...
///dom******************************************************************************///
	...
	//rias.global = basewin.global;///移到 riasBase 中
	//rias.setContext = basewin.setContext;///移到 riasBase 中
	//rias.withGlobal = basewin.withGlobal;///移到 riasBase 中
	var _dom = rias.dom = {};
	...
	_dom.contains = function(parent, child){
		parent = rias.orDomNode(parent);
		child = rias.orDomNode(child);
	...
	var marginBox2contentBox = _dom.marginBox2contentBox = function(/*DomNode*/ node, /*Object*/ mb, /*Object*/ computedStyle){
		// summary:
	...
	var isBodyLtr = _dom.isBodyLtr = domGeom.isBodyLtr;
	...
	var getComputedStyle = _dom.getComputedStyle = function(node, always){
		if(always || !node.__riasrComputedStyle){
			node.__riasrComputedStyle = domStyle.getComputedStyle(node);
		}
		return node.__riasrComputedStyle;
	};
	var refreshComputedStyle = _dom.refreshComputedStyle = function(node){
		node.__riasrComputedStyle = domStyle.getComputedStyle(node);
		return node.__riasrComputedStyle;
	};
	...
	_dom.placeTo = function(node, parent){
		...
		if(rias.isString(parent)){
			parent = rias.by(parent, rias.by(node));
		}
	...
	_dom.placeAt = function(node, args){
		...
		var parent,
			around;
		around = rias.isDijit(args.around) ? args.around.domNode : rias.isString(args.around) ? rias.orDomNode(args.around, rias.by(node)) : args.around;
		...
		var ltr = parent ? parent.isLeftToRight() : isBodyLtr(node.ownerDocument),
	...
	popup.open = function(/*__OpenArgs*/ args, parent){
		...
		var self = this,
			...
			ltr = args.parent && args.parent.isLeftToRight ? args.parent.isLeftToRight() : isBodyLtr(widget.ownerDocument),
			around = args.around,
		...
		if(args.onCancel){
			if(widget.afterCancel){
				handlers.push(widget.on("afterCancel", args.onCancel));
		...
		if(widget.afterSubmit){
			handlers.push(widget.on("afterSubmit", _execute));
		}else{
			handlers.push(widget.on(widget.onExecute ? "execute" : "change", _execute));
		}
	...
	_dom.moveOffScreen = function(/*Widget*/ widget){
		// summary:
		...
		var wrapper = popup._createWrapper(widget);

		// Besides setting visibility:hidden, move it out of the viewport, see #5776, #10111, #13604
		var ltr = isBodyLtr(widget.ownerDocument),
			style = {
				visibility: "hidden",
				top: "-9999px",
				display: ""
	...
	_dom.visible = function(any, visiblity, opacity){
		///优化速度。
		//any = rias.isDomNode(any) ? any : (any = rias.by(any)) ? any.domNode : undefined;
		if(any && any.domNode){
			any = any.domNode;
		}
		if(any){
			...
			//return rias.a11y._isElementShown(any) && !_dom.containsClass(any, "dijitHidden");
			//return rias.a11y._isElementVisible(any);// && !_dom.containsClass(any, "dijitHidden");
			//ws = refreshComputedStyle(any);
			return ws.display != "none" && !_dom.containsClass(any, "dijitHidden") && (any.getAttribute("type") != "hidden");
		}
		return undefined;
	};
	//_dom.isVisible = function(widget){
	//	widget = rias.isDomNode(widget) ? widget : (widget = rias.by(widget)) ? widget.domNode : undefined;
	//	return rias.a11y._isElementShown(widget) && !_dom.containsClass(widget, "dijitHidden");
	//};
	_dom.isVisible = function(node, checkAncestors){
		///优化速度。
		//node = rias.orDomNode(node);
	...
	/// focus 是 singleton，不能用 extend，只能用 safeMixin
	rias.safeMixin(focus, {
		_onBlurNode: function(/*DomNode*/ node){
			...
			this._clearFocusTimer = setTimeout(rias.hitch(this, function(){
				...
				///还是用 watch("curNode") 的 publish("focusedNode") 好些
				//rias.dom.focusedNodePrev = this.curNode;
				this.set("curNode", null);
				///还是用 watch("curNode") 的 publish("focusedNode") 好些
				//rias.dom.focusedNode = null;
			}), 0);
		...
		_setStack: function (newStack, by, newFocused) {
			...
			//console.debug("_setStack - ", newStack);
			if (lastNewIdx < 0 || newStack[lastNewIdx] == oldStack[lastOldIdx] && lastOldIdx === lastNewIdx) {
				/// 如果 length 不同也需要重置。
				/// 没有 newStack 时，不处理。
				return;
			}
			this.set("activeStack", newStack);
			_dom.focusedStack = newStack;
			var widget, i,
			...
			for (i = lastOldIdx; i >= 0; i--) {
				...
				if (widget && rias.isFunction(widget._setLastFocusedNode)) {
					widget._setLastFocusedNode(oldWidget);
				}
				if(widget && newStack.indexOf(oldStack[i]) < 0){
					widget._hasBeenBlurred = true;
					widget.set("focused", false);
					//console.debug("blur:", i, widget.id);
					if (widget._focusManager == this) {
						widget._onBlur(by, newStack, oldStack);
					}
			...
			for (i = 0; i <= lastNewIdx; i++) {
				...
				if(widget && oldStack.indexOf(newStack[i]) < 0){
					...
					//console.debug("focused:", i, widget.id);
					if (widget._focusManager == this) {
						widget._onFocus(by, newStack, oldStack);
					}
			...
			if (newFocused && widget && rias.isFunction(widget._getFocusableNode)) {
				...
				if(focusableNode !== newFocused){
					//console.debug("focusableNode !== newFocused - ", focusableNode);
		...
		focus: function(/*Object|DomNode */ handle, forceVisible){
			if(!handle){
				return;
			}
	...
	//rias.subscribe("focusNode", function(node){/// focusNode 在 dijit/_base/focus 中 publish，废弃，还是直接 watch 好些
	focus.watch("curNode", function(name, oldVal, newVal){
		///subscribe("focusNode")未响应 blur，这样可以保留当前 focusNode
		_dom.focusedNodePrev = _dom.focusedNode;
		_dom.focusedNode = newVal;
		//console.debug(node);
	});
...
///dijit******************************************************************************///
	//a11y._isElementVisible = function(/*Element*/ elem){
		...
		//return (domStyle.get(elem).display != "none") && (domAttr.get(elem, "type") != "hidden");
	//	return elem.style.display != "none" && (elem.getAttribute("type") != "hidden");
	//};
	...
	a11y._getTabWidget = function(widget, prev){
		...
		var arr = [];
		var node = rias.orDomNode(widget),
			w;
	...
	rias._WidgetBase = _WidgetBase.extend({
		...
		create: function(params, srcNodeRef){
			...
			this.srcNodeRef = _dom.byId(srcNodeRef);
		...
		buildRendering:function () {
			...
			if (this.baseClass) {
				domClass.add(this.domNode, this.splitBaseClass());
			}
			if(this.widgetCss){
				domClass.add(this.domNode, this.widgetCss);
			}
		},
		postCreate: function(){
			if(this.domNode){
		...
		placeAt: function(/* String|DomNode|_Widget */ reference, /* String|Int? */ position, noresize){
			...
			var child = this,
				p = !reference.tagName && rias.by(reference);
		...
		_parentResize: function(){
			//console.debug("_parentResize - " + this.id);
			var p = this._riasrParent;
			if(p && p.resize){
				if(!this.__updateSize){
					this.needParentResize = false;
					p.set("needLayout", true);
					rias.debounce(p.id + "resize", function(){
						///debounce 后，有可能 _raisrParent 改变。
						//console.debug(p.id + "_resize debounce callback...");
						if(!p._canResize()){
							return;
						}
						p.resize();
					}, this, 260, function(){
						//console.debug(p.id + "_resize debounce pass...");
					})();
				}else{
		...
		restore: function(forceVisible, child, _deep){
			...
			if(_deep > 19){
				console.debug("restore _deep: " + _deep);
				debugger;
			}
			forceVisible = false;
			if(forceVisible && _deep < 20){
				parent = self.getParent();
				if(parent && parent !== rias.webApp){
					if(rias.isFunction(parent.restore)){
						dr = parent.restore(forceVisible, self, ++_deep);
					}else {
		...
		//	this.inherited(arguments);
		//},
		_setLastFocusedNode: function(widget){
		...
		_getFocusableNode: function(node){
			...
			if(!rias.a11y.isFocusable(node) && this._lastFocusedNode){
				...
				//s += "\n_lastFocusedNode:" + (node ? node.id : "");
			}
			//if(!rias.a11y.isFocusable(node) && this.activeWidget){
			//	node = _node(this._getActiveWidgetNode(this.activeWidget));
			//}
			if(!rias.a11y.isFocusable(node) || !isDescendant(node, this.domNode)){
				...
				//s += "\n_getFocusItems:" + (node ? node.id : "");
			}
			this._focusableNode = node;
			//console.debug("_getFocusableNode - \n" + s);
			return node;
		},
	...
	rias._Widget = _Widget.extend({
		_setContextMenuAttr: function(menu){
			if(this._contextMenu){
				this._contextMenu.removeTarget(this);
			}
			menu = rias.by(menu, this);
			if(!rias.isInstanceOf(menu, "rias.riasw.widget.Menu")){
				return;
			}
			menu.addTarget(this);
			this.set("_contextMenu", menu);
			this._set("contextMenu", menu);
		}
	});
	rias._TemplatedMixin = _TemplatedMixin;
	rias._CssStateMixin = _CssStateMixin.extend({
		_applyAttributes: function(){
		...
		_setStateClass: function(){
			...
			var baseClasses = this.splitBaseClass().concat(this._widgetCss ? [this._widgetCss] : []),
	...
	var _dInit = rias.newDeferred();
	rias.whenLoaded._deferreds.push(_dInit);
	rias.require([
		...
		_Container.extend({
			...
			postCreate: function(){
				...
				//}
				rias.forEach(self.splitBaseClass(), function(c){
			...
			_getActiveWidgetNode: function(){
				var node = this.activeWidget;
			...
			_setActiveWidgetAttr: function(node){
				this._set("activeWidget", rias.by(node) || node);
			},
			_getFocusableNode: function(node){
				function _node(n){
					if(a11y.effectiveTabIndex(n) >= -1){
			...
			_getNeedLayoutAttr: function(){
				var self = this,
					n;
				if(!self._started || self.__updateSize || self.isDestroyed(false) || !self.get("visible")){
					return false;
				}
				return self.needLayout
					|| rias.some(self.getChildren(), function(child){
				...
				/*if(!value){
					rias.some(this.getChildren(), function(child){
						if(child.get("needLayout")){
							console.debug("child needlayout - " + child.id);
							value = true;
							return value;
						}
						return false;
			...
			_doBeforeLayout: function(box){
				if(!box){
					...
					if(!this._contentBox || !_dom.boxEqual(box, this._contentBox, 1)){
			...
			_doResize: function(box){
				...
				//this.layout();
				if(this.get("needLayout") || this._needResizeChild || !this._marginBox0 || !_dom.boxEqual(box, this._marginBox0, 1)){
			...
			_setDomNodeSize: function(box){
				...
				if(this.restrictPadding >= 0){
					noOverflowCall(this.getParentNode(), function(){
			...
			_internalResize: function(changeSize, resultSize){
				...
				//this.domNode.__riasrComputedStyle = getComputedStyle(this.domNode);
				//if(this.domNode != this.containerNode){
				//	this.containerNode.__riasrComputedStyle = getComputedStyle(this.containerNode);
				//}
				...
				this.afterResize();
				//console.debug("resize - " + this.id + " - " + rias.__dt() + " ms.");
			...
			addChild: function(/*dijit/_WidgetBase*/ child, /*int?*/ insertIndex, noresize){
				...
				child._riasrParent = this;
				child.set("_riasrParentNode", this.domNode);
				this._setupChild(child, true, insertIndex, noresize);
			},
			...
			removeChild: function(/*Widget|int*/ child, noresize){
				...
				if(child){
					...
					this._setupChild(child, false, undefined, noresize);
					if(child._splitterWidget){
						rias.destroy(child._splitterWidget);
						child._splitterWidget = undefined;
					}
	...
	_TextBoxMixin._setSelectionRange = dijit._setSelectionRange = _dom._setSelectionRange;
	_dom.selectInputText = _TextBoxMixin.selectInputText;
