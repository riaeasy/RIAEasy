define([
...
], function(rias, script, _Widget, _TemplatedMixin, _WidgetsInTemplateMixin, _Plugin, _SpellCheckParser,
            TextBox, Button, DropDownButton, MultiSelect, DialogPanel, Menu, MenuItem, MenuSeparator){

	//rias.experimental("dojox.editor.plugins.SpellCheck");
	var pluginsName = "rias.riasw.editor.htmleditor.plugins";

	var SpellCheckControl = rias.declare(pluginsName + "._spellCheckControl", [_Widget, _TemplatedMixin, _WidgetsInTemplateMixin], {
		// summary:
			...
			"<table role='presentation' class='dijitEditorSpellCheckTable'>" +
				"<tr>" +
					"<td colspan='3' class='alignBottom'>" +
						...
						"<div class='dijitEditorSpellCheckBusyIcon' id='${id}_progressIcon'></div>" +
					"</td>" +
				"</tr><tr>" +
					"<td class='dijitEditorSpellCheckBox'>" +
						"<input dojoType='rias.riasw.form.TextBox' required='false' intermediateChanges='true' class='dijitEditorSpellCheckBox' dojoAttachPoint='unfoundTextBox' id='${textId}'/>" +
					"</td><td>" +
						"<button dojoType='rias.riasw.form.Button' class='blockButton' dojoAttachPoint='skipButton'>${skip}</button>" +
					"</td><td>" +
						"<button dojoType='rias.riasw.form.Button' class='blockButton' dojoAttachPoint='skipAllButton'>${skipAll}</button>" +
					"</td>" +
				"</tr><tr>" +
					"<td class='alignBottom'>" +
						"<label for='${selectId}'>${suggestions}</label>" +
					"</td><td colspan='2'>" +
						"<button dojoType='rias.riasw.form.Button' class='blockButton' dojoAttachPoint='toDicButton'>${toDic}</button>" +
					"</td>" +
				"</tr><tr>" +
					"<td>" +
						"<select dojoType='rias.riasw.form.MultiSelect' id='${selectId}' class='dijitEditorSpellCheckBox listHeight' dojoAttachPoint='suggestionSelect'></select>" +
					"</td><td colspan='2'>" +
						"<button dojoType='rias.riasw.form.Button' class='blockButton' dojoAttachPoint='replaceButton'>${replace}</button>" +
						"<div class='topMargin'>" +
							"<button dojoType='rias.riasw.form.Button' class='blockButton' dojoAttachPoint='replaceAllButton'>${replaceAll}</button>" +
						"<div>" +
					"</td>" +
				"</tr><tr>" +
					"<td>" +
						"<div class='topMargin'>" +
							"<button dojoType='rias.riasw.form.Button' dojoAttachPoint='cancelButton'>${cancel}</button>" +
						"</div>" +
					"</td><td>" +
					"</td><td>" +
					"</td>" +
			...
			select.addItems = function(/*Array*/ items){
				...
				if(items && items.length > 0){
					rias.forEach(items, function(item, i){
						o = rias.dom.create("option", {innerHTML: item, value: item}, _this.domNode);
						if(i == 0){
							o.selected = true;
						}
					});
	...
	var SpellCheck = rias.declare(pluginsName + ".SpellCheck", [_Plugin], {
		...
		_initButton: function(){
			...
			var dialogPane = (this._dialog = new DialogPanel({
				ownerRiasw: this,
			...
			this.button = new DropDownButton({
				...
				dropDown: dialogPane,
				id: rias.getUniqueId(this.declaredClass.replace(/\./g,"_")) + "_dialogPane",
					...
					if(_this._dialogContent.closable){
						...
						if(rias.has("ie")){
							...
							if(pos < list.length && pos >=0 ){
								rias.dom.setStyle(list[pos], _this._normalIncorrectStyle);
							}
						}
						if(this._opened){
							rias.dom.closePopup(this.dropDown);
							if(focus){ this.focus(); }
							this._opened = false;
							this.state = "";
						}
			...
			if(this.interactive){
				...
				_this._contextMenu = new Menu({
					...
					bindDomNode: function(/*String|DomNode*/ node){
						...
						rias.dom.setAttr(node, "_dijitMenu" + this.id, this._bindings.push(binding));
						...
						var doConnects = rias.hitch(this, function(cn){
							return [
								...
								rias.connect(cn, this.leftClickToOpen ? "onclick" : "oncontextmenu", this, function(evt){
								...
								rias.connect(cn, "onkeydown", this, function(evt){
