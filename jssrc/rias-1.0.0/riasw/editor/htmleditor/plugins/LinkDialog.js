define([
	...
	"rias/riasw/form/Select",
	"rias/riasw/form/ValidationTextBox",
	"rias/riasw/layout/DialogPanel",
	"dojo/i18n!dijit/nls/common",
	"dojo/i18n!./nls/LinkDialog"
], function(rias, _Plugin, rangeapi, Button, DropDownButton, Select, ValidationTextBox, DialogPanel){
	...
	var LinkDialog = rias.declare(pluginsName + ".LinkDialog", _Plugin, {
		...
		/*linkDialogTemplate: [
			"<table role='presentation'>",
				"<tr>",
					...
					"</td><td>",
						"<input data-dojo-type='rias.riasw.form.ValidationTextBox' required='true' id='${id}_urlInput' name='urlInput' data-dojo-props='intermediateChanges:true'/>",
					"</td>",
				"</tr><tr>",
					"<td>",
						"<label for='${id}_textInput'>${text}</label>",
					"</td><td>",
						"<input data-dojo-type='rias.riasw.form.ValidationTextBox' required='true' id='${id}_textInput' name='textInput' data-dojo-props='intermediateChanges:true'/>",
					"</td>",
				"</tr><tr>",
					"<td>",
						"<label for='${id}_targetSelect'>${target}</label>",
					"</td><td>",
						"<select id='${id}_targetSelect' name='targetSelect' data-dojo-type='rias.riasw.form.Select'>",
				...
				"</tr><tr>",
					"<td colspan='2'>",
						"<button data-dojo-type='rias.riasw.form.Button' type='submit' id='${id}_setButton'>${set}</button>",
						"<button data-dojo-type='rias.riasw.form.Button' type='button' id='${id}_cancelButton'>${buttonCancel}</button>",
					"</td>",
				"</tr>",
			"</table>"
		].join(""),*/
		...
		_initButton: function(){
			...
			var messages = rias.delegate(
				rias.i18n.getLocalization("dijit", "common", this.lang),
			...
			//this.button.loadDropDown = rias.hitch(this, "_loadDropDown");
			this.tag = this.command == 'insertImage' ? 'img' : 'a';
			this._urlRegExp = new RegExp("^" + this.urlRegExp + "$", "i");
			this._emailRegExp = new RegExp("^" + this.emailRegExp + "$", "i");
			this.button = new DropDownButton({
				ownerRiasw: this,
				label: messages[this.command],
				showLabel: false,
				tooltip: messages[this.command],
					...
					moduleMeta: {
						...
						_riaswChildren: [{
							...
							_riaswChildren: [{
								...
								onChange:  function(){
									self._checkAndFixInput(this._riasrModule);
								}
							},{
								_riaswType: "rias.riasw.form.ValidationTextBox",
								...
								_riaswType: "rias.riasw.form.Select",
								_riaswIdOfModule: "targetSelect",
								name: "targetSelect",
		...
		/*_loadDropDown: function(callback){
			...
			var _this = this;
			this.tag = this.command == 'insertImage' ? 'img' : 'a';
			var messages = rias.delegate(rias.i18n.getLocalization("dijit", "common", this.lang),
				rias.i18n.getLocalization(pluginsName, "LinkDialog", this.lang));
			var dropDown = (this.dropDown = this.button.dropDown = new DialogPanel({
				ownerRiasw: this,
				dialogType: "tip",
				caption: messages[this.command + "Title"],
			...
			dropDown.startup();
			this._urlInput = rias.by(this._uniqueId + "_urlInput");
			this._textInput = rias.by(this._uniqueId + "_textInput");
			this._setButton = rias.by(this._uniqueId + "_setButton");
			this.own(rias.by(this._uniqueId + "_cancelButton").on("click", rias.hitch(this.dropDown, "onCancel")));
			if(this._urlInput){
	...
	var ImgLinkDialog = rias.declare(pluginsName + ".ImgLinkDialog", [LinkDialog], {
		...
		/*linkDialogTemplate: [
			...
			"<label for='${id}_urlInput'>${url}</label>",
			"</td><td>",
			"<input dojoType='rias.riasw.form.ValidationTextBox' regExp='${urlRegExp}' " +
				"required='true' id='${id}_urlInput' name='urlInput' data-dojo-props='intermediateChanges:true'/>",
			"</td></tr><tr><td>",
			"<label for='${id}_textInput'>${text}</label>",
			"</td><td>",
			"<input data-dojo-type='rias.riasw.form.ValidationTextBox' required='false' id='${id}_textInput' " +
