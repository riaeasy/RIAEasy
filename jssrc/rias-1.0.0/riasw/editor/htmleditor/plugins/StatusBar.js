...
], function(rias, _Widget, _TemplatedMixin, _Plugin, Resizer) {

	//rias.experimental("dojox.editor.plugins.StatusBar");
	var pluginsName = "rias.riasw.editor.htmleditor.plugins";

	var _StatusBar = rias.declare(pluginsName + "._StatusBar", [_Widget, _TemplatedMixin],{
		...
		templateString:
			'<div class="dojoxEditorStatusBar">' +
				'<table><tbody><tr>'+
					'<td class="dojoxEditorStatusBarText" tabindex="-1" aria-role="presentation" aria-live="aggressive">' +
						'<span dojoAttachPoint="barContent">&nbsp;</span>' +
					'</td><td>' +
						'<span dojoAttachPoint="handle"></span>' +
					'</td>' +
				'</tr></tbody><table>'+
			'</div>',
	...
	var StatusBar = rias.declare(pluginsName + ".StatusBar", _Plugin, {
		...
		setEditor: function(editor){
			...
			if(this.resizable){
				...
				rias.dom.setStyle(this.statusBar.handle.parentNode, "display", "none");
			}
			var pos = null;
			if(editor.footer.lastChild){
				pos = "after";
			}
			rias.dom.place(this.statusBar.domNode, editor.footer.lastChild || editor.footer, pos);
		...
		destroy: function(){
			...
			if(this._msgListener){
				rias.disconnect(this._msgListener);
