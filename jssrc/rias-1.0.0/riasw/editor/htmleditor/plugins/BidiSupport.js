			...
			editor.onLoadDeferred.then(function(){
				...
				for(i = 0; i < edPlugins.length; i++){
					...
					if(p.constructor === EnterKeyHandling){						
						...
						editor._indentImpl = function(){//rias.hitch(self, "_indentAndOutdent", "indent");
							self._indentAndOutdent("indent");
						};
						editor._outdentImpl = function(){//rias.hitch(self, "_indentAndOutdent", "outdent");
							self._indentAndOutdent("outdent");
						};
					}else if(p.constructor === FontChoice && p.command === "formatBlock"){
						self.own(rias.before(p.button, "_execCommand", function(){//rias.hitch(self, "_handleNoFormat")));
							self._handleNoFormat();
						}));
					}
				}
				editor.addPlugin({
				...
				rias.forEach(nodesInfo.nodes, function(x){
					...
					if(this._hasTag(x, "LI")){
						...
						if(realDir == "rtl" && marginLeft != 0){
							rias.dom.setStyle(x, {marginLeft: "", marginRight: "" + marginLeft + "px"});
						}else if(realDir == "ltr" && marginRight != 0){
							rias.dom.setStyle(x, {marginRight: "", marginLeft: "" + marginRight + "px"});
						}
					}
				}, this);
		...
		_prepareLists: function(nodesInfo, arg){
			...
			rias.forEach(nodesInfo.nodes, function(x, index, arr){
				if(has("mozilla") || has("webkit")){
					...
					if(this._hasTag(x, "LI")){
						var dir = rias.dom.getComputedStyle(x).direction;
		...
		_execInsertLists: function(nodesInfo){
			rias.forEach(nodesInfo.nodes, function(x, index){
				if(this._hasTag(x, "LI")){
					...
					if(x.firstChild && x.firstChild.tagName){
						if(this._hasTagFrom(x.firstChild,this._lineStyledTextArray)){
							var style = rias.dom.getComputedStyle(x.firstChild),
								align = this._refineAlignment(style.direction, style.textAlign);
							...
							rias.dom.setStyle(x, {marginLeft: leftMargin, marginRight: rightMargin});
							rias.dom.setStyle(x.firstChild, {direction: "", textAlign: ""});
							if(!has("mozilla")){
								rias.dom.setStyle(x.firstChild, {marginLeft: "", marginRight: ""});
							}
						}
					}
				...
				if(has("ie")){
					...
					if(this._hasTag(x, "P") && this.blockMode.toUpperCase() == "DIV"){
						if(this._hasTag(x.firstChild,"SPAN") && rias.dom.hasAttr(x.firstChild, "bogusIEFormat")){
							if(rias.dom.getAttr(x.firstChild, "bogusIEFormat").toUpperCase() === "PRE"){
								var pre = rias.dom.create("pre", {innerHTML: x.innerHTML}, x, "before");
					...
					var style = rias.dom.getComputedStyle(div), dir = style.direction, align = style.textAlign,
						curAlign = rias.dom.getComputedStyle(x).textAlign;
		...
		_prepareIndent: function(nodesInfo){
			rias.forEach(nodesInfo.nodes, function(x){
				if(has("mozilla")){
					var cell = this._getParentFrom(x, ["TD"]);
					if(!!cell && (query("div[tempRole]", cell).length == 0)){
						rias.dom.create("div", {innerHTML: this.bogusHtmlContent, tempRole: "true"}, cell);
					}
					if(this._hasTag(x, "LI")){
				...
				if(has("webkit") && this._hasTag(x, "LI") && this._hasStyledTextLineTag(x.firstChild)){
					...
					x.removeChild(x.firstChild);
					rias.dom.create("span",{innerHTML: this.bogusHtmlContent, bogusFormat: name}, x, "first");
				}
			},this);			
		},
				...
				if(has("mozilla") || has("webkit")){
					...
					if(has("webkit")){
						if(this._hasTag(x, "LI") && this._hasStyledTextLineTag(x.firstChild)){
							...
							x.removeChild(x.firstChild);
							styledSpan = rias.dom.create("span", {innerHTML: this.bogusHtmlContent, bogusFormat: name}, x, "first");
						}
					}
					if(x.firstChild && x.firstChild.tagName){     
						if(this._hasTagFrom(x.firstChild, this._lineStyledTextArray)){
