define([
	...
	"rias/riasw/form/Uploader", //FIXME: deprecated.  Use Uploader instead
	"dojo/i18n!./nls/LocalImage"
], function(rias, _Plugin, LinkDialog, DialogPanel, Button, TextBox, ValidationTextBox, DropDownButton, Uploader, messages) {
	...
	var LocalImage = rias.declare(pluginsName + ".LocalImage", LinkDialog.ImgLinkDialog, {
		...
		//
		//		Dependencies:
		//		This plugin depends on rias.riasw.form.Uploader to upload the images on the local driver.
		...
		linkDialogTemplate: [
			...
			"<table role='presentation'>",
				"<tr>",
					...
					"</td>",
				"</tr><tr>",
					"<td class='dijitEditorEilDialogField'>",
						"<input dojoType='rias.riasw.form.ValidationTextBox' class='dijitEditorEilDialogField'" +
				...
				"</tr><tr>",
					"<td>",
						"<input dojoType='rias.riasw.form.TextBox' required='false' id='${id}_textInput' " +
				...
				"</tr><tr>",
					"<td colspan='2'>",
						"<button dojoType='rias.riasw.form.Button' id='${id}_setButton'>${set}</button>",
					"</td>",
				"</tr>",
			"</table>"
		].join(""),
		...
		_initButton: function(){
			// summary:
			//		Override _Plugin._initButton() to initialize DropDownButton and DialogPanel.
			// tags:
			//		protected
			var self = this;
			...
			var dropDown = (this.dropDown = new DialogPanel({
				...
				onClose: function(){
					rias.disconnect(self.blurHandler);
					self.blurHandler = null;
					this.onHide();
				},
				onCancel: function(){
		...
		_initialUploader: function(){
			...
			if(self.uploadable && !self._uploader){
				...
				// DialogPanel will call reset on all the widgets contained within it.
				...
				self.connect(fup, "onClick", function(){
				...
				self.connect(fup, "onChange", function(data){
					self._isLocalFile = true;
					urlInput.set("value", data[0].name); //Single selection
					urlInput.focus();
				});

				self.connect(fup, "onComplete", function(data){
				...
				self.connect(fup, "onError", function(evtObject){
					// summary:
					//		Fires on errors
