		...
		setEditor: function(editor){
			...
			editor.onLoadDeferred.then(rias.hitch(this, function(){
				this.connect(this.editor.editNode, "onmousemove", function(evt){
					var editorCoords = rias.dom.position(editor.iframe, true),
					...
					if(!this.isDragging){
						...
						if(obj.tagName && obj.tagName.toLowerCase() == "td"){
							var objCoords = rias.dom.position(obj), ox = objCoords.x, ow = objCoords.w,
						...
						var activeCell = this.activeCell,
							activeCoords = rias.dom.position(activeCell), ax = activeCoords.x, aw = activeCoords.w,
							sibling = nextSibling(activeCell), siblingCoords, sx, sw,
							containerCoords = rias.dom.position(getTable(activeCell).parentNode),
							ctx = containerCoords.x, ctw = containerCoords.w;

						if(sibling){
							siblingCoords = rias.dom.position(sibling);
							sx = siblingCoords.x;
							sw = siblingCoords.w;
						}
				...
				this.connect(ruleDiv, "onmousedown", function(evt){
					var editorCoords = rias.dom.position(editor.iframe, true),
						tableCoords = rias.dom.position(getTable(this.activeCell));
				...
				this.connect(ruleDiv, "onmouseup", function(evt){
					var activeCell = this.activeCell,
						activeCoords = rias.dom.position(activeCell), aw = activeCoords.w, ax = activeCoords.x,
						sibling = nextSibling(activeCell), siblingCoords, sx, sw,
						editorCoords = rias.dom.position(editor.iframe), ex = editorCoords.x,
						table = getTable(activeCell), tableCoords = rias.dom.position(table),
					...
					marginBox(headerCell, rias.dom.position(activeCell).w);
					if(sibling){
						marginBox(headerSibling, rias.dom.position(sibling).w);
					}
					rias.dom.setStyle(editor.editNode, {
						cursor: "auto"
					});
			...
			function getHeaderCell(/*DomNode*/ t){
				...
				var tds = rias.dom.query("td", getTable(t)),
					len = tds.length;
				for(var i = 0; i < len; i++){
					if(rias.dom.position(tds[i]).x == rias.dom.position(t).x){
						return tds[i];
					}
				}
				return null;
