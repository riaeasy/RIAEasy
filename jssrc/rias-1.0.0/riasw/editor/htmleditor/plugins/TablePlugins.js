define([
	...
	"rias/riasw/widget/ColorPalette",
...
], function(rias, _Plugin, rangapi, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin,
	DialogPanel, Button, DropDownButton, TextBox, FilteringSelect, Menu, MenuItem, MenuSeparator, ColorPalette, ColorPicker,
	...
	var TableHandler = rias.declare(_Plugin, {
		...
		initialize: function(editor){
			...
			editor.onLoadDeferred.then(rias.hitch(this, function(){
				...
				this._myListeners = [
					rias.connect(this.editorDomNode , "mouseup", this.editor, "onClick"),
					rias.connect(this.editor, "onDisplayChanged", this, "checkAvailable"),
					rias.connect(this.editor, "onBlur", this, "checkAvailable"),
					rias.connect(this.editor, "_saveSelection", this, function(){
						// because on IE, the selection is lost when the iframe goes out of focus
						this._savedTableInfo = this.getTableInfo();
					}),
					rias.connect(this.editor, "_restoreSelection", this, function(){
...
//		return Math.floor(new Date().getTime() * 0.00000001);
		...
		connectTableKeys: function(){
			...
			this.cnKeyDn = rias.connect(node, "onkeydown", this, "onKeyDown");
			this.cnKeyUp = rias.connect(node, "onkeyup", this, "onKeyUp");
			this._myListeners.push(rias.connect(node, "onkeypress", this, "onKeyUp"));
		},

		disconnectTableKeys: function(){
			//console.log("disconnect")
			rias.disconnect(this.cnKeyDn);
			rias.disconnect(this.cnKeyUp);
		...
		onKeyUp: function(evt){
			...
			if(key == 9 && this.stopEvent){ rias.event.stopEvent(evt);}
		},
	...
	var TablePlugins = rias.declare(pluginsName + ".TablePlugins", _Plugin, {
			...
			destroy: function(){
				...
				this.inherited(arguments);
				rias.disconnect(this._availableTopic);
	...
	var EditorTableDialog = rias.declare(pluginsName + ".EditorTableDialog", [DialogPanel, _TemplatedMixin, _WidgetsInTemplateMixin], {
		// summary:
		...
		onInsert: function(){
			...
			var cl = rias.connect(this, "onHide", function(){
				rias.disconnect(cl);
				var self = this;
				setTimeout(function(){
		...
		onCancel: function(){
			...
			//		when closed.
			var c = rias.connect(this, "onHide", function(){
				rias.disconnect(c);
				var self = this;
				setTimeout(function(){
					self.destroyRecursive();
	...
	var InsertTable = rias.declare(pluginsName + ".InsertTable", TablePlugins, {
		...
		modTable: function(){
			...
			w.show();
			var c = rias.connect(w, "onBuildTable", this, function(obj){
				rias.disconnect(c);
	...
	var EditorModifyTableDialog = rias.declare([DialogPanel, _TemplatedMixin, _WidgetsInTemplateMixin], {
		...
		postCreate: function(){
			...
			this.connect(this.borderCol, "click", function(){
				...
				rias.dom.openPopup({popup:w1, around:this.borderCol});
				w1.focus();
			});
			...
			this.connect(this.backgroundCol, "click", function(){
				...
				rias.dom.openPopup({popup:w2, around:this.backgroundCol});
				w2.focus();
			});
			this.own(w1, w2);
			this.pickers = [ w1, w2 ];
	...
	var CellColorDropDown = rias.declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
		// summary:
			...
			"<div style='display: none; position: absolute; top: -10000; z-index: -10000'>" +
				"<div dojoType='rias.riasw.layout.DialogPanel' dojoAttachPoint='dialog' class='dojoxEditorColorPicker'>" +
					...
					"<div style='margin: 0.5em 0em 0em 0em'>" +
						"<button dojoType='rias.riasw.form.Button' type='submit' dojoAttachPoint='_setButton'>${buttonSet}</button>" +
						"&nbsp;" +
						"<button dojoType='rias.riasw.form.Button' type='button' dojoAttachPoint='_cancelButton'>${buttonCancel}</button>" +
					"</div>" +
				"</div>" +
			"</div>",
	...
	var ColorTableCell = rias.declare(pluginsName + ".ColorTableCell", TablePlugins, {
		...
		constructor: function(){
			...
			this.closable = true;
			this.buttonClass = DropDownButton;
			...
			rias.before(this.dropDown, "onOpen", function(){
