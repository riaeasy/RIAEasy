define([
	...
	"rias/riasw/form/FilteringSelect",
	"rias/riasw/store/MemoryStore",
	"dojo/i18n!./nls/FontChoice"
], function(rias, _Plugin, rangeapi, _Widget, _TemplatedMixin, _WidgetsInTemplateMixin, FilteringSelect, MemoryStore){

	// module:
	//		dijit/_editor/plugins/FontChoice
	var pluginsName = "rias.riasw.editor.htmleditor.plugins";

	var _FontDropDown = rias.declare(pluginsName + "._FontDropDown", [_Widget, _TemplatedMixin, _WidgetsInTemplateMixin], {
			// summary:
			...
			templateString:
				"<span style='white-space: nowrap' class='dijit dijitReset dijitInline'>" +
					...
					"<input data-dojo-type='rias.riasw.form.FilteringSelect' required='false' data-dojo-props='labelType:\"html\", labelAttr:\"label\", searchAttr:\"name\"' class='${comboClass}' " +
	...
	var _FontNameDropDown = rias.declare(pluginsName + "._FontNameDropDown", _FontDropDown, {
		...
		postMixInProperties: function(){
			...
			if(rias.isArray(this.extraFonts)){
				this.values = rias.concatUnique([], this.extraFonts, this.values);
			}
