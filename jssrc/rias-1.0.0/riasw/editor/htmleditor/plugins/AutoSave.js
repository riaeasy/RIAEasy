define([
	...
	var AutoSave = rias.declare(pluginsName + ".AutoSave", Save, {
		...
		_initButton: function(){
			...
			this._menuItemAutoSaveClickHandler = rias.connect(menuItemAutoSave, "onClick", this, "_showAutSaveSettingDialog");
		},
		...
		_showAutSaveSettingDialog: function(){
			...
			rias.input({
				...
				value: this.interval,
				infos: this._strings["saveSettingdialogParamLabel"],
				onSubmit: function(){
					...
					if(interval > 0){
						...
						rias.disconnect(self._menuItemAutoSaveClickHandler);
						self._menuItemAutoSave.set("label", self._strings["saveSettingLabelOff"]);
						self._menuItemAutoSaveClickHandler = rias.connect(self._menuItemAutoSave, "onClick", self, "_onStopClick");
		...
		_onStopClick: function(){
			...
			rias.disconnect(this._menuItemAutoSaveClickHandler);
			this._menuItemAutoSave.set("label", this._strings["saveSettingLabelOn"]);
			this._menuItemAutoSaveClickHandler = rias.connect(this._menuItemAutoSave, "onClick", this, "_showAutSaveSettingDialog");
		...
		onSuccess: function(resp, ioargs){
			...
			rias.message({
				dialogType: "tip",
				around: this.button,
				content: rias.substitute(this._strings["saveMessageSuccess"], {
		...
		onError: function(error, ioargs){
			...
			rias.message({
				dialogType: "tip",
				around: this.button,
				content: rias.substitute(this._strings["saveMessageFail"], {
					"0": rias.dateLocale.format(new Date(), {selector: "time"})
				}),
				//caption: "",
				showCaption: false,
