define([
	...
	var ViewSource = rias.declare(pluginsName + ".ViewSource", _Plugin, {
			...
			try{
				...
				if(source){
					...
					var resizer = function(){
						...
						this._resizer = setTimeout(function(){
					...
					setTimeout(rias.hitch(this, this._resize), 100);
		...
		_resize: function(){
			...
			var eb = rias.dom.position(ed.domNode);

			// Styles are now applied to the internal source container, so we have
			// to subtract them off.
			var containerPadding = rias.dom.getPadBorderExtents(ed.iframe.parentNode);
			var containerMargin = rias.dom.getMarginExtents(ed.iframe.parentNode);

			var extents = rias.dom.getPadBorderExtents(ed.domNode);
			var edb = {
				w: eb.w - extents.w,
			...
			rias.dom.setMarginBox(this.sourceArea, {
		...
		_createSourceView: function(){
			...
			rias.dom.place(this.sourceArea, ed.iframe, "before");
			...
			for(i = 0; i < edPlugins.length; i++){
				...
				// display the textarea.
				p = edPlugins[i];
				if(p && (//p.declaredClass === (dijit._scopeName + "._editor.plugins.FullScreen") ||
					p.declaredClass === pluginsName + ".FullScreen")){
					this._fsPlugin = p;
					break;
				}
			}
