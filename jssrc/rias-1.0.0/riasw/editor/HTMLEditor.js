...
define([
	"rias",
	"rias/riasw/editor/RichText",///修改 style 不是字符串时的问题。
	"rias/riasw/widget/Toolbar",
	"rias/riasw/widget/ToolbarSeparator",
	...
	var riaswType = "rias.riasw.editor.HTMLEditor";
	var keys = rias.keys,
		has = rias.has;
	var pluginsDefault = [
			"preview",
			...
			"|", "insertEntity",
			"|", "createLink", "unlink", "insertAnchor", "insertImage",
			...
			{name: "statusBar", resizable: false},
			"safePaste", "collapsibleToolbar"
		],
		pluginsAll = [
			"newPage", "autoSave", "preview", "print",
			{name: "prettyPrint", indentBy: "3", entityMap: rias.html.entities.html.concat(rias.html.entities.latin)},
			{name: "viewSource", stripScripts: true, stripComments: true}, "showBlockNodes",
			//{name: "fullscreen", zIndex: 5000},///暂时不用，需要修改为 弹出窗口
			"|", "undo", "redo", "cut", "copy", "paste", "pasteFromWord",
			"|", "selectAll", "findReplace", "removeFormat", "delete",
			"|", "justifyLeft", "justifyRight", "justifyCenter", "justifyFull",
			"||", "bold", "italic", "underline", "strikethrough", "subscript", "superscript", "foreColor", "hiliteColor",
			"|", {name: "fontName", extraFonts: ["微软雅黑", "宋体"]}, "fontSize", "formatBlock",
			"||", "pageBreak", "insertHorizontalRule", "insertOrderedList", "insertUnorderedList", "indent", "outdent", "blockquote", "toggleDir",
			"|", "insertEntity", "smiley",
			"|", "createLink", "unlink", "insertAnchor", "insertImage",
	...
	var Widget = rias.declare(riaswType, [RichText], {
		...
		postCreate: function(){
			...
			if(has("webkit")){
				...
				rias.dom.setStyle(this.domNode, "KhtmlUserSelect", "none");
			}
		...
		startup: function(){
			...
			var i = rias.indexOf(this.plugins, "default");
			if(i >= 0){
				this.plugins.splice.apply(this.plugins, [i, 1].concat(rias.filter(pluginsDefault, function(a){
		...
		addPlugin: function(/*String||Object||Function*/ plugin, /*Integer?*/ index){
			...
			if(!args.setEditor){
				...
				if(args.name){
					...
					//}else{
					//	rias.publish(dijit._scopeName + ".Editor.getPlugin", obj);	// publish
					}
				}
				if(!obj.plugin){
					try{
		...
		resize: function(box){
			...
			if(box){
				...
				var cs = rias.dom.getComputedStyle(this.domNode);
				rias.dom.setMarginBox(this.domNode, box, cs);
				box = rias.dom.getMarginBox(this.domNode, cs);
				this._contentBox = rias.dom.marginBox2contentBox(this.domNode, box, cs);
				this.layout();
			}
			/*
			 else{
		...
		layout: function(){
			...
			var areaHeight = (this._contentBox.h -
				(this.getHeaderHeight() + this.getFooterHeight() +
					rias.dom.getPadBorderExtents(this.iframe.parentNode).h +
					rias.dom.getMarginExtents(this.iframe.parentNode).h));
			this.editingArea.style.height = areaHeight + "px";
			if(this.iframe){
				this.iframe.style.height = "100%";
			}
			//this.toolbar.needLayout = true;
			this.toolbar.resize();
		...
		_setStateClass: function(){
			try{
				...
				if(this.document && this.document.body){
					rias.dom.setStyle(this.document.body, "color", rias.dom.getStyle(this.iframe, "color"));
				}
	...
	Widget.pluginsDefault = pluginsDefault;
