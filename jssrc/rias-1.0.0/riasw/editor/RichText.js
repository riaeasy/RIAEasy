...
define([
	...
	"./htmleditor/range",
	"./htmleditor/html"
], function(rias, _Widget, _CssStateMixin, dijit, domReady, rangeapi, htmlapi){

	rias.theme.loadThemeCss([
		"riasw/editor/HTMLEditor.css",
		"riasw/editor/htmleditor/editorPlugins.css"
	]);
	...
	var riaswType = "rias.riasw.editor.RichText";
	var Widget = rias.declare(riaswType, [_Widget, _CssStateMixin], {
		...
		setupDefaultShortcuts: function(){
			...
			// tags:
			//		protected
			var exec = rias.hitch(this, function(cmd, arg){
				return function(){
		...
		_localizeEditorCommands: function(){
			...
			var inject = rias.hitch(this, function(){
		...
		open: function(/*DomNode?*/ element){
			...
			if(rias.isString(this.value)){
				...
				rias.dom.place(dn, ta, "before");
				var tmpFunc = rias.hitch(this, function(){
				...
				if(ta.form){
					...
					rias.on(ta.form, "submit", rias.hitch(this, function(){
						// Copy value to the <textarea> so it gets submitted along with form.
						// FIXME: should we be calling close() here instead?
						rias.dom.setAttr(ta, 'disabled', this.disabled); // don't submit the value if disabled
			...
			if(this.name !== "" && (!rias.config["useXDomain"] || rias.config["allowXdRichTextSave"])){
				var saveTextarea = rias.dom.byId(dijit._scopeName + "._editor.RichText.value");
			...
			ifr.style.height = "100%";
			ifr.frameBorder = 0;
			ifr._loadFunc = rias.hitch(this, function(w){
				...
				this.selection = new rias.dom.selection.SelectionManager(w);
		...
		_applyEditingAreaStyleSheets: function(){
			...
			var text = '', i = 0, url, ownerWindow = rias.dom.getWindow(this.ownerDocument);
			while((url = files[i++])){
		...
		addStyleSheet: function(/*dojo/_base/url*/ uri){
			...
			var url = uri.toString(), ownerWindow = rias.dom.getWindow(this.ownerDocument);
...
//			console.debug("dijit/_editor/RichText.addStyleSheet(): Style sheet "+url+" is already applied");
			...
			this.onLoadDeferred.then(rias.hitch(this, function(){
...
//			console.debug("dijit/_editor/RichText.removeStyleSheet(): Style sheet "+url+" has not been applied");
		...
		onLoad: function(/*String*/ html){
			...
			if(!this.window.__registeredWindow){
				...
				this._iframeRegHandle = rias.dom.focusManager.registerIframe(this.iframe);
			}
			...
			if(rias.has("webkit")){
				...
				this.own(rias.on(this.document, "mousedown", rias.hitch(this, function(e){
					var t = e.target;
			...
			var setContent = rias.hitch(this, function(){
				this.set("value", html);
				...
				if(this.focusOnLoad){
					...
					domReady(rias.hitch(this, "defer", "focus", this.updateInterval));
				}
		...
		/*setDisabled: function(disabled){
			...
			// tags:
			//		deprecated
			rias.deprecated('dijit.Editor::setDisabled is deprecated', 'use dijit.Editor::attr("disabled",boolean) instead', 2.0);
		...
		blur: function(){
			...
			// tags:
			//		deprecated
			if(!rias.has("ie") && this.window.document.documentElement && this.window.document.documentElement.focus){
		...
		focus: function(){
			...
			//if(!rias.contains(rias.dom.focusedStack, this.id)){///避免重复 focus，导致重定向和 blur
				if(!this.isLoaded){
		...
		getNodeHtml: function(/* DomNode */ node){
			...
			// tags:
			//		deprecated
			rias.deprecated('dijit.Editor::getNodeHtml is deprecated', 'use dijit/_editor/html::getNodeHtml instead', 2);
		...
		getNodeChildrenHtml: function(/* DomNode */ dom){
			...
			// tags:
			//		deprecated
			rias.deprecated('dijit.Editor::getNodeChildrenHtml is deprecated', 'use dijit/_editor/html::getChildrenHtml instead', 2);
		...
		close: function(/*Boolean?*/ save){
			...
			rias.dom.removeClass(this.domNode, this.baseClass);
			this.isClosed = true;
			this.isLoaded = false;
		...
		_getNodeChildrenHeight: function(node){
			...
			if(node && node.childNodes){
				...
				for(i = 0; i < node.childNodes.length; i++){
					var size = rias.dom.position(node.childNodes[i]);
					h += size.h;
				}
			}
			return h; // Number
		...
		// Needed to support ToggleDir plugin.  Intentionally not inside if(rias.has("dojo-bidi")) block
		// so that (for backwards compatibility) ToggleDir plugin works even when rias.has("dojo-bidi") is falsy.
