...
define([
	...
	rias.theme.loadThemeCss([
		...
		//"riasw/grid/dgrid/dgridskin.css"
	], false, function(){
	...
	_StoreMixin.extend({
		...
		save: function (onlySet) {
			///增加 onlySet，表示 只计算而不提交 store
			...
			function putter(id, dirtyObj, row) {
				...
				return function (object) {
					...
					return rias.when(onlySet && object || store.put(data)).then(function (result) {
		...
		_trackError: function (func, moreArgs) {
			...
			try {
				///增加 moreArgs，可以带参数
				promise = rias.when(func.apply(self, rias.toArray(arguments, 1)));
			} catch (err) {
				// report sync error
	...
	List.extend({
		...
		renderArray: function (results, beforeNode, options) {
			...
			//rias.publish("/rias/bind/start", [pp]);
			...
			//rias.publish("/rias/bind/done", [pp]);
						...
						if (rule = self._columnSizes[column.id]) {
							...
							if(s){
								...
								if(column.asButton){
									...
									colw += _getsw(column.headerNode, (rias.i18n.action.operation + " ").length + "em");
									colw += 10;
								}
							}else{
