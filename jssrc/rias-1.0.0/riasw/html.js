...
define([
	...
	"rias/riasw/xhr",
	"dojo/_base/lang",///lang.extend; 没有 rias.extend
	"dojox/html/entities",
	"dojox/html/format"
], function(rias, riasXhr, lang, _entities, _format){
	...
	function setNodeContent(/*DomNode*/ node, /*String|DomNode|NodeList*/ cont, doEmptyAll){
		// summary:
		...
		if(cont){
			...
			if(typeof cont == "string"){
				cont = rias.dom.toDom(cont, node.ownerDocument);
			}
			if(!cont.nodeType && rias.isArrayLike(cont)){
				// handle as enumerable, but it may shrink as we enumerate it
				for(var startlen = cont.length, i = 0; i < cont.length; i = startlen == cont.length ? i + 1 : 0){
					rias.dom.place( cont[i], node, "last");
				}
			}else{
				// pass nodes, documentFragments and unknowns through to dojo.place
				rias.dom.place(cont, node, "last");
			}
		}
	...
	var html = rias.html = {
			...
			return cont.replace(htmlAttrPaths,
				function(tag, start, name, delim, relUrl, delim2, cssText, end){
					return start + (name ?
						...
						('style=' + delim2 + html._adjustCssPaths(url, cssText) + delim2)
					) + end;
				}
			);
		},
		_snarfStyles: function(/*String*/cssUrl, /*String*/cont, /*Array*/styles){
			...
			return cont.replace(/(?:<style([^>]*)>([\s\S]*?)<\/style>|<link\s+(?=[^>]*rel=['"]?stylesheet)([^>]*?href=(['"])([^>]*?)\4[^>\/]*)\/?>)/gi,
				function(ignore, styleAttr, cssText, linkAttr, delim, href){
					...
					if(cssText){
						i = styles.push(cssUrl ? html._adjustCssPaths(cssUrl, cssText) : cssText);
					}else{
		...
		evalInGlobal: function(code, appendNode){
			...
			appendNode = appendNode || rias.dom.doc.body;
	...
	var _ContentSetter = rias.declare("rias.html._ContentSetter", null, {
		// node: DomNode|String
		...
		constructor: function(/*Object*/ params, /*String|DomNode*/ node){
			...
			node = this.node = rias.dom.byId( this.node || node );
		...
		_onEnd: function(){
			...
			var self = this,
				...
				code = this._code,
				styles = this._styles;
			this.parseDeferred = rias.newDeferred("html._ContentSetter.parse", rias.defaultDeferredTimeout << 1, function(){
				this.cancel();
			});
			...
			if(this._styleNodes && this._styleNodes.length){
				while(this._styleNodes.length){
					rias.dom.destroy(this._styleNodes.pop());
				}
			}
			// render new style nodes
			if(this.renderStyles && styles && styles.length){
				html._renderStyles(styles);
			}
		...
		_setNodeContent: function(){
			...
			try{
				this.empty(false);
				node = setNodeContent(node, this.content, false);
			}catch(e){
		...
		_parse: function(){
			...
			try{
				...
				var self = this;
				this._parseDeferred = rias.dom.parser.parse({
					rootNode: rootNode,
		...
		_onError: function(type, err, consoleText){
			...
			//setNodeContent(this.node, errText, false);
			try{
				//node.innerHTML = errMess;
				errText = rias.dom.create("div", {
					innerHTML: errText
				});
	...
	lang.extend(rias.dom.query.NodeList, {
