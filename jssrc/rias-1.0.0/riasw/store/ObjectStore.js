...
define([
	...
	var riasType = "rias.riasw.store.ObjectStore";
	return rias.declare(riasType, [rias.ObjectBase, Evented],{///因为不能继承自 rias.ClassBase
		// summary:
		...
		containsValue: function(item, attribute, value){
			...
			return rias.indexOf(this.getValues(item,attribute),value) > -1;
		},
		...
		fetch: function(args){
			...
			if(results.observe){
				...
				this.observing = results.observe(function(object, removedFrom, insertedInto){
					if(rias.indexOf(self._dirtyObjects, object) == -1){
						if(removedFrom == -1){
							self.onNew(object);
						}
