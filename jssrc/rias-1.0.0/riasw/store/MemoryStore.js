...
define([
	...
	Memory.extend({
		...
		loadByHttp: function(query, options){
			var self = this,
				d = rias.newDeferred("MemoryStore.loadByHttp", rias.defaultDeferredTimeout << 1, function(){
					this.cancel();
				});
			options = options || {};
		...
		loadByFile: function(query, options){
			var self = this,
				d = rias.newDeferred("MemoryStore.loadByFile", rias.defaultDeferredTimeout << 1, function(){
					this.cancel();
				}),
				args = {
					options: {}
			...
			if(rias.isString(args.query)){
				args.query = rias.queryToObject(args.query);
			}
	...
	var riaswType = "rias.riasw.store.MemoryStore";
