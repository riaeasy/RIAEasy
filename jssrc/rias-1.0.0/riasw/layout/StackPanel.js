	...
	var Widget = rias.declare(riaswType, _PanelBase, {
		...
		_transition: function(newWidget, oldWidget, animate){
			...
			return df.promise.always(function(result){
				...
				self.resize();
				if(newWidget && self.isShown(false) && self.get("visible")){
		...
		_showChild: function(/*dijit/_WidgetBase*/ page, /*Object*/ args){
			...
			if(page.resize){
				page.resize(args && args.box || this._containerContentBox || this._contentBox);
			}
		...
		_hideChild: function(/*dijit/_WidgetBase*/ page, /*Object*/ args){
			...
			return rias.when(p || page).always(function(){
		...
		_setupChild: function(/*dijit/_WidgetBase*/ child, added, insertIndex, noresize){
			...
			var childNode = child.domNode,
				wrapper = rias.dom.place(
					//"<div role='tabpanel' class='dijitLayoutContainer " + this._baseClass0 + "ChildWrapper dijitHidden'>",
					"<div role='tabpanel' class='" + this._baseClass0 + "ChildWrapper dijitHidden'>",
					child.domNode,
					"replace"),
		...
		addChild: function(/*dijit/_WidgetBase*/ child, /*Integer?*/ insertIndex, noresize){
			...
			this.inherited(arguments, [child, insertIndex, true]);///在 selectChild 中 resize
			if(this._started){
				if(child.get("selected") || !this.selectedChildWidget){
					this.selectChild(child, this.isShown(false) && this.get("visible"));
				}
			}
