...
define([
	...
	var Widget = rias.declare(riaswType, null, {
		...
		selectPanel: function(win, mode){
			...
			if(!win || rias.dom.contains(win, self.selected)){
				///不指定 win、或 win 已经 selected，即 contains(win, self.selected)，以及没有 destroy，则直接返回。
				if(win && !self.selected.isDestroyed(true)){
					win = self.selected;
					/*if(!_visible(win) || win.isHidden() || win.isCollapsed()){
						///不能用 whenPlayed，否则会导致递归。
					...
					}else if(!win.get("selected")){
						///已经是 selected，则不触发 onSelected
						win.set("selected", true);///需要先 set 以避免循环
						win.select(true);
					}*/
					if(!win.get("selected")){
						///已经是 selected，则不触发 onSelected
			...
			if(!rias.isDestroyed(win, true)){
				...
				if(self.selected){
					//self.selected.restore(true).then(function(result){
					self.selected.focus(true);
						df.resolve(self.selected);
					//});
				}else{
