...
define([
	...
	var riaswType = "rias.riasw.widget.Menu";
	var Widget = rias.declare(riaswType, [DropDownMenu], {
		// summary:
		//		A context menu you can assign to multiple elements
		restrictPadding: rias.dom.defaultRestrict || 0,
		...
		addTarget: function(any){
			var node = rias.orDomNode(any, this);
			if(node && !rias.contains(this._bindings, node, "node")){
		...
		bindDomNode: function(/*String|DomNode*/ node){
			...
			if(node.tagName.toLowerCase() == "iframe"){
				...
				cn = rias.dom.body(window.document);
			}else{
			...
			var doConnects = rias.hitch(this, function(cn){
				var selector = this.selector,
					...
					self = this;
				return [
					rias.on(cn, delegatedEvent(this.leftClickToOpen ? "click" : "contextmenu"), function(evt){
						evt.stopPropagation();
						evt.preventDefault();
					...
					rias.on(cn, delegatedEvent("keydown"), function(evt){
		...
		_openMyself: function(args){
			...
			if(coords){
				if(iframe){
					...
					var ifc = rias.dom.position(iframe, true),
						window = this._iframeContentWindow(iframe),
				...
				coords = rias.dom.position(target, true);
				coords.x += 10;
				coords.y += 10;
			}
