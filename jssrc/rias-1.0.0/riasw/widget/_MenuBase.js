...
], function(rias, _Widget, _CssStateMixin, _KeyNavContainer, _TemplatedMixin){

	// module:
	//		rias/riasw/widget/_MenuBase

	return rias.declare("rias.riasw.widget._MenuBase", [_Widget, _TemplatedMixin, _KeyNavContainer, _CssStateMixin], {
		// summary:
		...
		_setActivatedAttr: function(val){
			rias.dom.toggleClass(this.domNode, "dijitMenuActive", val);
			rias.dom.toggleClass(this.domNode, "dijitMenuPassive", !val);
		...
		_openItemPopup: function(/*dijit/MenuItem*/ from_item, /*Boolean*/ focus){
			...
			from_item._openPopup({
				...
				onExecute: rias.hitch(this, "_cleanUp", true),
				onClose: function(){
		...
		_closeChild: function(){
			...
			if(this.currentPopupItem){
				...
				// else, like a Dialog.
				if(this.focused){
					rias.dom.setAttr(this.selected.focusNode, "tabIndex", this.tabIndex);
