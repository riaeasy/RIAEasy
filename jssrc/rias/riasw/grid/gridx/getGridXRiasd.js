	...
	return function(params){
		...
		for(i = 0; i < l; i++){
			...
			if(btn == "btnAdd"){
				...
				//_btnAdd.placeAt(_topBar);
				_topBar._riaswChildren.push(_btnAdd);
			}else if(btn == "btnDele"){
				if(rias.isDijit(btn)){
					_btnDele = btn;
				}else{
				...
				//_btnDele.placeAt(_topBar);
				_topBar._riaswChildren.push(_btnDele);
			}else if(btn != "btnRefresh"){
				if(rias.isDijit(btn) || rias.isRiasd(btn)){
					//btn.placeAt(_topBar);
					_topBar._riaswChildren.push(btn);
				}
			}
		}
		gp.barTop = [_topBar];
		...
		for(i = 0, l = gp.structure.length; i < l; i++){
			if(rias.isString(gp.structure[i].format)){
