...
define("dojo/request/node", ["require", "./util", "./handlers", "../errors/RequestTimeoutError", "../node!http", "../node!https", "../node!url", "../node!stream"], function (require, util, handlers, RequestTimeoutError, http, https, URL, stream) {
    var Stream = stream.Stream, undefined;
    var defaultOptions = {method:"GET", query:null, data:undefined, headers:{}};
    function node(url, options) {
        ...
        var def = util.deferred(response, function (dfd, response) {
            response.clientRequest.abort();
        });
        url = URL.parse(url);
        var reqOptions = response.requestOptions = {hostname:url.hostname, port:url.port, socketPath:options.socketPath, method:options.method, headers:options.headers, agent:options.agent, pfx:options.pfx, key:options.key, passphrase:options.passphrase, cert:options.cert, ca:options.ca, ciphers:options.ciphers, rejectUnauthorized:options.rejectUnauthorized === false ? false : true};
        if (url.path) {
