	...
	"./_base"
		...
		function ShiftRows(s, Nb) {	   // shift row r of state S left by r bytes [5.1.2]
			var r, c,
				t = new Array(4);
			for (r = 1; r < 4; r++) {
				for (c = 0; c < 4; c++){// shift into temp copy
					t[c] = s[r][(c + r) % Nb];
				}
				for (c = 0; c < 4; c++){// and copy back
					s[r][c] = t[c];
				}
		...
		function AESEncryptCtr(plaintext, password, nBits) {
			...
			var blockSize = 16,  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
				counterBlock = new Array(blockSize),	// block size fixed at 16 bytes / 128 bits (Nb=4) for AES
				nonce = (new Date()).getTime();  // milliseconds since 1-Jan-1970
			...
			var keySchedule = KeyExpansion(key),
				...
				ciphertext = new Array(blockCount),  // ciphertext as array of strings
				cipherCntr, blockLength;
			...
			for (b = 0; b < blockCount; b++) {
				...
				var ct = '',
					plaintextByte, cipherByte;
				for (i = 0; i < blockLength; i++) {	 // -- xor plaintext with ciphered counter byte-by-byte --
					plaintextByte = plaintext.charCodeAt(b * blockSize + i);
		...
		function AESDecryptCtr(ciphertext, password, nBits) {
			...
			var blockSize = 16,  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
				...
				ctrTxt = ciphertext[0]; //unescCtrlChars(ciphertext[0]);
			counterBlock = stringToHex(ctrTxt);

			var plaintext = new Array(ciphertext.length - 1);

			for (b = 1; b < ciphertext.length; b++) {
				// set counter (block #) in last 8 bytes of counter block (leaving nonce in 1st 8 bytes)
				for (c = 0; c < 4; c++){
				...
				var pt = '',
					tmp = stringToHex(ciphertext[b]);
				for (i = 0; i < tmp.length; i++) {
					// -- xor plaintext with ciphered counter byte-by-byte --
					var //ciphertextByte = ciphertext[b].charCodeAt(i),
						plaintextByte = tmp[i] ^ cipherCntr[i];
