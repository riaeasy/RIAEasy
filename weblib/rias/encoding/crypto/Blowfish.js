...
////////////////////////////////////////////////////////////////////////////
		...
		function init(key){
			var k=key;
			if(rias.isString(k)){
				k = rias.map(k.split(""), function(item){
			...
			var pos = 0,
				...
				i, j, l;
			var box = {
				p: rias.map(boxes.p.slice(0), function(item){
					var l = k.length,
						j;
					for(j = 0; j < 4; j++){
...
////////////////////////////////////////////////////////////////////////////
		this.getIV = function(/* dojox.encoding.crypto.outputTypes? */ outputType){
			...
			switch(out){
				case crypto.outputTypes.Hex:{
					return rias.map(iv, function(item){
		...
		this.setIV = function(/* string */data, /* dojox.encoding.crypto.outputTypes? */inputType){
			...
			switch(ip){
				case crypto.outputTypes.String:{
					ba = rias.map(data.split(""), function(item){
		...
		this.encrypt = function(/* string */plaintext, /* string */key, /* object? */ao){
			...
			switch(out){
				case crypto.outputTypes.Hex:{
					return rias.map(cipher, function(item){
						return (item <= 0xf ? '0' : '') + item.toString(16);
					}).join("");	//	string
				}
				case crypto.outputTypes.String:{
		...
		this.decrypt = function(/* string */ciphertext, /* string */key, /* object? */ao){
			...
			switch(ip){
				case crypto.outputTypes.Hex:{
					c = [];
					for(i = 0, l = ciphertext.length - 1; i < l; i += 2){
						c.push(parseInt(ciphertext.substr(i, 2), 16));
					}
					break;
				}
				case crypto.outputTypes.String:{
					c = rias.map(ciphertext.split(""), function(item){
						return item.charCodeAt(0);
					});
					break;
				}
			...
			//	convert to string
			return rias.map(pt, function(item){
