...
define([
	...
	var st = rias.getObject("stats", true, math);
	...
	rias.mixin(st, {
		...
		bestFit: function(/* Object[]|Number[] */ a, /* String? */ xProp, /* String? */ yProp){
			...
			if(a[0] !== undefined && typeof(a[0]) === "number"){
				...
				a = rias.map(a, function(item, idx){
					return {
						x: idx,
						y: item
					};
			...
			if(d === 0){
				throw new Error("math.stats.bestFit: the denominator for Pearson's R is 0.");
			}
