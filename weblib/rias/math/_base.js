...
// AMD-ID "rias/math/_base"

define([
	"rias/riasBase"
], function(rias) {

	var math = rias.getObject("math", true, rias);
	...
	rias.mixin(math, {
		...
		_gamma: function(z){
			...
			if(z === 0){
				return answer;
			} // normal integer quick return
			if(Math.floor(z) === z){
				return NaN;
			} // undefined at nonpositive integers since sin() below will return 0
		...
		range: function(/* Number */a, /* Number? */b, /* Number? */step){
			...
			if(s > 0){
				...
				if(s < 0){
					...
					throw new Error("math.range: step must not be zero.");
				}
			}
			return range; 	// Array
		},
		...
		midpoint: function(/* Array */a, /* Array */b){
			...
			if(a.length !== b.length){
				console.error("math.midpoint: Points A and B are not the same dimensionally.", a, b);
			}
			var m=[];
