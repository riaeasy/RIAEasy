...
(function(config){
	...
	var shell = {
		...
		onBackButton: function(){
			...
			}, rias.desktop.desktopTitle || "请选择...", ["是的", "取消"]);
		},
		...
		_loadRias: function() {
			require([
				...
				rias.whenLoaded(function(result){
					...
					var theme = rias.urlParamsToObj(document.location.search).theme,
						mobileTheme = rias.urlParamsToObj(document.location.search).mobileTheme;
					if(!theme){
						theme = "rias";
					}
					if(!mobileTheme){
			...
			rias.whenLoaded(function(result){
				...
				rias.require([
					...
					rias.startupDesktop(rias.mixinDeep({}, initParams, {
							...
							console.info("app.startup, the mainModuleMeta is '" + initParams.mainModuleMeta + "'");
						});
				});
			});
		},
