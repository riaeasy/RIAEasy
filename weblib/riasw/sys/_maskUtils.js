define([
	...
	rias.has.add("mask-image-css", function(global, doc, elt){
	...
	rias.has.add("mask-image", function(){
		return rias.has("mask-image-css") || rias.has("svg");
	});
	...
	return {
		...
		createRoundMask: function(/*DomNode*/node, x, y, r, b, w, h, rx, ry, e){
			...
			if(rias.has("mask-image-css")){			// use -webkit-mask-image
				var id = ("DojoMobileMask" + x + y + w + h + rx + ry).replace(/\./g, "_");
				if (!cache[id]) {
					cache[id] = 1;
					var ctx = rias.dom.doc.getCSSCanvasContext("2d", id, tw, th);
					ctx.beginPath();
					if (rx === ry) {
						// round arc
						if(rx === 2 && w === 5){
			...
			}else if(rias.has("svg")){		// add an SVG image to clip the corners.
				if(node._svgMask){
					node.removeChild(node._svgMask);
				}
				var bg = null;
				for(var p = node.parentNode; p; p = p.parentNode){
					bg = rias.dom.getComputedStyle(p).backgroundColor;
					if(bg && bg !== "transparent" && !bg.match(/rgba\(.*,\s*0\s*\)/)){
						break;
					}
				}
				var svgNS = "http://www.w3.org/2000/svg";
				var svg = rias.dom.doc.createElementNS(svgNS, "svg");
				...
				var path = rias.dom.doc.createElementNS(svgNS, "path");
				e = e || 0;
				rx += e;
				ry += e;
