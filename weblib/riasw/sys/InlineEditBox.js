define([
	...
	"riasw/form/Button",
	"riasw/form/TextBox"
], function(rias, _WidgetBase, _TemplatedMixin, Button, TextBox){

	// module:
	//		riasw/sys/InlineEditBox

	var InlineEditor = rias.declare("riasw.sys._InlineEditor", [_WidgetBase, _TemplatedMixin], {
		// summary:
		...
		templateString:
			'<span data-dojo-attach-point="editNode" role="presentation" class="dijitReset dijitInline dijitOffScreen">' +
				...
				'<span data-dojo-attach-point="buttonContainer">' +
					'<button data-dojo-type="riasw/form/Button" data-dojo-props="label:\'${buttonSave}\',\'class\':\'saveButton\'" data-dojo-attach-point="saveButton" data-dojo-attach-event="onClick:save"></button>' +
					'<button data-dojo-type="riasw/form/Button" data-dojo-props="label:\'${buttonCancel}\',\'class\':\'cancelButton\'" data-dojo-attach-point="cancelButton" data-dojo-attach-event="onClick:cancel"></button>' +
				'</span>' +
			'</span>',
		...
		postCreate: function(){
			...
			if(this.inlineEditBox.autoSave){
					...
					this.after(ew, "onChange", rias.hitch(this, "_onChange"), true);
					// ESC and TAB should cancel and save.
					this.on(ew, "keydown", rias.hitch(this, "_onKeyDown"));
			}else{
				// If possible, enable/disable save button based on whether the user has changed the value
				if("intermediateChanges" in ew){
					ew.set("intermediateChanges", true);
					this.after(ew, "onChange", rias.hitch(this, "_onIntermediateChange"), true);
		...
		focus: function(node, forceVisible){
			...
			if(!this.isDestroyed(true)){
				this.editWidget.focus(node, forceVisible);
				...
				if(this.editWidget.focusNode){
					...
					this._focusManager._onFocusNode(this.editWidget.focusNode);

					if(this.editWidget.focusNode.tagName === "INPUT"){
						this.defer(function(){
							rias.dom.selectInputText(this.editWidget.focusNode);
						});
					}
				}
			}
	...
	var InlineEditBox = rias.declare("riasw.sys.InlineEditBox", _WidgetBase, {
		// summary:
		...
		//		riasw.editor.Editor (for editing HTML) or a Slider (for adjusting a number).
		...
		//		rather than plain text (ex: `riasw.editor.Editor`)
		renderAsHtml: false,

		// editor: String|Function
		//		MID (ex: "riasw/form/TextBox") or constructor for editor widget
		editor: TextBox,
		...
		postMixInProperties: function(){
			...
			rias.dom.addClass(this.displayNode, 'riaswInlineEditBoxDisplayMode');
		},
		_onDestroy: function(){
		...
		_setDisabledAttr: function(/*Boolean*/ disabled){
			...
			rias.dom.toggleClass(this.displayNode, "riaswInlineEditBoxDisplayModeDisabled", disabled);
		...
		_onMouseOver: function(){
			...
			//		private
			if(!this.disabled){
				rias.dom.addClass(this.displayNode, "riaswInlineEditBoxDisplayModeHover");
			}
		},
		_onMouseOut: function(){
			...
			// tags:
			//		private
			rias.dom.removeClass(this.displayNode, "riaswInlineEditBoxDisplayModeHover");
		},
		...
		edit: function(){
			...
			// After edit widget has finished initializing (in particular need to wait for riasw.editor.Editor),
