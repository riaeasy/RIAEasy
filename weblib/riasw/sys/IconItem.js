...
define([
	...
	"./iconUtils",
	"./lazyLoadUtils",
	"dojo/has!riasw-bidi?dojox/mobile/bidi/IconItem"
], function(rias, _ItemBase, iconUtils, lazyLoadUtils){

	// module:
	//		riasw/sys/IconItem

	var riaswType = "riasw.sys.IconItem";
	var IconItem = rias.declare(riaswType, _ItemBase, {
		// summary:
		//		An icon item widget.
		// description:
		...
		// tagType: String
		//		A name of the HTML tagType to create as domNode.
		tagType: "li",

		buildRendering: function(){
			this.domNode = this.srcNodeRef || rias.dom.create(this.tagType);

			if(this.srcNodeRef){
				// reparent
				this._tmpNode = rias.dom.create("div");
				for(var i = 0, len = this.srcNodeRef.childNodes.length; i < len; i++){
					this._tmpNode.appendChild(this.srcNodeRef.firstChild);
				}
			}

			this.iconDivNode = rias.dom.create("div", {
				className:"mblIconArea"
			}, this.domNode);
			this.iconParentNode = rias.dom.create("div", {
				className:"mblIconAreaInner"
			}, this.iconDivNode);
			this.labelNode = rias.dom.create("span", {
				className:"mblIconAreaTitle"
			}, this.iconDivNode);
		...
		startup: function(){
			...
			require([p.iconItemPaneClass], lang.hitch(this, function(module){
				...
				p.paneContainerWidget.addChild(w, this.getIndexInContainer());
	...
	return has("riasw-bidi") ? declare("dojox.mobile.IconItem", [IconItem, BidiIconItem]) : IconItem;
});
