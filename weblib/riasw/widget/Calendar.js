...
define([
...
], function(rias, CalendarLite, _WidgetBase, _CssStateMixin, _TemplatedMixin, DropDownButton) {

	//rias.theme.loadThemeCss([
	//	"riasw/widget/Calendar.css"
	//]);

	var riaswType = "riasw.widget.Calendar";
	var Widget = rias.declare(riaswType, [CalendarLite, _CssStateMixin], {

		baseClass: "riaswCalendar",

		cssStateNodes: {
			"decrementMonth": "riaswCalendarArrow",
			"incrementMonth": "riaswCalendarArrow",
			"previousYearLabelNode": "riaswCalendarPreviousYear",
			"nextYearLabelNode": "riaswCalendarNextYear"
		},
		...
		_createMonthWidget: function(){
			...
			return new Widget._MonthDropDownButton({
				ownerRiasw: this,
				id: this.id + "_mddb",
				"class": "riaswCalendarMonthButton",
				lang: this.lang,
		...
		_onDayMouseOver: function(/*Event*/ evt){
			...
			// set node to the <td>.
			var node = rias.dom.containsClass(evt.target, "riaswCalendarDateLabel") ? evt.target.parentNode : evt.target;

			if(node && (
				(node.dijitDateValue && !rias.dom.containsClass(node, "riaswCalendarDisabledDate"))
					|| node === this.previousYearLabelNode || node === this.nextYearLabelNode
				)){
				rias.dom.addClass(node, "riaswCalendarHoveredDate");
		...
		_onDayMouseOut: function(/*Event*/ evt){
			...
			var cls = "riaswCalendarHoveredDate";
			if(rias.dom.containsClass(this._currentNode, "riaswCalendarActiveDate")){
				cls += " riaswCalendarActiveDate";
			}
		...
		_onDayMouseDown: function(/*Event*/ evt){
			...
			if(node && node.dijitDateValue && !rias.dom.containsClass(node, "riaswCalendarDisabledDate")){
				rias.dom.addClass(node, "riaswCalendarActiveDate");
				this._currentNode = node;
			}
		},
		...
		_onDayMouseUp: function(/*Event*/ evt){
			...
			if(node && node.dijitDateValue){
				rias.dom.removeClass(node, "riaswCalendarActiveDate");
			}
		},
		...
		handleKey: function(/*Event*/ evt){
			...
			//		and also from `rias/form/_DateTimeTextBox` to pass a keydown event
			//		from the `riasw/form/DateTextBox` to be handled in this widget
			// returns:
		...
		onValueSelected: function(/*Date*/ /*===== date =====*/){
			...
			// description:
			//		Formerly used by `riasw/form/_DateTimeTextBox` (and thus `riasw/form/DateTextBox`)
	...
	Widget._MonthDropDownButton = rias.declare("riasw.widget.Calendar._MonthDropDownButton", DropDownButton, {
		// summary:
		...
		_setMonthAttr: function(month){
			...
			//	(rias.has("ie") === 6 ? "" : "<div class='riaswCalendarSpacer'>" + this.dropDown.domNode.innerHTML + "</div>") +
			//		"<div class='riaswCalendarMonthLabel riaswCalendarCurrentMonthLabel'>" + monthNames[month.getMonth()] + "</div>";
			this.set("label", monthNames[month.getMonth()]);
		}
	});

	Widget._MonthDropDown = rias.declare("riasw.widget.Calendar._MonthDropDown", [_WidgetBase, _TemplatedMixin, _CssStateMixin], {
		// summary:
		...
		baseClass: "riaswCalendarMonthMenu riaswMenu",
		...
		_setMonthsAttr: function(/*String[]*/ months){
			...
			rias.forEach(months, function(month, idx){
				var div = rias.dom.create("div", {
					className: "riaswCalendarMonthLabel",
					month: idx,
					innerHTML: month
				}, this.domNode);
				div._cssState = "riaswCalendarMonthLabel";	// trigger _CSSStateMixin magic; property, not attribute.
			}, this);
		},
