define([
	...
	// module:
	//		riasw/widget/CalendarLite

	var riaswType = "riasw.widget.CalendarLite";
	var CalendarLite = rias.declare(riaswType, [_WidgetBase, _TemplatedMixin], {
		// summary:
		...
		//		`<input>` field.  For a form element, use riasw/form/DateTextBox instead.
		//
		//		Note that the parser takes all dates attributes passed in the
		//		[RFC 3339 format](http://www.faqs.org/rfcs/rfc3339.html), e.g. `2005-06-30T08:05:00-07:00`
		//		so that they are serializable and locale-independent.
		//
		//		Also note that this widget isn't keyboard accessible; use riasw.widget.Calendar for that
		// example:
		//	|	var calendar = new riasw.widget.CalendarLite({}, dojo.byId("calendarNode"));
		//
		// example:
		//	|	<div data-dojo-type="riasw/widget/CalendarLite"></div>

		// Template for main calendar
		templateString:
			'<div class="riaswCalendarContainer dijitInline" role="presentation" aria-labelledby="${id}_mddb ${id}_year">' +
				'<div class="dijitReset riaswCalendarMonthContainer" role="presentation">' +
					'<div class="dijitReset riaswCalendarArrow riaswCalendarDecrementArrow" data-dojo-attach-point="decrementMonth">' +
						'<span class="riaswCalendarIncrementControl riaswCalendarDecrease" role="presentation"/>' +
						//'<span data-dojo-attach-point="decreaseArrowNode" class="riaswA11ySideArrow">-</span>' +
					'</div>' +
					'<div class="dijitReset riaswCalendarArrow riaswCalendarIncrementArrow" data-dojo-attach-point="incrementMonth">' +
						'<span class="riaswCalendarIncrementControl riaswCalendarIncrease" role="presentation"/>' +
						//'<span data-dojo-attach-point="increaseArrowNode" class="riaswA11ySideArrow">+</span>' +
					'</div>' +
				...
				'<table cellspacing="0" cellpadding="0" role="grid" data-dojo-attach-point="gridNode">' +
					'<thead class="riaswCalendarDayContainer">' +
						'<tr role="row">' +
							'${!dayCellsHtml}' +
						'</tr>' +
					'</thead>' +
					'<tbody data-dojo-attach-point="dateRowsNode" data-dojo-attach-event="ondijitclick: _onDayClick" class="dijitReset riaswCalendarBodyContainer">' +
						'${!dateRowsHtml}' +
					'</tbody>' +
				'</table>' +
				'<div class="dijitReset riaswCalendarYearContainer" role="presentation">' +
					'<div class="riaswCalendarYearLabel">' +
						'<span data-dojo-attach-point="previousYearLabelNode" class="dijitInline riaswCalendarPreviousYear" role="button"></span>' +
						'<span data-dojo-attach-point="currentYearLabelNode" class="dijitInline riaswCalendarSelectedYear" role="button" id="${id}_year"></span>' +
						'<span data-dojo-attach-point="nextYearLabelNode" class="dijitInline riaswCalendarNextYear" role="button"></span>' +
					'</div>' +
				'</div>' +
			'</div>',

		// Template for cell for a day of the week (ex: M)
		dowTemplateString: '<th class="dijitReset riaswCalendarDayLabelTemplate" role="columnheader" scope="col"><span class="riaswCalendarDayLabel">${d}</span></th>',

		// Templates for a single date (ex: 13), and for a row for a week (ex: 20 21 22 23 24 25 26)
		dateTemplateString: '<td class="dijitReset" role="gridcell" data-dojo-attach-point="dateCells"><span class="riaswCalendarDateLabel" data-dojo-attach-point="dateLabels"></span></td>',
		weekTemplateString: '<tr class="dijitReset riaswCalendarWeekTemplate" role="row">${d}${d}${d}${d}${d}${d}${d}</tr>',
		...
		baseClass: "riaswCalendar riaswCalendarLite",
		...
		_setValueAttr: function(/*Date|Number*/ value, /*Boolean*/ priorityChange){
			...
			if(this._isValidDate(value) && !this.isDisabledDate(value, this.lang)){
				...
				if(this._started && (priorityChange || typeof priorityChange == "undefined")){
		...
		_populateGrid: function(){
			...
			var firstDay = month.getDay(),
				...
				dayOffset = this.dayOffset >= 0 ? this.dayOffset : rias.date.getFirstDayOfWeek(this.lang);
			...
			rias.forEach(this.dateCells, function(template, idx){
				var i = idx + dayOffset;
				var date = new this.dateClassObj(month),
					number, clazz = "riaswCalendar", adj = 0;
				...
				if(!this.dateModule.compare(date, today, "date")){
					clazz = "riaswCalendarCurrentDate " + clazz;
				}

				if(this.isDisabledDate(date, this.lang)){
					clazz = "riaswCalendarDisabledDate " + clazz;
					template.setAttribute("aria-disabled", "true");
				}else{
					clazz = "riaswCalendarEnabledDate " + clazz;
				...
				template.className = clazz + "Month riaswCalendarDateTemplate";
				...
				rias.dom.setAttr(template, 'aria-label', rias.dateLocale.format(date, {
					selector: 'date',
					formatLength: 'long'
				}));
			}, this);
		...
		_populateControls: function(){
			...
			rias.forEach(["previous", "current", "next"], function(name){
				d.setFullYear(y++);
		...
		constructor: function(params /*===== , srcNodeRef =====*/){
			...
			this.dateModule = params.datePackage ? rias.getObject(params.datePackage, false) : rias.date;
			this.dateClassObj = this.dateModule.Date || Date;
			this.dateLocaleModule = params.datePackage ? rias.getObject(params.datePackage + ".locale", false) : rias.dateLocale;
		},
		...
		buildRendering: function(){
			...
			var d = this.dowTemplateString,
				...
				dayOffset = this.dayOffset >= 0 ? this.dayOffset : rias.date.getFirstDayOfWeek(this.lang);
			this.dayCellsHtml = rias.substitute([d, d, d, d, d, d, d].join(""), {d: ""}, function(){
				return dayNames[dayOffset++ % 7];
			});

			// Markup for dates of the month (referenced from template), but without numbers filled in
			var r = rias.substitute(this.weekTemplateString, {d: this.dateTemplateString});
		...
		_connectControls: function(){
			...
			var connect = rias.hitch(this, function(nodeProp, part, amount){
				this[nodeProp].dojoClick = true;
				return rias.on(this[nodeProp], "click", rias.hitch(this, function(){
		...
		_setCurrentFocusAttr: function(/*Date*/ date, /*Boolean*/ forceFocus){
			...
			if(oldCell && oldCell !== newCell){
				if(rias.has("webkit")){    // see #11064 about webkit bug
		...
		_getFocusableNode: function(){
			return this.inherited(arguments, [this._setCurrentFocusAttr(this.currentFocus, true)]);
		},
		...
		_onDayClick: function(/*Event*/ evt){
			...
			if(node && !rias.dom.containsClass(node, "riaswCalendarDisabledDate")){
		...
		_markSelectedDates: function(/*Date[]*/ dates){
			...
			function mark(/*Boolean*/ selected, /*DomNode*/ cell){
				rias.dom.toggleClass(cell, "riaswCalendarSelectedDate", selected);
				cell.setAttribute("aria-selected", selected ? "true" : "false");
			}

			// Clear previously selected cells.
			rias.forEach(this._selectedCells || [], rias.partial(mark, false));

			// Mark newly selected cells.  Ignore dates outside the currently displayed month.
			this._selectedCells = rias.filter(rias.map(dates, this._getNodeByDate, this), function(n){
				return n;
			});
			rias.forEach(this._selectedCells, rias.partial(mark, true));
		},
	...
	CalendarLite._MonthWidget = rias.declare("riasw.widget.CalendarLite._MonthWidget", _WidgetBase, {
		// summary:
		...
		_setMonthAttr: function(month){
			...
			var monthNames = this.dateLocaleModule.getNames('months', 'wide', 'standAlone', this.lang, month),
				spacer =
					(rias.has("ie") === 6 ? "" : "<div class='riaswCalendarSpacer'>" +
						rias.map(monthNames,function(s){
							return "<div>" + s + "</div>";
						}).join("") + "</div>");
			...
			this.domNode.innerHTML =
				spacer +
					"<div class='riaswCalendarMonthLabel riaswCalendarCurrentMonthLabel'>" +
