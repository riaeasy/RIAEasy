...
], function(rias, Evented){

	function convertRegex(character){
		return character == '*' ? '.*' : character == '?' ? '.' : character;
	}

	var riaswType = "riasw.store.ObjectStore";
	return rias.declare(riaswType, [rias.Destroyable, Evented],{///因为 store 有 set 方法，故不能继承自 rias.ObjectBase
		// summary:
		...
		postMixInProperties: function(){
			if(!this.ownerRiasw){
				this.ownerRiasw = this.objectStore.getOwnerRiasw();
			}
		...
		whenInited: function(callback){
			if(this.objectStore.whenInited){
				return this.objectStore.whenInited(callback);
			}
			var self = this;
			return rias.when(true).always(function(result){
				callback.apply(self.objectStore, arguments);
			});
		},
		...
		getValue: function(/*Object*/ item, /*String*/property, /*value?*/defaultValue){
			...
			return typeof item.get === "function" ?
				item.get(property) :
				property in item ? item[property] : defaultValue;
		},
		...
		containsValue: function(item, attribute, value){
			...
			return rias.contains(this.getValues(item, attribute), value);
		},
		...
		loadItem: function(args){
			...
			if(typeof args.item.load === "function"){
				rias.when(args.item.load(), function(result){
		...
		fetch: function(args){
			...
			args = rias.delegate(args, args && args.queryOptions);
			var self = this;
			var scope = args.scope || self;
			var query = args.query;
			if(typeof query == "object"){ // can be null, but that is ignore by for-in
				query = rias.delegate(query); // don't modify the original
				for(var i in query){
			...
			rias.when(results.total, function(totalCount){
				rias.when(results, function(results){
					if(args.onBegin){
			...
			if(results.observe){
				...
				this.observing = results.observe(function(object, removedFrom, insertedInto){
					if(!rias.contains(self._dirtyObjects, object)){
						if(removedFrom == -1){
							self.onNew(object);
						}
		...
		getLabel: function(/* dojo/data/api/Item */ item){
			...
			if(this.isItem(item)){
				return this.getValue(item, this.objectStore.labelProperty); //String
			}
		...
		getLabelAttributes: function(/* dojo/data/api/Item */ item){
			...
			return [this.objectStore.labelProperty]; //array
		},
		...
		fetchItemByIdentity: function(args){
			...
			var item;
			rias.when(this.objectStore.get(args.identity),
				function(result){
					item = result;
		...
		save: function(kwArgs){
			...
			try{
				///c 跟随 kwArgs 的释放自动释放？ kwArgs = undefined ?
				var c = rias.after(kwArgs, "onError", self, function(){
					c.remove();
				...
				for(var i = 0; i < dirtyObjects.length; i++){
					...
					rias.when(result, function(value){
						if(!(--left)){
