define([
	"riasw/riaswBase",
	"riasw/form/_SearchMixin"
], function(rias, SearchMixin){

	// module:
	//		riasw/form/_AutoCompleterMixin

	var AutoCompleterMixin = rias.declare("riasw.form._AutoCompleterMixin", SearchMixin, {
		// summary:
		//		A mixin that implements the base functionality for `riasw/form/ComboBox`/`riasw/form/FilteringSelect`
		// description:
		//		All widgets that mix in riasw/form/_AutoCompleterMixin must extend `riasw/form/_FormWidgetMixin`.
		// tags:
		//		protected
		...
		_setCaretPos: function(/*DomNode*/ element, /*Number*/ location){
			...
			rias.dom.selectInputText(element, location, location);
		},
		...
		_onKey: function(/*Event*/ evt){
			...
			if(key === rias.keys.ALT || key === rias.keys.CTRL || key === rias.keys.META || key === rias.keys.SHIFT){
		...
		_autoCompleteText: function(/*String*/ text){
			...
			rias.dom.selectInputText(fn, fn.value.length);
			...
			if(text[caseFilter](0).indexOf(this.focusNode.value[caseFilter](0)) === 0){
				...
				if((cpos + 1) > fn.value.length){
					...
					rias.dom.selectInputText(fn, cpos);
				}
			}else{
				// text does not autoComplete; replace the whole value and highlight
				fn.value = text;
				rias.dom.selectInputText(fn);
			}
		},
		...
		_openResultList: function(/*Object*/ results, /*Object*/ query, /*Object*/ options){
			...
			this._nextSearch = this.dropDown.onPage = rias.hitch(this, function(direction){
			...
			this.dropDown.createOptions(
				results,
				options,
				rias.hitch(this, "_getMenuLabelFromItem")
			);
		...
		_setBlurValue: function(){
			...
			var pw = this.dropDown;
			if(pw && (newvalue === pw._messages.previousMessage || newvalue === pw._messages.nextMessage)){
				this.set("value", this._lastValueReported, true);
		...
		postCreate: function(){
			...
			this.inherited(arguments);
			this.after(this, "onSearch", rias.hitch(this, "_openResultList"), true);
		},
		...
		doHighlight: function(/*String*/ label, /*String*/ find){
			...
			find = rias.regexp.escapeString(find); // escape regexp special chars
			...
			return this._escapeHtml(label.replace(
				...
				'\uFFFF$1\uFFFF')).replace(
				/\uFFFF([^\uFFFF]+)\uFFFF/g, '<span class="riaswComboBoxHighlightMatch">$1</span>'
		...
		reset: function(){
			// Overrides the _FormWidgetMixin.reset().
		...
		labelFunc: function(item, store){
			...
			try{
				...
				console.error(this, "labelFunc() error: ", e);
				text = this.filter(this.get("value"), item);
			}
	...
	if(rias.has("riasw-bidi")){
		AutoCompleterMixin.extend({
			_onSetTextDir: function(/*String*/ textDir){
				this.inherited(arguments);
				// update the drop down also (_ComboBoxListMixin)
				if(this.dropDown){
