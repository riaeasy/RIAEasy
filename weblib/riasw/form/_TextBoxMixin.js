define([
	...
	var _TextBoxMixin = rias.declare("riasw.form._TextBoxMixin", null, {
		// summary:
		//		A mixin for textbox form input widgets

		_handleValue: true,
		enterAsTab: rias.has("riasw-enterAsTab"),
		editable: true,
		spellcheck: false,
		...
		postCreate: function(){
						...
						default:
							...
							this.defer(function(){
								if(this.textbox.value !== this._lastInputEventValue){
									rias.on.emit(this.textbox, "input", {bubbles: true});
								}
							});
					}
				}
			...
			this.own(
				rias.on(this.textbox, "keydown, keypress, paste, cut, compositionend", rias.hitch(this, handleEvent)),
				rias.on(this.textbox, "input", rias.hitch(this, "_onInput")),
				...
				rias.on(this.domNode, "keypress", function(e){ e.stopPropagation(); })
			);
		},

		reset: function(){
			// Overrides `riasw/_FormWidget/reset()`.
		...
		_setSpellcheckAttr: function(value){
			rias.dom.setAttr(this.textbox, "spellcheck", !!value);
		},
		...
		_getValueAttr: function(){
			...
			// description:
			//		For `riasw/form/TextBox` this basically returns the value of the `<input>`.
			//
			//		For `riasw/form/MappedTextBox` subclasses, which have both
		...
		_setValueAttr: function(value, /*Boolean?*/ priorityChange, /*String?*/ formattedValue){
			...
			if(value !== undefined){
				...
				if(typeof formattedValue !== "string"){
					...
					// example, riasw/form/FilteringSelect determines the formatted value asynchronously and applies it using a
		...
		_getDisplayedValueAttr: function(){
			...
			//		sent to the server (see `riasw/form/ValidationTextBox.serialize()`)
		...
		_refreshState: function(){
			...
			//		in `riasw/form/TextBox` does nothing, but subclasses override.
			// tags:
			//		protected
		},
		...
		_onFocus: function(/*String*/ by){
			...
			if(this.selectOnClick && by === "mouse"){
				...
				// the selection.
				this._selectOnClickHandle = rias.on.once(this.domNode, "mouseup, touchend", rias.hitch(this, function(evt){
					// Check if the user selected some text manually (mouse-down, mouse-move, mouse-up)
					// and if not, then select all the text
					if(!this._isTextSelected()){
						rias.dom.selectInputText(this.textbox);
					}
				}));
