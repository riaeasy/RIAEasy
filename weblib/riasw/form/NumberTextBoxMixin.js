define([
	...
	var getDecimalInfo = function(constraints){
		...
		var bundle = i18n.getLocalization("dojo.cldr", "number", i18n.normalizeLocale(constraints.locale)),
	...
	var riaswType = "riasw.form.NumberTextBoxMixin";
	var NumberTextBoxMixin = rias.declare(riaswType, null, {
		...
		pattern: function(constraints){
			...
			///忽略 focused 好些，只要有 editOptions 就按 editOptions 处理
			return '(' + (this.editOptions ? this._regExpGenerator(rias.delegate(constraints, this.editOptions)) + '|' : '') +
				this._regExpGenerator(constraints) + ')';
		},
		...
		_setConstraintsAttr: function(/*Object*/ constraints){
			...
			this._decimalInfo = getDecimalInfo(rias.delegate(constraints, this.editOptions));
		},
		...
		format: function(/*Number*/ value, /*number.__FormatOptions*/ constraints){
			...
			if(this.editOptions){///忽略 focused 好些，只要有 editOptions 就按 editOptions 处理
		...
		parse: function(/*String*/ value, /*number.__FormatOptions*/ constraints){
			...
			var parserOptions = rias.mixin({}, constraints, this.editOptions);///忽略 focused 好些，只要有 editOptions 就按 editOptions 处理
			...
			if(this.editOptions && isNaN(v)){///忽略 focused 好些，只要有 editOptions 就按 editOptions 处理
		...
		filter: function(/*Number*/ value){
			...
			//
			//		See `riasw/form/TextBox.filter()` for more details.
		...
		_getValueAttr: function(){
			...
			if(isNaN(v) && this.textbox.value !== ''){
				if(this.constraints.exponent !== false
					&& /\de[-+]?\d/i.test(this.textbox.value)
					&& (new RegExp("^" + number._realNumberRegexp(rias.delegate(this.constraints)) + "$").test(this.textbox.value))){	// check for exponential notation that parse() rejected (erroneously?)
		...
		isValid: function(/*Boolean*/ isFocused){
			// Overrides riasw/form/RangeBoundTextBox.isValid() to check that the editing-mode value is valid since
		...
		_isValidSubset: function(){
			// Overrides riasw/form/ValidationTextBox._isValidSubset()
			//
