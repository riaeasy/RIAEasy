...
define([
	...
	rias.has.add('FormData', function(){
		return !!window.FormData;
	});
	rias.has.add('xhr-sendAsBinary', function(){
		var xhr = window.XMLHttpRequest && new window.XMLHttpRequest();
		return xhr && !!xhr.sendAsBinary;
	});
	rias.has.add('file-multiple', function(){
		return !!({
			'true': 1,
			'false': 1
		}[rias.dom.getAttr(document.createElement('input',{type:"file"}), 'multiple')]);
	});

	var Widget = rias.declare("riasw.form.UploaderMixin", null, {
		// summary:
		...
		connectForm: function(){
			...
			if(!this._fcon && !!this.getForm()){
				this._fcon = true;
				this.on(this.form, "submit", function(evt){
					//console.debug('Uploader submit.');
					rias.stopEvent(evt);
		...
		supports: function(what){
			...
			switch(what){
				case "multiple":
					if(this.force === "iframe"){
						return false;
					}
					return rias.has("file-multiple");
				case "FormData":
					return rias.has(what);
				case "sendAsBinary":
					return rias.has("xhr-sendAsBinary");
			}
