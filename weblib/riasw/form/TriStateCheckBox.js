...
define([
	...
	//rias.loadThemeCss(["riasw/form/TriStateCheckBox.css"]);

	var riaswType = "riasw.form.TriStateCheckBox";
	var Widget = rias.declare(riaswType, [Button, _ToggleButtonMixin], {
		...
		templateString:
			'<div class="dijitReset" role="presentation">' +
				'<div class="riaswTriStateCheckBoxInner" data-dojo-attach-point="stateLabelNode"></div>' +
				'<input data-dojo-attach-point="focusNode" class="dijitReset riaswTriStateCheckBoxInput" data-dojo-attach-event="onclick:_onClick" type="${type}" role="${type}" ${!nameAttrSetting}/>' +
			'</div>',

		baseClass: "riaswTriStateCheckBox",
		...
		postCreate: function(){
			rias.dom.setAttr(this.stateLabelNode, 'innerHTML', this._stateLabels[this._stateType]);
		...
		_setCheckedAttr: function(/*String|Boolean*/ checked, /*Boolean?*/ priorityChange){
			...
			var stateIndex = rias.indexOf(this.states, checked);
			if(stateIndex >= 0){
				this._currentState = stateIndex;
				this._stateType = this._getStateType(checked);
				rias.dom.setAttr(this.focusNode, "value", this.stateValues[this._stateType]);
				rias.dom.setAttr(this.stateLabelNode, 'innerHTML', this._stateLabels[this._stateType]);
				this.inherited(arguments);
			}else{
		...
		_setStatesAttr: function(/*Array|String*/ states){
			if(rias.isArray(states)){
				this._set("states", states);
			}else if(rias.isString(states)){
				var map = {
					"true": true,
					"false": false,
					"mixed": "mixed"
		...
		_setValueAttr: function(/*String|Boolean*/ newValue, /*Boolean*/ priorityChange){
			...
			if(typeof newValue === "string" && (rias.indexOf(this.states, newValue) < 0)){
				if(newValue == ""){
					newValue = "on";
				}
			...
			if(this._created){
				this._currentState = rias.indexOf(this.states, newValue);
		...
		_onFocus: function(){
			if(this.id){
				rias.query("label[for='" + this.id + "']").addClass("riaswCheckBoxFocusedLabel");
			}
			this.inherited(arguments);
		},
		...
		_onBlur: function(){
			if(this.id){
				rias.query("label[for='" + this.id + "']").removeClass("riaswCheckBoxFocusedLabel");
			}
