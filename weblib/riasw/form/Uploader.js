...
define([
	...
	"riasw/xhr",
	"riasw/form/Button",
	"riasw/form/UploaderMixin",
	"riasw/sys/_TemplatedMixin",
	"dojo/i18n!./nls/Uploader"
],function(rias, riasXhr, Button, UploaderMixin, _TemplatedMixin){

	var HTML5 = rias.declare("riasw.form.uploader.HTML5", [], {
		...
		postCreate: function(){
			...
			if(this.uploadOnSelect){
				this.after(this, "onChange", function(data){
					this.upload(data[0]);
				}, true);
			}
		},
		...
		addDropTarget: function(node, /*Boolean?*/ onlyConnectDrop){
			...
			//		won't be connected to rias.stopEvent, and they need to be
			//		canceled by user code to allow DnD files to happen.
			//		This API is only available in HTML5 plugin (only HTML5 allows
			//		DnD files).
			if(!onlyConnectDrop){
				this.on(node, 'dragenter', rias.stopEvent);
				this.on(node, 'dragover', rias.stopEvent);
				this.on(node, 'dragleave', rias.stopEvent);
			}
			this.on(node, 'drop', '_drop');
		},
		...
		createXhr: function(){
			...
			xhr.onreadystatechange = rias.hitch(this, function(){
				if(xhr.readyState === 4){
					...
					clearInterval(timer);
					try{
						if(riasXhr.checkStatus(xhr.status)){
	...
	var IFrame = rias.declare("riasw.form.uploader.IFrame", [], {
		postMixInProperties: function(){
			this.inherited(arguments);
	...
	var riaswType = "riasw.form.Uploader";
	var Widget = rias.declare(riaswType, [Button, UploaderMixin, _TemplatedMixin, HTML5, IFrame], {
		// summary:
		...
		templateString:
			'<div role="button" data-dojo-attach-point="focusNode,buttonNode" class="dijitReset" data-dojo-attach-event="ondijitclick:_onClick" aria-labelledby="${id}_label">' +
				...
				/// 需要显式定义 labelNode
				'<div data-dojo-attach-point="labelNode" class="riaswButtonLabel riaswNoLabel" id="${id}_label"></div>' +
		...
		baseClass: "riaswButton riaswUploader",
		...
		_buildDisplay: function(){
			if(this.showInput){
				...
				this.after(this,'onChange', function(files){
					var i = 0,
						//l = files.length,
						f, r = [];
					while((f = files[i++])){
				...
				this.after(this, 'reset', function(){
		...
		//	console.warn("Uploader value is read only");
		//},
