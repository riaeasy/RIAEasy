...
//RIAStudio ClientWidget - form._TimePicker.

define([
	"riasw/riaswBase",
	"riasw/sys/_WidgetBase",
	"riasw/sys/_ListMixin"
], function(rias, _WidgetBase, _ListMixin){

	// module:
	//		riasw/form/_TimePicker


	var TimePicker = rias.declare("riasw.form._TimePicker", [_WidgetBase, _ListMixin], {
		// summary:
		//		A time picker dropdown, used by riasw/form/TimeTextBox.
		//		This widget is not available as a standalone widget due to lack of accessibility support.

		// baseClass: [protected] String
		//		The root className to use for the various states of this widget
		baseClass: "riaswTimePicker",
		...
		//		Must divide riasw/form/_TimePicker.visibleIncrement evenly
		...
		_createOption: function(/*Number*/ index){
			...
			var dateString = rias.dateLocale.format(date, this.constraints);
			...
			if(this.value && !rias.date.compare(this.value, date, this.constraints.selector)){
				div.selected = true;
		...
		show: function(){
			// Since _ListMixin::_setSelectedAttr() calls scrollIntoView(), shouldn't call it until list is visible.
			this.set("selectedNode", this._selectedNode);
		...
		_highlightOption: function(/*node*/ node, /*Boolean*/ highlight){
			...
			rias.dom.toggleClass(node, this.baseClass + "ItemHover", highlight);
			if(rias.dom.containsClass(node, this.baseClass + "Marker")){
				rias.dom.toggleClass(node, this.baseClass + "MarkerHover", highlight);
			}else{
				rias.dom.toggleClass(node, this.baseClass + "TickHover", highlight);
			}
		},

		handleKey: function(/*Event*/ e){
			// summary:
			//		Called from `riasw/form/_DateTimeTextBox` to pass a keypress event
			//		from the `riasw/form/TimeTextBox` to be handled in this widget
			// tags:
			//		protected
			if(e.keyCode === rias.keys.DOWN_ARROW){
				this.selectNextNode();
				this._onOptionSelected({target: this._highlighted_option}, false);
				e.stopPropagation();
				e.preventDefault();
				return false;
			}else if(e.keyCode === rias.keys.UP_ARROW){
				this.selectPreviousNode();
				this._onOptionSelected({target: this._highlighted_option}, false);
				e.stopPropagation();
				e.preventDefault();
				return false;
			}else if(e.keyCode === rias.keys.ENTER || e.keyCode === rias.keys.TAB){
				// mouse hover followed by TAB is NO selection
				if(!this._keyboardSelected && e.keyCode === rias.keys.TAB){
				...
				return e.keyCode === rias.keys.TAB;
			}
			return undefined;
		},

		// Implement abstract methods for _ListMixin
	...
	/*=====
	 TimePicker.__Constraints = declare(locale.__FormatOptions, {
		 // clickableIncrement: String
		 //		See `riasw/form/_TimePicker.clickableIncrement`
		 clickableIncrement: "T00:15:00"
	 });
	 =====*/
