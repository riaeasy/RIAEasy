...
define([
	...
	var dhm = rias.getObject("dom.html.metrics", true, rias);
	...
	dhm.getTextBox = function(/* String */ text, /* Object */ style, /* String? */ className){
		...
		// take a measure
		m.innerHTML = text;
		var box = rias.dom.getPosition(m);
		...
		if(rias.has("ie")){
			f.onreadystatechange = function(){
				if(f.contentWindow.document.readyState === "complete"){
					f.onresize = f.contentWindow.parent[rias._scopeName].dom.html.metrics._fontresize;
				}
			};
		}else{
			f.onload = function(){
				f.contentWindow.onresize = f.contentWindow.parent[rias._scopeName].dom.html.metrics._fontresize;
			};
		}
	...
	rias.on(window, "beforeunload", function(){
		// destroy our font resize iframe if we have one
		var f = dhm._fontResizeNode;
		if(f){
			if(rias.has("ie") && f.onresize){
				f.onresize = null;
	...
	rias.ready(function(){
		...
		if("fontSizeWatch" in rias.config && !!rias.config.fontSizeWatch){
