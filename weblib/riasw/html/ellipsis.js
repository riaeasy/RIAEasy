...
define([
	...
	return {
		...
		// description:
		//		Add "riaswEllipsis" on any node that you want to ellipsis-ize. In order to function properly,
		//		the node with the riaswEllipsis class set on it should be a child of a node with a defined width.
		//		It should also be a block-level element (i.e. `<div>`) - it will not work on td elements.
		//		NOTE: When using the riaswEllipsis class within tables, the table needs to have the table-layout: fixed style
	};
	=====*/
	
	if(rias.has("ff") < 7){ //TODO: feature detect text-overflow in computed style?
		// The delay (in ms) to wait so that we don't keep querying when many
		// changes happen at once - set config "rias-dom-FFEllipsisDelay" if you
		// want a different value
		var delay = 1;
		if("rias-dom-FFEllipsisDelay" in rias.config){
			delay = Number(rias.config["rias-dom-FFEllipsisDelay"]);
			if(isNaN(delay)){
				delay = 1;
			}
		}
		...
		var create = rias.dom.create;
		var dd = rias.dom.doc;
		var dp = rias.dom.place;
		var iFrame = create("iframe", {
			className: "riaswEllipsisIFrame",
		...
		var createIFrameEllipsis = function(/* Node */ n){
			...
			var c = create("div", {className: "riaswEllipsisContainer"});
			var e = create("div", {className: "riaswEllipsisShown", style: {display: "none"}});
		...
		var hc = rias.dom.hasClass;
		var doc = rias.dom.doc;
		var s, fn, opt;
		...
		var fx = function(){
			rias.forEach(s[fn].apply(s, [opt]), function(n){
				if(!n || n._djx_ellipsis_done){ return; }
				n._djx_ellipsis_done = true;
				if(createXULEllipsis && n.textContent === n.innerHTML && !hc(n, "riaswEllipsisSelectable")){
		...
		rias.ready(function(){
			...
			var connFx = function(){
				...
				t = setTimeout(function(){
					...
					// our next change
					c = rias.after(rias.dom.body(), "DOMSubtreeModified", connFx);
				}, delay);
			};
			connFx();
		});
