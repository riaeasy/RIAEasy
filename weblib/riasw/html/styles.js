...
define([
	...
	var dh = rias.getObject("dom.html", true, rias);
	...
	dh.activeStyleSheet = function(/*String?*/title){
		...
		if(arguments.length === 1){
			...
			rias.forEach(sheets, function(s){
	...
	dh.getStyleSheets = function(){
		...
		rias.forEach(sheets, function(n){
			...
			if(rias.has("ie")){
				...
				if(s.cssText.indexOf("#default#VML") === -1){
					if(s.href){
						...
						// Imported via @import
						rias.forEach(s.imports, function(si){
