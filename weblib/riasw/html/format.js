...
define([
	"riasw/hostDojo",
	"./entities"
], function(rias, Entities) {

	var dhf = rias.getObject("dom.html.format", true, rias);
	...
	dhf.prettyPrint = function(html/*String*/, indentBy /*Integer?*/, maxLineLength /*Integer?*/, map/*Array?*/, /*boolean*/ xhtml){
		...
		var contentDiv = rias.dom.doc.createElement("div");
				...
				if(maxLineLength && maxLineLength > 0){
					...
					while(txt){
						if(txt.length > maxLineLength){
							...
							txt = rias.trim(txt.substring((i === txt.length)?txt.length:i + 1, txt.length));
							if(line){
								_iTxt = "";
								for(i = 0; i < indentDepth; i++){
									_iTxt += iTxt;
		...
		var openTag = function(node){
			...
			var nText = rias.trim(outerHTML(node));
			...
			tag = tag.replace(rgxp_styleMatch, function(match){
				...
				style = rias.trim(style.substring(1,style.length -1));
				style = style.split(";");
				var trimmedStyles = [];
				rias.forEach(style, function(s){
					s = rias.trim(s);
					if(s){
		...
		var processNode = function(node) {
			...
			if(children){
				...
				for(i = 0; i < children.length; i++){
					var n = children[i];
					if(n.nodeType === 1){
						var tg = rias.trim(n.tagName.toLowerCase());
						if(rias.has("ie") && n.parentNode !== node){
