	...
	var rangeapi = {
		...
		getIndex: function(/*DomNode*/ node, /*DomNode*/ parent){
			...
			while(node !== parent){
				...
				//console.debug("Error finding index of a node in riasw/range.getIndex()");
				//}
				ret.unshift(i);
	...
	if(!window.getSelection){
		var ie = rangeapi.ie = {
			...
			setRange: function(range, startContainer, startOffset, endContainer, endOffset, collapsed){
				...
				if(!collapsed){
					start = ie.setEndPoint(range, endContainer, endOffset);
				}
				range.setEndPoint('EndToEnd', start);
