define([
	...
	"../range",
	"../../RichText"
], function(rias, _Plugin, rangeapi, RichText){

	// module:
	//		riasw/_editor/plugins/EnterKeyHandling

	var pluginsName = "riasw.editor.htmleditor.plugins",
		has = rias.has;

	var EnterKeyHandling = rias.declare(pluginsName + ".EnterKeyHandling", _Plugin, {
		// summary:
		...
		setEditor: function(editor){
			...
			if(this.blockNodeForEnter == 'BR'){
				...
				editor.onLoadDeferred.then(rias.hitch(this, function(d){
					this.own(rias.on(editor.document, "keydown", rias.hitch(this, function(e){
						if(e.keyCode == rias.keys.ENTER){
					...
					if(has("ie") >= 9 && has("ie") <= 10){
						this.own(rias.on(editor.document, "paste", rias.hitch(this, function(e){
				...
				var h = rias.hitch(this, "handleEnterKey");
				editor.addKeyHandler(13, 0, 0, h); //enter
				editor.addKeyHandler(13, 0, 1, h); //shift+enter
				this.own(this.editor.on('KeyPressed', rias.hitch(this, 'onKeyPressed')));
			}
		},
		...
		handleEnterKey: function(e){
				...
				if(header){
					...
					if(rangeapi.atBeginningOfContainer(header, range.startContainer, range.startOffset)){
						...
						if(rs && rs.nodeType == 3){
							...
							rias.dom.place(brNode, startNode, "after");
							rias.dom.place(endNode, brNode, "after");
							rias.dom.destroy(rs);
							newrange = rangeapi.create(this.editor.window);
						...
						if(range && range.startContainer){
							...
							if(rs && rs.nodeType == 3){
								...
								if(!targetNode){
									...
									rias.dom.place(brNode, targetNode, "before");
									rias.dom.place(endNode, brNode, "after");
								}
								newrange = rangeapi.create(this.editor.window);
								newrange.setStart(endNode, 0);
			...
			if(rangeapi.atEndOfContainer(block.blockNode, node, endOffset)){
				if(block.blockNode === block.blockContainer){
					...
					rias.dom.place(newblock, block.blockNode, "after");
				}
				...
				if(this.editor.height){
					rias.dom.scrollIntoView(newblock);
				}
			}else if(rangeapi.atBeginningOfContainer(block.blockNode,
				range.startContainer, range.startOffset)){
				rias.dom.place(newblock, block.blockNode, block.blockNode === block.blockContainer ? "first" : "before");
				if(newblock.nextSibling && this.editor.height){
					...
					rias.dom.scrollIntoView(newblock.nextSibling);
				}
				_letBrowserHandle = false;
				...
				if(rs && rs.nodeType == 3){
					...
					rias.dom.place(endNode, rs, "after");
					rias.dom.destroy(rs);
		...
		removeTrailingBr: function(container){
			...
			if(para.lastChild){
				if((para.childNodes.length > 1 && para.lastChild.nodeType == 3 && /^[\s\xAD]*$/.test(para.lastChild.nodeValue)) ||
					...
					rias.dom.destroy(para.lastChild);
				}
			}
	...
	_Plugin.registry["enterKeyHandling"] = _Plugin.registry["enterkeyhandling"] = function(args){
		return new EnterKeyHandling(args);
	};
