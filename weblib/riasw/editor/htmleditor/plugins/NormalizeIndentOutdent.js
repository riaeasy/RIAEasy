define([
	...
	"../_Plugin",
	"../range"
], function(rias, _Plugin, rangeapi) {

	var pluginsName = "riasw.editor.htmleditor.plugins";

	var NormalizeIndentOutdent = rias.declare(pluginsName + ".NormalizeIndentOutdent", _Plugin, {
		// summary:
		...
		setEditor: function(editor){
			...
			editor._indentImpl = rias.hitch(this, this._indentImpl);
			editor._outdentImpl = rias.hitch(this, this._outdentImpl);
			...
			editor.queryCommandEnabled = rias.hitch(this, this._queryCommandEnabled);
		...
		_queryCommandEnabled: function(command){
			...
			if(c === "indent"){
				ed = this.editor;
				sel = rangeapi.getSelection(ed.window);
				...
				if(range.startContainer === range.endContainer){
					...
					if(this._isRootInline(range.startContainer)){
						...
						if(start){
							...
							rias.dom.place(div, start, "after");
					...
					while(curNode && curNode !== end){
						if(curNode.nodeType === 1){
							...
							if(tag === "li"){
								...
								if(!div){
									div = ed.document.createElement("div");
									rias.dom.place(div, curNode, "after");
		...
		_indentElement: function(node){
			...
			rias.dom.setStyle(node, style, cIndent);
			...
			if(type){
				...
				if(prevTag && prevTag.tagName.toLowerCase() == "li"){
					...
					if(embList){
						...
						rias.dom.setStyle(newList, {
							paddingTop: "0px",
							paddingBottom: "0px"
						});
						li = ed.document.createElement("li");
						rias.dom.setStyle(li, {
							listStyleImage: "none",
							listStyleType: "none"
						});
		...
		_outdentList: function(listItem){
			...
			if(lpTg === "li" || lpTg === "ol" || lpTg === "ul"){
				if(lpTg === "ol" || lpTg === "ul"){
					...
					if(prevListLi){
						...
						if(firstItem !== listItem){
							rias.dom.place(firstItem, list, "before");
							...
							li = ed.document.createElement("li");
							rias.dom.place(li, list, "before");
							li.appendChild(list);
							listParent = li;
						}
						rias.dom.setStyle(list, {
							paddingTop: "0px",
							paddingBottom: "0px"
						});
					}
				...
				if(!prevLi){
					...
					rias.dom.place(listItem, listParent, "after");
		...
		_isLtr: function(){
			...
			var cs = rias.dom.getComputedStyle(editDoc);
...
// Register this plugin.
	_Plugin.registry["normalizeIndentOutdent"] = _Plugin.registry["normalizeindentoutdent"] = function(args){
		return new NormalizeIndentOutdent(rias.mixin(args, {
			indentBy: args.indentBy > 0 ? args.indentBy : 40,
			indentUnits: args.indentUnits && args.indentUnits.toLowerCase() === "em" ? "em" : "px"
		}));
	};
