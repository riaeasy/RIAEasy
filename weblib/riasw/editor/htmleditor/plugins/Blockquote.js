define([
	...
	"../_Plugin",
	"../range",
	"dojo/i18n!./nls/Blockquote"
], function(rias, ToggleButton, _Plugin, rangeapi) {

	var pluginsName = "riasw.editor.htmleditor.plugins";

	var Blockquote = rias.declare(pluginsName + ".Blockquote", _Plugin, {
		// summary:
		...
		_initButton: function(){
			...
			this._nlsResources = rias.i18n.getLocalization(pluginsName, "Blockquote");
			this.button = new ToggleButton({
				ownerRiasw: this,
				disabled: this.get("disabled"),
				readOnly: this.get("readOnly"),
				label: this._nlsResources.blockquote,
				tooltip: this._nlsResources.blockquote,
				showLabel: false,
				iconClass: this.iconClassPrefix + " " + this.iconClassPrefix + "Blockquote",
				tabIndex: "-1",
				onClick: rias.hitch(this, "_toggleQuote")
			});
		},
		...
		setEditor: function(editor){
			...
			this.editor = editor;
			this._initButton();
			//this.after(this.editor, "onNormalizedDisplayChanged", "updateState", true);
			this.subscribe(editor.id + "_normalizedDisplayChanged", function(){
				this.updateState();
			});
						...
						if(range.startContainer === range.endContainer){
							...
							if(this._isRootInline(range.startContainer)){
								...
								while ((this._isTextElement(node) ||
									this._isInlineFormat(this._getTagName(node)) || this._getTagName(node) === "li") && node !== ed.editNode && node !== ed.document.body){
									node = node.parentNode;
								}
								if(node !== ed.editNode && node !==	node.ownerDocument.documentElement){
									bq = ed.document.createElement("blockquote");
									rias.dom.place(bq, node, "after");
									bq.appendChild(node);
								}
							}
							if(bq){
								...
								if(curNode.nodeType === 1){
									...
									if(tag !== "br"){
										...
										if(this._isInlineFormat(tag)){
											...
											if(!bq){
												bq = ed.document.createElement("blockquote");
												rias.dom.place(bq, curNode, "after");
						...
						if(range.startContainer === range.endContainer){
							...
							if(found){
								...
								while(elem.firstChild){
									...
									rias.dom.place(lastChild, elem, "before");
								}
							...
							while(bnNodes.length){
								...
								if(bn.parentNode){
									...
									while(bn.firstChild){
										rias.dom.place(bn.firstChild, bn, "before");
									}
	...
	_Plugin.registry.blockquote = function(args){
		return new Blockquote(args);
	};
