define([
	...
	// module:
	//		riasw/_editor/plugins/FullScreen
		...
		_containFocus: function(e){
			...
			if(this.isFullscreen){
				...
				if(!ed.isTabIndent &&
					...
					var f = this._focusManager.get("currNode");
		...
		_setFullScreen: function(full){
			...
			if(full){
				...
				while(editorParent && editorParent !== body){
					rias.dom.addClass(editorParent, "riaswForceStatic");
				...
				this._resizeHandle2 = rias.after(ed, "onResize", function(){
					if(self._resizer){
						clearTimeout(self._resizer);
						delete self._resizer;
					}
					self._resizer = setTimeout(function(){
						delete self._resizer;
						self._resizeEditor();
					}, 10);
				});
				...
				while(editorParent && editorParent !== body){
					rias.dom.removeClass(editorParent, "riaswForceStatic");
					editorParent = editorParent.parentNode;
				}
