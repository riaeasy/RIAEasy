define([
	...
	"../range",
	"./EnterKeyHandling"
], function(rias, Button, _Plugin, rangeapi, EnterKeyHandling) {

	var pluginsName = "riasw.editor.htmleditor.plugins";

	var AutoUrlLink = rias.declare(pluginsName + ".AutoUrlLink", [_Plugin], {
		// summary:
		...
		setEditor: function(/*riasw.editor.Editor*/ editor){
			// summary:
			...
			if(!rias.has("ie")){
				...
				rias.some(editor._plugins, function(plugin){
					// Need to detect which enter key mode it is now
					if(plugin.isInstanceOf(EnterKeyHandling)){
				...
				this.after(editor, "onClick", "_recognize", true);
				this.after(editor, "onBlur", "_recognize", true);
			}
		},
		...
		_keyPress: function(evt){
			...
			// tags:
			//		protected
			var ks = rias.keys, v = 118, V = 86,
				kc = evt.keyCode, cc = evt.charCode;
			if(cc == ks.SPACE || (evt.ctrlKey && (cc == v || cc == V))){
				setTimeout(rias.hitch(this, "_recognize"), 0);
			}else if(kc == ks.ENTER){
			...
			if(selection){
				...
				if(node.nodeType == 3 && !this._inLink(node)){
					...
					while(item){
						...
						ed.execCommand("insertHTML", rias.substitute(template, {url: range.toString()}));
						cost += item.end;
						item = result.shift();
						linked = true;
					}
		...
		_findLastEditingNode: function(/*DomNode*/ node){
			...
			// tags:
			//		private
			var blockTagNames = rangeapi.BlockTagNames,
				editNode = this.editor.editNode, blockNode;

			if(!node){ return node; }
			if(this.blockNodeForEnter == "BR" &&
				(!(blockNode = rangeapi.getBlockAncestor(node, null, editNode).blockNode) ||
				...
				if((blockNode || (blockNode = rangeapi.getBlockAncestor(node, null, editNode).blockNode)) &&
					blockNode.tagName.toUpperCase() == "LI"){
					node = blockNode;
				}else{
					node = rangeapi.getBlockAncestor(node, null, editNode).blockNode;
				}
				...
				if(node){
					node = node.lastChild;
					while(node){
						if(node.nodeType == 3 && rias.trim(node.nodeValue) != ""){
							break;
	...
	_Plugin.registry["autoUrlLink"] = _Plugin.registry["autourllink"] = function(args){
		return new AutoUrlLink(args);
	};
