define([
	...
	// module:
	//		riasw/_editor/plugins/AlwaysShowToolbar
	...
	return rias.declare(pluginsName + ".AlwaysShowToolbar", _Plugin, {
		...
		//	|		AlwaysShowToolbar: "riasw/editor/htmleditor/plugins/AlwaysShowToolbar"
		//	|	</script>
		//	|	<div data-dojo-type="Editor" height=""
		...
		globalOnScrollHandler: function(){
			...
			if(!this._scrollSetUp){
				...
				this._scrollThreshold = rias.dom.getPosition(tdn, true).y;
			}
			...
			if(scrollPos > this._scrollThreshold && scrollPos < this._scrollThreshold + this._lastHeight){
				...
				if(!this._fixEnabled){
					...
					if(isIE6){
						s.left = rias.dom.getPosition(tdn).x;
						...
						rias.dom.addClass(tdn, 'riaswIEFixedToolbar');
					}else{
						s.position = "fixed";
						s.top = "0px";
					}
				...
				if(isIE6){
					s.left = "";
					rias.dom.removeClass(tdn, 'riaswIEFixedToolbar');
		...
		_onDestroy: function(){
			...
			if(rias.has("ie") < 7){
				rias.dom.removeClass(this.editor.header, 'riaswIEFixedToolbar');
			}
