define([
	...
	"../_Plugin",
	"../html"
], function(rias, _Plugin, htmlapi) {

	var pluginsName = "riasw.editor.htmleditor.plugins";

	var NormalizeStyle = rias.declare(pluginsName + ".NormalizeStyle", _Plugin,{
		// summary:
		...
		setEditor: function(editor){
			...
			if(this.mode === "semantic"){
				this.editor.contentDomPostFilters.push(rias.hitch(this, this._convertToSemantic));
			}else if(this.mode === "css"){
				this.editor.contentDomPostFilters.push(rias.hitch(this, this._convertToCss));
			}
			...
			if(rias.has("ie")){
				...
				this.editor.contentDomPreFilters.push(rias.hitch(this, this._convertToSemantic));
				this._browserFilter = this._convertToSemantic;
			}else if(rias.has("webkit")){
				this.editor.contentDomPreFilters.push(rias.hitch(this, this._convertToCss));
				this._browserFilter = this._convertToCss;
			}else if(rias.has("mozilla")){
			...
			this.editor._inserthtmlImpl = rias.hitch(this, this._inserthtmlImpl);
		},
					...
					if(cNode.nodeType == 1){
						if(cNode.id !== "riaswEditorBody"){
							...
							if(style && tag != "table" && tag != "ul" && tag != "ol"){
								...
								var f = style.fontFamily ? style.fontFamily.toLowerCase() : "";
								...
								var wrapNodes = function(wrap, pNode){
									if(wrap){
										...
										if(tag == "span" && !pNode.style.cssText){
											...
											rias.dom.place(wrap, pNode, "before");
								...
								if(f){
									...
									sTag.setAttribute("face", f);
									cNode.style.fontFamily = "";
								}
						...
						if(cNode.childNodes){
							...
							var nodes = [];
							rias.forEach(cNode.childNodes, function(n){ nodes.push(n);});
							rias.forEach(nodes, convertNode);
						}
					}
					return cNode;
				};
		...
		_normalizeTags: function(node){
			...
			var doc = this.editor.document;
			rias.dom.query("em,s,strong", node).forEach(function(n){
							...
							if(tag){
								...
								switch(tag){
									...
									case "font": // Try to deal with colors
										var styles = {};
										if(rias.dom.getAttr(cNode, "color")){
											styles.color = rias.dom.getAttr(cNode, "color");
										}
										if(rias.dom.getAttr(cNode, "face")){
											//styles.fontFace = rias.dom.getAttr(cNode, "face");
											styles.fontFamily = rias.dom.getAttr(cNode, "face");
										}
										...
										if(rias.dom.getAttr(cNode, "size")){
											styles.fontSize = sizeMap[rias.dom.getAttr(cNode, "size")];
										}
										span = doc.createElement("span");
										rias.dom.setStyle(span, styles);
										break;
								}
								if(span){
									...
									rias.dom.place(span, cNode, "before");
			...
			var compressSpans = function(node){
				...
				var genStyleMap = function(styleText){
					...
					if(styleText){
						...
						rias.forEach(styles, function(s){
							if(s){
								var ss = s.split(":");
								var key = ss[0] ? rias.trim(ss[0]): "";
								var val = ss[1] ? rias.trim(ss[1]): "";
								if(key && val){
									var i;
									var nKey = "";
				...
				if(node && node.nodeType == 1){
					...
					if(tag === "span" && node.childNodes && node.childNodes.length === 1){
						...
						while(c && c.nodeType == 1 && c.tagName && c.tagName.toLowerCase() == "span"){
							if(!rias.dom.getAttr(c, "class") && !rias.dom.getAttr(c, "id") && c.style){
								...
								if(s1 && s2){
									...
									if(combinedMap){
										...
										rias.dom.setStyle(node, combinedMap);
										while(c.firstChild){
				...
				if(node.childNodes && node.childNodes.length){
					rias.forEach(node.childNodes, compressSpans);
				}
			};
			compressSpans(node);
		},
		...
		_inserthtmlImpl: function(html){
			...
			if(html){
				...
				html = htmlapi.getChildrenHtml(div);
				div.innerHTML = "";
...
// Register this plugin.
	_Plugin.registry["normalizeStyle"] = _Plugin.registry["normalizestyle"] = function(args){
		return new NormalizeStyle(args);
	};
