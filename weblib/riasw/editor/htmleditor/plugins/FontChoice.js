define([
...
], function(rias, _Plugin, rangeapi, _WidgetBase, _TemplatedMixin, FilteringSelect, MemoryStore){

	// module:
	//		riasw/_editor/plugins/FontChoice
	var pluginsName = "riasw.editor.htmleditor.plugins";

	var _FontDropDown = rias.declare(pluginsName + "._FontDropDown", [_WidgetBase, _TemplatedMixin], {
			// summary:
			...
			templateString:
				"<span style='white-space: nowrap' class='dijitReset dijitInline'>" +
					"<label class='dijitLeft dijitInline' for='${selectId}'>${label}</label>" +
					"<input data-dojo-type='riasw.form.FilteringSelect' required='false' data-dojo-props='labelType:\"html\", labelAttr:\"label\", searchAttr:\"name\"' class='${comboClass}' " +
			...
			postMixInProperties: function(){
				...
				this.strings = rias.i18n.getLocalization(pluginsName, "FontChoice");

				// Set some substitution variables used in the template
				this.label = this.strings[this.command];
				this.tooltip = this.strings[this.command];
			...
			postCreate: function(){
				...
				this.select.set("store", new MemoryStore({
					...
					idProperty: "value",
					labelProperty: "label",
					data: rias.map(this.values, function(value){
			...
			_setValueAttr: function(value, priorityChange){
				...
				this.select.set('value', rias.indexOf(this.values, value) < 0 ? "" : value, priorityChange);
				if(!priorityChange){
			...
			focus: function(node, forceVisible){
				...
				if(!this.isDestroyed(true)){
					this.select.focus(node, forceVisible);
				}
			},
	...
	var _FontNameDropDown = rias.declare(pluginsName + "._FontNameDropDown", _FontDropDown, {
		// summary:
		...
		postMixInProperties: function(){
			...
			if(rias.isArray(this.extraFonts)){
				rias.concatUnique(this.values, this.extraFonts);
			}
			this.values.unshift("_default");
		},
		...
		getLabel: function(value, name){
			...
			if(this.plainText || value === "_default"){
				return name;
			}else{
				return "<span style='font-family: " + value + "'>" + name + "</span>";
			}
		},
	...
	var _FontSizeDropDown = rias.declare(pluginsName + "._FontSizeDropDown", _FontDropDown, {
		// summary:
		...
		comboClass: "riaswFontSizeCombo",

		// values: [public] Number[]
		//		The HTML font size values supported by this plugin
		values: [3, 1, 2, 4, 5, 6, 7], // sizes according to the old HTML FONT SIZE
		...
		getLabel: function(value, name){
			...
			if(this.plainText || value === "_default"){
				...
				return "<span style='font-size: " + value + "'>" + name + "</span>";
			}
		},
	...
	var _FormatBlockDropDown = rias.declare(pluginsName + "._FormatBlockDropDown", _FontDropDown, {
		// summary:
		...
		comboClass: "riaswFormatBlockCombo",
		...
		_execCommand: function(editor, command, choice){
			...
			if(choice === "noFormat"){
				var self = this,
					...
					sel = rangeapi.getSelection(editor.window);
				if(sel && sel.rangeCount > 0){
					...
					if(range){
						...
						var processChildren = function(node, ary){
								...
								for(i = 0; i < node.childNodes.length; i++){
									...
									if(c.nodeType === 1){
										if(editor.selection.inSelection(c)){
											...
											if(rias.indexOf(self.values, tag) !== -1){
												ary.push(c);
											}
						...
						var unformatNodes = function(nodes){
							// summary:
							...
							if(nodes && nodes.length){
								editor.beginEditing();
								while(nodes.length){
									self._removeFormat(editor, nodes.pop());
								}
								editor.endEditing();
							}
						};
		...
		_removeFormat: function(editor, node){
			...
			if(editor.customUndo){
				...
				while(node.firstChild){
					rias.dom.place(node.firstChild, node, "before");
				}
	...
	var FontChoice = rias.declare(pluginsName + ".FontChoice", _Plugin, {
		// summary:
		...
		//
		//	|	plugins="[{name:'riasw._editor.plugins.FontChoice', command:'fontName', values:['Verdana','Myriad','Garamond']},...]"
		//
		...
		//		processing is handled by this plugin, not by Editor.
		...
		_initButton: function(){
			...
			var self = this,
				editor = this.editor;
			this.button = new clazz(rias.delegate({
				ownerRiasw: this,
			...
			this.own(this.button.select.on("change", function(choice){
				...
				if(self.command === "fontName" && choice.indexOf(" ") !== -1){
					choice = "'" + choice + "'";
				}

				// Invoke, the editor already normalizes commands called through its
				// execCommand.
				if(self.button._execCommand){
					self.button._execCommand(editor, self.command, choice);
				}else{
					if(!editor.execCommand(self.command, choice)){
						//if(self.command == "fontSize" && !(choice >= 1 && choice <= 7)){
						//	editor.document.execCommand("fontsize", true, choice);
						//}
					}
				}
			}));
		...
		updateState: function(){
			...
			if(this.button){
				...
				var quoted = rias.isString(value) && (value.match(/'([^']*)'/) || value.match(/"([^"]*)"/));
				if(quoted){
					value = quoted[1];
				}

				if(_c === "fontName"){
					if(!value || rias.indexOf(this.button.values, value) < 0){
					...
					if(!value || value === "p"){
						...
						var sel = rangeapi.getSelection(_e.window);
						if(sel && sel.rangeCount > 0){
							var range = sel.getRangeAt(0);
							if(range){
						...
						while(elem && elem !== _e.editNode && elem !== _e.document){
							...
							if(tg && rias.indexOf(this.button.values, tg) > -1){
								value = tg;
								break;
							}
						...
						if(rias.indexOf(this.button.values, value) < 0){
							value = "noFormat";
						}
					}
				}
	...
	_Plugin.registry.fontName = _Plugin.registry.fontname = function(args){
		return new FontChoice(rias.mixin({
			command: "fontName"
		}, args));
	};
	_Plugin.registry.fontSize = _Plugin.registry.fontsize = function(args){
		return new FontChoice(rias.mixin({
			command: "fontSize"
		}, args));
	};
	_Plugin.registry.formatBlock = _Plugin.registry.formatblock = function(args){
		return new FontChoice(rias.mixin({
			command: "formatBlock"
		}, args));
	};
