...
define([
...
], function(rias, fx, _WidgetBase, _CssStateMixin,
			util, config, i18n, shim, mEditor, mEditorFeatures,
	...
	function getDisplay(window, document, element) {
		...
		while (temp && temp !== document && display !== "none") { //$NON-NLS-0$
			...
			display = rias.dom.getComputedStyle(temp).display;
	...
	mTextView.TextView.prototype.update = function(styleChanged, sync) {
		...
		//if (this._parent.__riasrWidget && !this._parent.__riasrWidget.visible) {
		if (!rias.dom.isVisible(this._parent, true)) {
	...
	mTextView.TextView.prototype._queueUpdate = function() {
		//if (this._updateTimer || this._ignoreQueueUpdate || this._parent.__riasrWidget && !this._parent.__riasrWidget.visible) {
		if (this._updateTimer || this._ignoreQueueUpdate || !rias.dom.isVisible(this._parent, true)) {
			return;
		}
		var self = this;
	...
	var riaswType = "riasw.editor.OrionEditor";
	var Widget =  rias.declare(riaswType, [_WidgetBase, _CssStateMixin], {
		...
		buildRendering: function(){
			...
			if (!this.editor) {
				...
				this.textView._AUTO_SCROLL_RATE = _AUTO_UPDATE_DELAY;
			}
		...
		_destroyEditor: function() {
			...
			if(this.contentAssistNode){
				rias.destroy(this.contentAssistNode);
				delete this.contentAssistNode;
			}
		},
		_onDestroy: function () {
			this._destroyEditor();
		...
		_createEditor: function (options) {
			...
			var parentNode = self.domNode;//options.parentNode || self.domNode;
			...
			if (typeof options.theme === "string") { //$NON-NLS-0$
				...
				theme.setThemeClass(themeClass, {
					href: options.theme
				});
				options.theme = theme;
			}
			var textViewFactory = function() {
				...
				self.after(tv, "_handleKeyDown", function(e){
					if(e.keyCode === 32){
						if (e.preventDefault) {
							e.preventDefault();
							e.stopPropagation();
			...
			self.contentAssistNode = self.ownerDocument.createElement("div");
			rias.dom.setAttr(self.contentAssistNode, "id", self.id + "_contentAssist");
			//if (!options.readOnly) {
				contentAssistFactory = {
					createContentAssistMode: function(editor) {
						contentAssist = new mContentAssist.ContentAssist(editor.getTextView());
						var contentAssistWidget = new mContentAssist.ContentAssistWidget(contentAssist, self.contentAssistNode);
			...
			if (contents === undefined) {
				contents = getTextFromElement(parentNode);
			}
			...
			if (!options.noComputeSize && getDisplay(window, doc, parentNode) !== "none" && getHeight(parentNode) <= 50) { //$NON-NLS-0$
				var height = editor.getTextView().computeSize().height;
			 parentNode.style.height = height + "px"; //$NON-NLS-0$
			}*/
