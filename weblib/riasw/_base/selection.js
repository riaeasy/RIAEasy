define([
	...
	var SelectionManager = function(win){
		...
		this.selectElement = function(/*DomNode*/ element, /*Boolean?*/ nochangefocus){
			...
			var range;
			element = rias.dom.byId(element);	// TODO: remove for 2.0 or sooner, spec listed above doesn't allow for string
			if(doc.getSelection){
				// W3C path
				var selection = doc.getSelection();
				range = doc.createRange();
				...
				try{
					...
					if(tg === "img" || tg === "table"){
						range = rias.dom.body(doc).createControlRange();
					}else{
						range = rias.dom.body(doc).createRange();
					}
