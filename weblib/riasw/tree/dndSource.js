define([
	...
	"./_dndSelector"
], function(rias, dndCommon, DNDManager, _dndSelector){
	...
	var dndSource = rias.declare("riasw.tree.dndSource", _dndSelector, {
		// summary:
		//		Handles drag and drop operations (as a source or a target) for `riasw.tree.Tree`
		...
		constructor: function(/*riasw/tree/Tree*/ tree, /*riasw/tree/dndSource*/ params){
			// summary:
			...
			this.sourceState = "";
			if(this.isSource){
				rias.dom.addClass(this.node, "dojoDndSource");
			}
			this.targetState = "";
			if(this.accept){
				rias.dom.addClass(this.node, "dojoDndTarget");
			}

			// set up events
			this.topics = [
				rias.subscribe("/dnd/source/over", rias.hitch(this, "onDndSourceOver")),
				rias.subscribe("/dnd/start", rias.hitch(this, "onDndStart")),
				rias.subscribe("/dnd/drop", rias.hitch(this, "onDndDrop")),
				rias.subscribe("/dnd/cancel", rias.hitch(this, "onDndCancel"))
			];
		},
		destroy: function(){
		...
		checkItemAcceptance: function(/*===== target, source, position =====*/){
			...
			//		Use rias.by(target) to get the TreeNode.
			// source: riasw/tree/dndSource
		...
		itemCreator: function(nodes /*===== , target, source =====*/){
			...
			return rias.map(nodes, function(node){
				return {
					"id": node.id,
		...
		_onDragMouse: function(e, firstTime){
			...
			if(newTarget && this.betweenThreshold > 0){
				...
				if(!this.targetBox || oldTarget !== newTarget){
					this.targetBox = rias.dom.getPosition(newTarget.rowNode, true);
				}
			...
			if(this.isDragging){
				...
				if(this.mouseDown && this.isSource &&
					...
					if(nodes.length){
						...
						nodes = rias.map(nodes, function(n){
							return n.domNode;
						});
		...
		onMouseDown: function(e){
			...
			if(e.type === "touchstart" || rias.mouse.isLeft(e)){	// ignore right click
				this.mouseDown = true;
		...
		onDndSourceOver: function(source){
			...
			// source: Object
			//		The riasw/tree/dndSource / dojo/dnd/Source which has the mouse over it
			// tags:
			//		private
			if(this !== source){
				this.mouseDown = false;
		...
		onDndStart: function(source, nodes, copy){
			...
			// source: Object
			//		The riasw/tree/dndSource / dojo/dnd/Source which is providing the items
			// nodes: DomNode[]
		...
		onDndDrop: function(source, nodes, copy){
			...
			if(this.containerState === "Over"){
				...
				if(this.dropPosition === "Before" || this.dropPosition === "After"){
					...
					insertIndex = target.getIndexInContainer();
					if(this.dropPosition === "After"){
						insertIndex = target.getIndexInContainer() + 1;
				...
				rias.forEach(nodes, function(node, idx){
					...
					if(rias.indexOf(sourceItem.type, "treeNode") !== -1){
						childTreeNode = sourceItem.data;
					...
					if(source === this){
						...
						// TODO: riasw/tree/dndSelector should implement deleteSelectedNodes()
						// and this code should go there.

						if(typeof insertIndex === "number"){
							if(newParentItem === oldParentItem && childTreeNode.getIndexInContainer() < insertIndex){
								insertIndex -= 1;
							}
						}
