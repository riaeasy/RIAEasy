...
define([
	...
	rias.getIdPath = function(widget){
		var a = [];
		while(rias.isRiasObject(widget)){
			if(widget._riaswIdInModule){
				a.unshift(widget._riaswIdInModule);
				widget = widget.getOwnerModule();
			}else if(widget._riaswAttachPoint){
				a.unshift(widget._riaswAttachPoint);
				widget = widget.getOwnerRiasw();
			}else if(rias.isRiaswDesktop(widget)){
				a.unshift(widget.id);
				widget = undefined;
			}else{
				return widget.id;
	...
	rias.getModuleCaptionPath = function(widget){
		var a = [];
		while(rias.isRiasObject(widget)){
			if(rias.isRiaswDesktop(widget)){
				//a.unshift(widget.desktopTitle);
				widget = undefined;
			}else if(rias.isRiaswModule(widget)){
				a.unshift(widget.caption ? widget.caption.replace(/<[^>]+>/g, "") : "");
				widget = widget.getOwnerModule();
			}else{
				widget = widget.getOwnerModule();
			}
	...
	rias.ownerDialogBy = function(any){
		...
		if(w){
			...
			if(!rias.isRiaswDialog(w)){
				w = arguments.callee(w.getOwnerModule());
			}
		}
		return w;
	};
	rias.desktopBy = function(any){
		/// desktopBy 取 ownerModule
		var w = rias.by.apply(this, arguments || []);
		if(w){
	...
	rias.parentSceneBy = function(any){
		/// parentSceneBy 取 containerRiasw
		var w = rias.by.apply(this, arguments || []);
		if(w){
			if(!rias.isRiaswScene(w)){
...
///dom******************************************************************************///
	...
	rias.mixin(rias.hash, {
		...
		getTargetParams: function(hash){
			var p = hash.split("&"),
				r = {}, ai, name, value;
			p.shift();
			rias.forEach(p, function(item){
				ai = item.split("=");
				name = ai[0];
				value = decodeURIComponent(ai[1] || "");
				if(name && value){
					r[name] = value;
				}
			});
		...
		buildHash: function(/*String*/ hash, /*riasWidget*/sender, /*Object*/params){
			if(hash.charAt(0) !== "#"){
				hash = "#" + hash;
			}
			sender = rias.by(sender);
			if(sender){
				//hash += rias.getIdPath(sender);
				hash += sender.id;
			}
			if(rias.isObjectSimple(params)){
				var item,
					ps = "";
				for(item in params){
					if(item && params[item] != null){
						ps += "&" + item + "=" + encodeURIComponent(params[item]);
					}
				}
				if(ps){
					hash += ps;
		...
		updateHash: function(sender, detail, triggerEvent){
			// summary:
			//		Dispatches this transition event. Emits a "startTransition" event on the target.
			/// triggerEvent 是 sender 监听到的“原始事件”
			sender = rias.by(sender);
			///注意： detail 不能有 复杂对象
			detail.sender = sender.id;
			if(sender){
				rias.on.emit(sender.domNode, "desktop-updateHash", {
					bubbles: true,
					cancelable: true,
					detail: detail,
